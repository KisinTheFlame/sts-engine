...
当前能打的战斗：铁甲，**63 / 63 个编队**、**65 / 65 只怪**（两条线都已收官）、108 张牌、
87 颗遗物、28 瓶药水。爬升度这条轴在前三幕的 54 个编队上有 asc0 与 asc19 两档；
第四幕与事件编队目前只有 asc0。

背书方式是逐帧对拍：`test/golden/traces` 下 214 份 trace / **43305 场战斗**由参考项目真实的
`BattleContext` 产出，重放侧消费同一份动作序列、逐帧比对全量状态快照。

**run 层仍是近似的**：事件、商店、奖励（卡 / 金币 / 药水 / 遗物掉落）、篝火，以及
「哪场仗出现在第几层」的接线（`sts-map` / `sts-encounters` / `sts-neow` 三个游戏级模块
都已经写好，但还没接进 run）。整套完成前**不会发布新版本**。进度与拆解见 [TODOS.md](TODOS.md)。
