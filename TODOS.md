...
| 编队 | 13      | 63                | `ENCOUNTER_BUILDERS` / `ENCOUNTER_SETUP`    |
...
| **`Monster::miscInfo` 的其余用法**       | 本批把 `rolledDamage` 改名回 `miscInfo`（参考只有一个字段、含义逐怪种不同），但只用到虱子咬击伤害与红奴隶主 `usedEntangle` 两种。仍缺：刺穿之书的连刺计数、地精巫师的蓄力位、暗影的咬击伤害、六火幽魂的分割伤害、蠕动血块的位掩码          |
...
当前 181MB / 12556 例（第十六批 +375 例 = `exordium_wildlife` 一个编队 × 375）。
...
⚠ **例数是随数据变的**，换 variant 就得重量。下面标 ⁝ 的是在**当前布局（12556 例 =
12180 + 第十六批 `exordium_wildlife` 的 375）**上量的；标 ⁜ 的是第十五批那版布局（12180 例 =
...
⚠ **⁑ / ⁜ / ⁝ 这三批的分母与别的不同**：`ENC_V0` 编队各只有 375 例（只留 variant 0），
