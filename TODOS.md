...
现状是 **14 / 20 个编队、19 / 25 只怪**的 asc 分档有 trace 背书（asc19），
其余（三个精英 + 三个 Boss）在 `ascension > 0` 下**显式抛错**——
`EnemyDef.ascCalibrated` + `constructMonster` 的 throw + 编队级的
`ASC_SUPPORTED_ENCOUNTERS`。判据与「未登记的编队直接抛错」同源：静默拿 asc0 的血量区间
与招式数值去打 asc19 的仗，会产出「看着合理、其实不是原版」的数值。
...
- 测试：
  - `test/sts-combat-trace.test.ts` —— 逐帧对拍（16861 例），外加一条**数据自身的不变量**：
  ...
  - `test/data-tables.test.ts` —— 数据表不变量。**数据表是两代实现共用的**，它错了游戏级实现
    ...
    以及本批**故意没校准**的六只（三精英 + 三 Boss）不许带 `hpHigh`
...
当前 **247MB / 15181 例**（第二十批 +375 例 = `hexaghost` 一个编队；新文件 12MB，
与 `the_guardian` 同量级）。上一批是 236MB / 14806 例。
...
- **酸液史莱姆（大）的 asc17 出招读的是 _M 号_ 的腐蚀喷吐枚举**
  （`MonsterSpecific.cpp:2006`，在 `ACID_SLIME_L` 那条 case 里写着
  `lastTwoMoves(MMID::ACID_SLIME_M_CORROSIVE_SPIT)`）。第二十一批开爬升度这条轴时确认。
  L 号的 `moveHistory` 里永远不会出现 M 号的招式，所以那个条件**恒假**——
  它后面的 `randomBoolean(0.6F)` 与两个返回值整支是死代码，`roll < 40` 时恒出腐蚀喷吐。
  真实游戏的 `AcidSlime_L` 在那里读的是它自己的腐蚀喷吐，**看着是复制粘贴笔误**
  （同一块 asc17 代码的第二、三段用的都是 L 号自己的枚举，只有第一段是 M）。

  **裁定（第二十二批执行）：打补丁。** 两条判据都成立，与缠绕那条同一个结论：
  ① `large_slime@asc19` 就在已登记内容里，分歧实测存在（1 例）；② 补丁有预言机
  （改完重新生成即可重放验证）。修法是一个词，不是发明。

  **这条最能说明判据不是静态的。** 同一个笔误第十四批就发现了，那时**正确地没有打**——
  当时 trace 全是 asc0，整块 asc17 是死代码，判据 ② 不成立。开了爬升度这条轴之后
  ② 才成立。所以每条「已确认但未打补丁」的记录都要在覆盖面扩大时**重新过一遍判据**，
  不能记完就忘。

  执行要求：① 复活的 `randomBoolean(0.6F)` 在补丁注释里**显式标注未验证**
  （与缠绕补丁里那个阈值 50 同样处理）；② 只影响 `large_slime@asc19` 一个已冻结文件，
  走 `ALLOW_CHANGED="large_slime@asc19"`，**必须确认没有别的文件变**；
  ③ 背书只有 1 例，如实记进盲区，不要写成「已验证」。

  以下是打补丁之前的记录，保留以便追溯：两条判据都过得去
  （① `large_slime` 就在本批的 asc19 编队里，分歧真实存在；② 修法是**一个词**，
  不是发明），卡住的是 WORKFLOW 第 5 步那句「参考项目看着像 bug → 不要自己拍板」。
  ⚠ **影响面是可量的**：把它「修好」之后对拍红 **1 例**——分歧真实存在，但只有 1/120 条
  `large_slime@asc19` 走到，因为要求「连着两回合腐蚀喷吐 + 这一次 roll<40」。
  我们这边照抄的形状就是「拿 M 号的招式 id 去查 L 号的历史」（同样恒假），
  将来要打补丁只需把 `"corrosive_spit"` 改成 `"corrosive_spit_l"` 一个词。
...
这一节是**下一条战线的输入**（爬升度 / 第二幕 / 遗物 / 药水）。它回答三个问题：
哪些东西有背书、哪些盲区还是 0、这份数据现在有多大。

### 一、背书覆盖：20 个编队 / 25 只怪，全部有 trace 背书

harness 的 `encounters` 列表（`trace_dump.cpp`，从第一个 commit 起一个字没改过）就是
**第一幕的 20 个编队**。第十三~二十批把它们全部装完了，**没有一个例外**：

| 编队                                                            | 装入批次 | 保留策略   |
| --------------------------------------------------------------- | -------- | ---------- |
| `cultist` `jaw_worm` `jaw_worm_horde` `two_louse` `three_louse` | 一~十二  | `all`      |
| `small_slimes` `lots_of_slimes`                                 | 十三     | `variant0` |
| `large_slime`                                                   | 十四     | `variant0` |
| `blue_slaver` `red_slaver` `looter` `exordium_thugs`            | 十五     | `variant0` |
| `exordium_wildlife`                                             | 十六     | `variant0` |
| `gremlin_gang`                                                  | 十七     | `variant0` |
| `gremlin_nob` `lagavulin` `three_sentries`                      | 十八     | `variant0` |
| `the_guardian` `slime_boss`                                     | 十九     | `variant0` |
| `hexaghost`                                                     | 二十     | `variant0` |
...
#### （丙）~~**最大的一块结构性盲区不在上表里：爬升度**~~ —— 第二十一批做掉了大半

~~每只怪都有 asc2 / asc3 / asc4 的伤害档、asc7 的血量档、asc17~19 的出招与数值档，
全部是死代码——harness 固定 `ascension = 0`。~~

**第二十一批开了这条轴**（14 个普通编队 × asc19，做法见 WORKFLOW「爬升度这条轴」）。
现状与剩余：

| 状态                                       | 内容                                                      |
| ------------------------------------------ | --------------------------------------------------------- |
| ✅ 有背书                                  | 14 / 20 个编队、19 / 25 只怪的 asc 分档（两个方向都有）   |
| ⏳ 等第二十二批                            | 三精英 + 三 Boss（阈值 8 / 9，另有 asc18/19 分档）        |
| ❌ 仍是盲区：**阈值本身**                  | asc19 钉住的是分档的**两侧行为**，不是「分界线在 N」      |
| ❌ 仍是盲区：**三档里的中间那档**          | `{a,b,c}` 的 b 在 `{0,19}` 下永远走不到（实测 6 条 0 例） |
| ❌ 永远关不掉：**奴隶主的 asc17 内联条件** | 逻辑冗余（`lastTwoMoves ⊆ lastMove`），原作就这么写的     |

后三条的详情与关门条件见「验证方式 · 第二十一批」与「待裁定」。
⚠ **`EnemyDef` 只装得下一组血量区间**那条已经不成立了——本批加了 `hpHigh`，
并且未校准的怪现在**直接抛错**而不是静默用低档值。

### 三、数据体积与例数

| 指标                      | 第十九批  | 第二十批  | 第二十一批                   |
| ------------------------- | --------- | --------- | ---------------------------- |
| `test/golden/traces` 总量 | 236MB     | 247MB     | **268MB**                    |
| 对拍例数                  | 14806     | 15181     | **16861**                    |
| 编队文件数                | 19        | 20        | **34**（20 asc0 + 14 asc19） |
| `variant0` 冻结文件       | 14×375 行 | 15×375 行 | **15×375 + 14×120 行**       |

- 第二十一批新增的 14 个 `@asc19` 文件合计约 **21MB**（每个 120 行 = 40 种子 × 3 层），
  最大的是 `gremlin_gang@asc19` 2.3MB——**比 asc0 那批小一个量级**，因为种子只有 40 个。
  所以「再加一个中间档位」的边际成本是可接受的，见「待裁定」里那条的关门条件。
...
- `ENC_V0` 的 15 个文件合计约 **116MB**，每个 375 行。最大的三个是
  `slime_boss` 18.3MB、`hexaghost` 12.2MB、`the_guardian` 11.8MB——都是 Boss，血厚仗长。
- ⚠ **第十三~二十一批全部是纯追加**（唯一的例外是第十六批的 `usedEntangle` 补丁改写了
  `red_slaver` / `exordium_thugs` 两个文件）。所以这些文件在 git 里各只有一份 blob。

### 四、下一步的三个候选（按「解锁量 / 爆炸半径」排）

1. ~~**爬升度这条轴**~~ **第二十一批已开**。剩下的两半各自独立：
   ① **精英与 Boss 的 asc 分档**（第二十二批，**另开一个 variant**，别扩第二十一批那个的
   `encounters`）；② **成对档位**（asc7 优先），关掉「阈值本身」与「中间那一档」两条盲区。
