...
✅✅ **第四十七批把怪物这条线收官了：65 / 65。** 最后六只是第四幕的
`CORRUPT_HEART` / `SPIRE_SHIELD` / `SPIRE_SPEAR` 与 `MASKED_BANDITS_EVENT` 的
`BEAR` / `POINTY` / `ROMEO`，三个编队一批装完。收官小结见文末「怪物线收官小结」。
✅✅ **第四十八批把编队这条线也收官了：63 / 63。** 最后六个是 `TWO_FUNGI_BEASTS`
（第一幕 strong 池）与五个事件编队（`LAGAVULIN_EVENT` / `COLOSSEUM_EVENT_SLAVERS` /
`COLOSSEUM_EVENT_NOBS` / `MUSHROOMS_EVENT` / `MYSTERIOUS_SPHERE_EVENT`），一批装完
——上一批那句「大概率是一批装完的事」兑现了，而且**比预期更便宜**：六个全部由已登记的怪
组成，连 `ENCOUNTER_BUILDERS` 都没加一条（参考那六条 case 是裸的 `createMonster`，
走默认建怪路径）。收官小结见文末「编队线收官小结」。
⚠⚠ **它同样带来一个副作用**：`sts-combat-wiring.test.ts` 那条「未迁移编队」用例失去了
它历来的样本族（真实存在但没登记的编队一个都不剩），本批照怪物那条的办法改成断言
**参考 63 个编队与 `SUPPORTED_ENCOUNTERS` 双向相等**，样本换成旧近似表的残留
`two_orb_walkers`（参考压根没有这一项）。逐条见「验证方式 · 第四十八批 · 五」。
⚠⚠ **收官带来一个副作用，下一个人要知道**：`sts-combat-rules.test.ts` 那条
「未登记怪物 rollMove 会显式抛错」用例**失去了样本**——「在 `enemies.ts` 里却不在
`MOVE_RULES` 里」的怪一只都不存在了，而 WORKFLOW 禁止造哨兵怪。本批把那条用例改成断言
**让那道 throw 不可达的不变量本身**（`ALL_ENEMIES` 与新导出的 `SUPPORTED_MONSTER_IDS`
双向相等 + 恰好 65），比原来那条样本用例更强、而且不会再被顶掉。那道 throw 留着，
守的是「参考将来加了第 66 只怪」。

⚠ **上表全部是「爬升度 0」这一维的登记数。第二十一批起还有第二维：爬升度。**
✅ **第二十二批把它铺满了第一幕**（20 / 20 编队、25 / 25 只怪），
✅ **第三十批铺满了第二幕**（19 / 19 编队、17 / 17 只怪），
✅ **第四十六批铺满了第三幕**（15 + 颚虫军团 = 16 / 16 编队、17 / 17 只怪）
——**三幕 54 / 54 个编队、59 / 59 只怪，都是 asc0 与 asc19 两个档位**。
`EnemyDef.ascCalibrated` + `constructMonster` 的 throw + 编队级的 `ASC_SUPPORTED_ENCOUNTERS`
这道闸门**留着**——现在挡着的只剩**腐化之心**一只（第四幕，它那条 def 本来就只有血量、
没有招式）。第三十二~三十九批当时照抄却没有预言机的那些分档（尖刺客的荆棘 `{3,4,7}`、
暗球游荡者的 `GENERIC_STRENGTH_UP` asc17 档、尖塔增生缠绕的 asc17 档、大嘴咆哮/流涎的
asc17 档……）第四十六批全部拿到了例数，逐条见「验证方式 · 第四十六批」。
...
trace 会逐字节重复），所以覆盖面是 **30 / 54 个编队**（第一幕 11 + 第二幕 12 +
✅ **第四十七批的第三幕 7 个**）。做法与两步验证见 WORKFLOW「目标策略这条轴」。
⚠ 三维目前是**部分笛卡尔积**：`{asc0, asc19} × 54 编队` 全满（第四十六批），`tgt1`
基本只与 asc0 配过。✅ **第四十七批开了第一个双后缀文件** `donu_and_deca@asc19@tgt1`
（迪卡守护方阵的**镀甲**那一半只在 asc>=19 存在，它的 `noAliveGate` 要两条轴同时才看得见，
实测红 **7 例**）。缺的那一格：`asc19 × tgt1` 的其余 22 个组合仍是
「百夫长狂怒连斩 asc2 伤害档」唯一的关门条件。
⚠ ✅ **第四十七批甲关掉的两条**：迪卡守护方阵的**格挡** `noAliveGate`（53 例，
其中 `donu_and_deca@tgt1` 46 + `@asc19@tgt1` 7；asc0 / asc19 那两份原文件仍是 0，
正好印证第三十九 / 四十六批那条裁定）与**镀甲** `noAliveGate`（7 例）。
⚠⚠ **没关掉的那条要写清楚**：`MINION_LEADER` 判胜路径在**觉醒者**身上仍然 **0 例**
——`awakened_one@tgt1` 里玩家一次都没能在邪教徒还活着时打死二阶段。蜥蜴法师那一半
其实早就被第四十批的 `reptomancer@relic4`（17 例）关掉了，旧盲区表把两者写成一条是错的。
新的关门条件是「一副能打穿觉醒者二阶段的牌组 × `@tgt1`」，不是目标策略本身。
...
⚠ 这一节是**交接**，不是方向。方向仍看下面的「待办」，做法看 [WORKFLOW.md](WORKFLOW.md)。
它记的是「上一个人做到哪、下一步的第一刀该切在哪、以及**这次学到的、写在别处会被淹掉的判据**」。

### 一、按「单位成本 / 解锁量」排的下一步

⚠⚠ **上一版这张表把「中毒 + 扭曲漏斗」排在第 1 位，第五十八批把它撤销了——理由记在这里，
免得下一个人重新推一遍。** `Actions::PoisonLoseHpAction()` 的函数体是 **`return sts::Action();`**
（`Actions.cpp:936-938`）——一个 `actFunc` 为空的 `std::function`。`Monster::applyPreTurnLogic:36`
在「这只怪带中毒」时把它 `addToBot`，而 `executeActions` 的 `a(*this)`（`BattleContext.cpp:767`）
会当场抛 `std::bad_function_call`，**harness 整个终止**。所以**中毒属于「参考压根没实现完」
那一族**，与暗影精华 / 活体样本同类：**永远不会有预言机，扭曲漏斗永久排除**。
⚠ 顺带查清：全参考给怪物上中毒的只有四处（毒液 `Monster.cpp:345`、`Player.cpp:711`、
扭曲漏斗 `:424`、毒药水 `:2409`），**没有任何怪物自己带中毒**——所以这个洞在当前语料里
不可达，不是「开着的缺口」，是「这条路本来就不通」。

| 顺位 | 做什么                               | 为什么排这里                                                                                                                                                                                    |
| ---- | ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | **`sts-neow` 的效果**（一.6 第二片） | 待办里最大的一块，但**第一片已经就位**（真实选项已进 `GameState.neowOptions`），换代那一步是「逐个转写 19 × 6 的效果 + 把事件屏切过去」。⚠ 老档已按近似效果结算过，回填只能给出「本该是哪四个」 |
| 2    | **六条 run 级 RNG 流**（一.7）       | 奖励 / 遗物池 / 事件 / 商店 / 宝箱，**全新写而非转写**。做完才能删掉玩具 `rng.ts`——那是「近似实现全部删除」这条总纲最后一块没兑现的                                                             |
| 3    | **`asc7 + asc16`**（跨三幕）         | 纯数据、实现侧一行不改，但 ≈ 200MB。它关掉「三档里的中间那一档」13 条 + 冠军两族阈值 + 血量阈值 7/8/9                                                                                           |
| 4    | 剩下 23 颗遗物                       | 逐条的阻塞原因见一.9 的分类表：充能球 6 / 姿态 2 / 造参考没实现的牌 2 / 选牌屏 5 / 状态牌可打性 2 / 参考自己注释掉的 1（梅兰奇，可像情绪芯片那样登记成空操作）                                  |

### 二、这次学到的、值得先读一遍的四条

1. ⚠⚠ **排除理由会过期，要定期回扫那张表。** 第四十九批退役了「读 `r.data` ⇒ 没有可观察面」
   （第四十四批就把前提做没了）、第五十批退役了「读 `curRoom` ⇒ 结构性不可达」（关门条件是
   `DeckVariant` 上一个 `Room` 字段）。**两批各多开了几颗遗物，而实现侧几乎没写新逻辑。**
2. ⚠⚠ **「0 例」先问是不是变体配置的问题，再记成盲区。** 第五十批六条探针里有两条 0 例，
   根都在配置（投索只发给了析取的一边 / 缩放仪那个 variant 满血进场、`heal` 被上限夹掉），
   改配置重生成之后各红 120。**判据：门有几个析取项就要有几个 variant 各点亮一项；
   数值被上限夹住时，先把上限让开再谈「数值有没有背书」。**
3. ⚠ **「样本用光了 → 改成断言不变量」这个套路本会话用了三次**（第四十七批怪物、
   第四十八批编队、第五十二批爬升度），三次形状完全一致。下一次遇到「这条用例的样本
   被自己铺量顶掉了」，直接照抄，不要去造哨兵样本。
4. ⚠⚠ **接线拆成「翻译层 + 换代」两批，这次两次都成立**（地图五十五 / 五十六、涅奥五十七 /
   下一批）。翻译层能单独验证，换代那一步同时动路由、测试与 `migrate`——混在一起出问题
   没法定位。**`sts-neow` 的第二片就按这个切。**

### 三、⚠ 一条别再撞的墙

**孢子云 `addToTop ↔ addToBot` 不要再拿编队去撞了**（第二十五 / 三十一 / 四十八批三次量、
三次 0 例）。它卡的是「那一刻队列里有没有别的动作」，与场上有几只带孢子云的怪无关。
唯一没试过的方向是「亡语 + 多条排队动作」（例如带死亡触发另有入队的遗物）。
...
5. ~~接 `sts-map`：决定楼层与房间类型~~ ✅ **第五十五 / 五十六批完成**（前一半是翻译层，后一半本批）。
   ⚠ **玩具地图（`map/map.ts` 的 `generateMap` + `ENABLED_MAP_TYPES`）本批删掉**，`availableNext` 留着
   （它只读 `MapGraph`，与谁生成的无关）。⚠ 换代**买到**的东西：真实地图一上来就有精英 / 事件 /
   商店 / 篝火 / 宝箱，而 run 层的路由早就为这五种写好了分支——此前一条都走不到。
   ⚠ 每幕 `Random(seed + offset)` 自成一体，所以换幕时**重算**是对的（与遭遇序列那条**持久流**
   正相反，别照抄那边的写法）。原始记录：**第五十五批交付了前一半**：翻译层
...
6. 接 `sts-neow`：19 种 bonus × 6 种 drawback，其中效果要逐个实现（不是纯映射，工作量最大）。
   **第五十七批交付了第一片**：真实的四个选项已经算出来存进 `GameState.neowOptions`
   （`newRun` 一次、`migrate` 从 seed 重算回填），4 条用例钉住「与 `generateNeowOptions`
   一致」「前两个无代价 / 后两个必有代价 / 都不是哨兵」「确定性」「回填」。
   ⚠⚠ **事件屏仍在用近似的四个选项**——换过去要先把 19 × 6 的**效果**转写完
   （选牌屏、随机稀有牌、遗物池、诅咒、`10%` 掉血……），那是这条线的主体。
   套路与地图那条（第五十五 / 五十六批）相同：先做能单独验证的一半。
   ⚠ 换过去那一批还要处理一件**这一片没有**的麻烦：老档已经选过并结算了**近似**效果，
   与真实效果不一致，而那是不可重放的历史——回填只能给出「本该是哪四个」。
...
9. ⚠ **`stsCombatCoverage` 不查遗物**（第四十一批发现）——**第五十四批做成了可选的严格模式**
   （`GameState.strictRelicCoverage`，默认关）。开着时带一颗未转写的遗物当场开不了战、
   理由点名它；关着时保持现状。⚠ **默认为什么必须是关**：未登记的 84 颗里有 **65 颗在参考的
   `src/combat` 里一个读点都没有**（run 层遗物，永远不会进 `SUPPORTED_RELIC_IDS`），
   其中就有铁甲的初始遗物**燃烧之血**——一律拦下不是「更严格」，是把引擎关掉。
   ⚠⚠ **真正的修法是把两族分开建模**（「有钩子却没转写」vs「战斗内本来就没有钩子」），
   那时严格模式才能成为默认。这条账仍然开着，只是从**沉默的缺口**变成了**可开关的缺口**。
   原始记录：
   它逐条查编队 / 牌 / 药水，
...
- 测试：
  - `test/sts-combat-trace.test.ts` —— 逐帧对拍（**41266 例**，第四十六批），外加一条**数据自身的不变量**：
...
### 第五十二批：第四幕 + 蒙面强盗的爬升度（六只怪的 `hpHigh` / `ascCalibrated` 补齐）

第四十七批乙留的账：「第四幕的爬升度是它自己的一批」。本批还了——三个编队 × asc19，
六只怪的第二组血量区间从注释搬进 `hpHigh`，`ascCalibrated` 与编队级闸门同批开。
例数 +360（**44025 → 44385**），文件 220 → **223**。

#### 一、新预言机当场抓到一处真实偏差（这一批最重要的产出）

第一次生成 `shield_and_spear@asc19` 之后对拍 **120 条全红**，断点在第一回合末的牌堆快照。
根因是**第四十七批就记下、但当时无法裁定**的那道分岔：尖塔长矛的灼烧打击在 asc18 走
`MakeTempCardInDrawPile(BURN, 2, false)`，而那个 action 的 `shuffleInto` 为 `false` 时
**一张都不塞**。我们这边照旧塞两张进弃牌堆 ⇒ 高档整条对不上。

**裁定：照抄 as-built（asc>=18 什么都不塞）**，三条判据只过第 ①（分歧真实存在，
真实游戏 asc18 会把灼伤塞进抽牌堆），而**补上它没有预言机**——预言机就是参考本身。

⚠ 它需要一个**新的分档方向**：`add_card` 上的 `belowAscension`（`ascension < N` 才结算）。
此前两个方向是「换个数值」（`ascAmount`）与「多一条语句」（`minAscension`），
这一条是**整条效果在高档消失**，三者互不可替代。

#### 二、⚠⚠ 熊的血量区间是反直觉的，而且它推翻了一条测试断言

`MonsterIds.h:156` 写的是 `{{38,52},{40,44}}`——高档把区间**收窄**了（下限 38 → 40，
上限反而 52 → 44），不是整体抬高。`data-tables.test.ts` 那条「第二组恒不低于第一组」的
断言因此**在上限那一半是错的**，本批按名放行了熊、并把理由写在断言旁边。

> **判据：「爬升度只会让怪更硬」在招式数值上大体成立，在血量区间上不成立。**
> 全 65 只里只有熊一只这样，所以是点名放行而不是删断言——下一只同族的怪必须回来读那段注释。

#### 三、三档阈值并存

尖塔护盾 / 长矛是**精英族** `asc >= 8`、腐化之心是 **Boss 族** `asc >= 9`、
三只强盗是**普通怪族** `asc >= 7`。asc19 一次点亮三族的高侧。
⚠ 别按「第四幕都用 9」推——那正是这一批最容易抄错的地方。

#### 四、`data-tables.test.ts` 又一条用例失去了样本

「没标 `ascCalibrated` 的敌人不许带 `hpHigh`」这条用例的样本族（本批之前是这六只）
**清零了**——`enemies.ts` 里一只没校准的怪都不剩。照第四十七 / 四十八批的办法改成断言
**不变量本身**：`ASC_CALIBRATED` 与 `ALL_ENEMIES` 双向相等且恰好 65，且每一只都带
第二组区间（`hpNoRoll` 的三只除外）。**这是这个套路第三次用**，三次的形状完全一致。

#### 五、变异测试（4 条探针，44385 例基线，全部非 0）

| 改坏的地方                                  |    例数 | 落点                         |
| ------------------------------------------- | ------: | ---------------------------- |
| 尖塔护盾高档血量 125 → 110                  | **120** | `shield_and_spear@asc19`     |
| 腐化之心高档血量 800 → 750                  | **120** | `the_heart@asc19`            |
| 熊的高档区间写成低档 `{38,52}`              | **120** | `masked_bandits_event@asc19` |
| 去掉灼烧打击的 `belowAscension`（高档照塞） | **120** | `shield_and_spear@asc19`     |

---

### 第五十批：**房间类型**这条轴——一次开五颗遗物（91 → 96 / 180）

第四十三批把缩放仪 / 昆虫标本 / 奴隶主颈圈 / 勇气投索 / 古董茶具排除时写下的关门条件是
「给 `DeckVariant` 加一个 `Room` 字段 + trace 头部加一个 `room`」。**本批把它建了，五颗一起开。**
这是第二次「退役一条过期的排除理由」（上一批是娃娃 / 巨铃的 `r.data`）——两批加起来说明
**那张排除表要定期回扫**，它记的是当时的前提，不是永久判决。

例数 43545 → **44025**（+480）；文件 216 → **220**。两步验证做了：先只加字段与头部输出、
不加任何 variant，`--check` **216 个文件逐字节复现**；过了才填 variant。

#### 一、三个 variant，拆法是承重的

|            | 房间                          | 遗物                               | 编队                             |
| ---------- | ----------------------------- | ---------------------------------- | -------------------------------- |
| `@relic20` | `room = ELITE`                | 昆虫标本 + 勇气投索 + 奴隶主颈圈   | `three_sentries` / `gremlin_nob` |
| `@relic21` | `room = BOSS`（50/80 血进场） | 缩放仪 + 奴隶主颈圈 + **勇气投索** | `champ`                          |
| `@relic22` | `lastRoom = REST`             | 古董茶具                           | `champ`                          |

⚠⚠ **颈圈的门是析取（`ELITE || BOSS`），另外两颗各认一边**，所以一个 variant 只能点亮
析取的一半。`@relic20` / `@relic21` 各一边，颈圈**两边都带**——于是它的两条臂各有背书，
两个邻居又证明了自己在另一个房间**没有**触发。
⚠ 茶具读的是 **`gc.lastRoom`**，与另外四颗读的 `room` 是**两个不同的字段**。`@relic22`
因此**只设 `lastRoom`、`room` 留 `INVALID`**：抄成读 `room` 的话这份文件一点能量都不会加。

#### 二、⚠⚠ 两条「0 例」当场被修掉了，而不是记成盲区——过程值得抄

第一轮跑完六条探针有**两条 0 例**，两条的根都是**变体配置**而不是实现：

1. **「把投索的门写成 `ELITE || BOSS`」红 0 例**——因为投索只在 `@relic20`（精英）里，
   全语料没有一条「Boss 房 + 带投索」的 trace 能与它分歧。**修法：把投索也发给 `@relic21`。**
2. **「缩放仪 `heal(25)` 改成 24」红 0 例**——`@relic21` 当时是 **60/80** 进场，而 `heal`
   会被上限夹掉：60+25 与 60+24 都落在 80。**修法：改成 50/80**，25 点整个装得下。

改完重生成，两条各红 **120**。

> **判据（新增一条）：一颗遗物的门有几个析取项，就要有几个 variant 各点亮一项；
> 一颗遗物的数值被上限夹住时，先把上限让开再谈「数值有没有背书」。**
> ⚠ 顺带撞到一个**管线行为**，下一个人会遇到：`--install` 把**已经装进目录的文件**一律当成
> 冻结的，所以「同一批里重生成自己刚装的文件」会被拒绝（`✗ 已冻结的数据被扰动了`）。
> 正确做法是**先删掉本批那几个未提交的 `.jsonl` 再 `--install`**，而不是去动 `ALLOW_CHANGED`。

#### 三、变异测试（6 条探针，44025 例基线，全部非 0）

| 改坏的地方                        |    例数 | 落点                                             |
| --------------------------------- | ------: | ------------------------------------------------ |
| 颈圈的门去掉 `BOSS`               | **120** | `champ@relic21`                                  |
| 颈圈的门去掉 `ELITE`              | **240** | `three_sentries@relic20` + `gremlin_nob@relic20` |
| 投索的门加上 `BOSS`               | **120** | `champ@relic21`（修配置之后才有）                |
| 茶具改读 `room` 而不是 `lastRoom` | **120** | `champ@relic22`                                  |
| 昆虫标本的取整 `trunc` → `round`  | **194** | 116 + 78（三哨卫 / 地精头目）                    |
| 缩放仪 `heal(25)` → 24            | **120** | `champ@relic21`（修配置之后才有）                |

⚠ 昆虫标本那条 194 例正是「C++ 的 `static_cast<int>(maxHp * .75f)` 是**向零截断**」的背书
——三哨卫 39~~45 血、地精头目 82~~86 血，两者都有大量 `.75` 落不整的取值。
⚠ 它的循环是**裸的 `i < monsterCount`**（贤者之石那一族，没有过滤器）。本批**没有**给它
预留空位的编队，理由是查过的：空格 `maxHp = 0` ⇒ `0.75 × 0 = 0`，写不写都一样，
**那个缺失的过滤器在这颗遗物身上本来就不可观察**。

---

### 第四十九批：遗物战线第六批——杜乌娃娃 / 巨铃 / 达摩鲁 / 情绪芯片（87 → 91 / 180）

**纯追加，没有补丁。** 开工前的 `--check` 里 214 个已提交文件逐字节复现；`--install` 之后
`git diff --stat -- test/golden/traces` **为空**（零个 `M`），只有 2 个 `??`。
例数 43305 → **43545**（+240）；文件 214 → **216**。

#### 一、这一批**退役了第四十三批写下的一条排除理由**

第四十三批筛 70 颗「单读点」遗物时，把杜乌娃娃与巨铃排除了，理由写的是
「读 `r.data`，而引擎的 `bc.relics` 不带 data、harness 发出去的也恒是 0 ⇒ 这一格是空操作、
没有可观察面」。**第四十四批把这条理由的两半都做掉了**（`RelicSpec` 有了 `data` 字段、
`BattleContext.relics` 变成 `{id, data}`），只是当时没人回头去改那张排除表。

> **教训（值得当成套路）：排除理由要写清楚它依赖什么前提，因为前提是会过期的。**
> 本批回头重扫了那张表，退役的就这一条；其余 20 条（充能球 / 姿态 / `curRoom` / 选牌屏 /
> 全池造牌）的前提都还成立。
...
| 遗物                    | 读点                                                                 | 形状                                     |
| ----------------------- | -------------------------------------------------------------------- | ---------------------------------------- |
| 杜乌娃娃 `DU_VU_DOLL`   | `:268-270`                                                           | 同步 `buff<STRENGTH>(r.data)`            |
| 巨铃 `GIRYA`            | `:280-282`                                                           | **与上一行逐字相同**，只差 case 标签     |
| 达摩鲁 `DAMARU`         | `:260-262`（init，**同步**）+ `Player.cpp:513`（回合开始，**入队**） | 硫磺那一族：**两个时点，且两处形状不同** |
| 情绪芯片 `EMOTION_CHIP` | `Player.cpp:518-520`                                                 | **一个空的 `if`**（`// todo`）           |

⚠⚠ **娃娃与巨铃必须分到两个 variant**：两者的函数体逐字相同，塞进同一个 variant 只能看到
**两者之和**，「哪颗给了多少」就成了第四十三批 `@relic13`（苦无 / 手里剑）那种盲区。
本批 `@relic18` 只带娃娃（`data = 3`）、`@relic19` 只带巨铃（`data = 2`），**同一副牌组、
同一个编队、同一份药水**，于是两份文件的差别只有开局力量读 3 还是读 2。
⚠ 两颗的 `data` **语义完全不同**（娃娃是牌组里诅咒的张数、巨铃是已举过几次），但
**战斗内看不出这个区别**——这一格只看数值。语义差别属于 run 层。

⚠ 编队选 `CHAMP` 是为了**仗长**（实测平均 9.18 / 9.32 个玩家回合）：达摩鲁的第二个时点
每个玩家回合加一层，所以快照里的专注层数（最高量到 **15**）是「有没有把两处都抄上」的直读。

⚠ 情绪芯片的背书是**反方向**的：参考什么都不做，所以任何我们发明的效果都会让 120 条
全红（实测把它写成「入队 +1 能量」红 120 例）。**它属于「好奇心」那一族**（参考给出了答案，
答案是「不做」），不属于「活体样本」那一族（参考排了一个没人应答的 InputState、直接卡死）。
⚠ 平静姿态那一半参考也没实现（`// todo handle mantra change stance`），所以专注会一路涨到
15 都不触发姿态切换。**照抄，不要补**——补它没有预言机。

#### 三、变异测试（6 条探针，43545 例基线）

| 改坏的地方                                  |    例数 | 落点            |
| ------------------------------------------- | ------: | --------------- |
| 娃娃读常数 1 而不是 `r.data`                | **120** | `champ@relic18` |
| 巨铃读常数 1 而不是 `r.data`                | **120** | `champ@relic19` |
| 达摩鲁**只抄** `initRelics`（丢掉回合开始） | **120** | `champ@relic18` |
| 达摩鲁**只抄**回合开始（丢掉 `initRelics`） | **120** | `champ@relic18` |
| 情绪芯片写成「入队 +1 能量」                | **120** | `champ@relic18` |
| 达摩鲁回合开始那处改成**同步**（不入队）    |   **0** | ⚠ 盲区，见下    |

⚠ **那条 0 例是结构性的**：回合开始那一刻队列里没有别的动作，`addToBot` 与同步落在同一个
位置，而下一帧快照要等到玩家出第一张牌、队列早已排空。关门条件是「在回合开始钩子与队列
排空之间读专注层数的东西」——参考里没有这种东西（读专注的只有被注释掉的姿态切换）。
**这一条与孢子云那条同族**：不是薄，是不可观察。

#### 四、⚠⚠ 顺带量清楚了一件事：「遗物还剩 89 颗」这个数字**严重高估了剩余工作量**

⚠⚠ **第五十八批复核了这张表的检索式，两处都漏了，教训单记**：
① 只 grep `R::<名字>` 会漏——参考里 `R::X` 与 `RelicId::X` **两种写法并存**；
② 只扫 `src/combat` 会漏——姜 / 萝卜那一族的判定写在 **`include/combat` 的头文件**里。
补上之后：**无钩子 61 颗**（不是 65，其中已登记 0）、**有钩子但未登记 32 颗**（不是 24）。
复核命令记在这里，下次直接跑、别再手推：

```bash
grep -rnE "(R|RelicId)::<大写 id>\b" $REF/src/combat $REF/include/combat
```

⚠ **这条更正与同一轮撤销「中毒 + 扭曲漏斗」同源**：两次都是「按不完整的检索下的结论」。
**凡是要写进计划表的分类数，先把检索式本身核一遍。**

本批扫了全部 89 颗未登记遗物在 `src/combat` 里的读点：

| 分类                                    | 颗数   | 归属                                                                                              |
| --------------------------------------- | ------ | ------------------------------------------------------------------------------------------------- |
| **在参考的战斗代码里一个读点都没有**    | **61** | 它们是**纯 run 层**遗物（回血 / 金币 / 商店 / 篝火 / 地图），属于「一、接线」那条线，不是战斗铺量 |
| 有读点、但未登记（缺失机制 / 还没轮到） | **32** | 见下。⚠ 第五十八批复核后的数                                                                      |

那 24 颗的阻塞原因（逐条查过读点，不是推的）：

- **充能球**（6）：破裂核心 / 核电池 / 共生病毒 / 冰核 / 如尼电容 / 嵌入器（涨球位）
- **姿态 / 观者专属**（2）：泪滴坠饰（平静姿态）、圣水（造神迹——参考没实现那张牌）
- **造参考没实现的牌**（2）：纯净之水（神迹）、忍者卷轴（飞刀）
- **读 `gc.curRoom` / `lastRoom`**（5）：缩放仪 / 奴隶主颈圈 / 勇气投索 / 昆虫标本 / 古董茶具
  ——⚠ **关门条件是现成的**：给 `DeckVariant` 加一个 `Room` 字段（与 `ascension` 同一个
  「默认值保持字节不变」的套路）+ trace 头部加一个 `room`，**一次开五颗**
- **选牌屏 / 全池造牌**（5）：工具箱 / 枯枝 / 赌博芯片 / 死藤（`nilrys_codex`）/ 秘典
- **让状态牌 / 诅咒牌变得可打**（2）：医疗包 / 蓝烛——⚠ 它俩改的是**牌的可打性**，
...
> **结论：战斗内遗物真正剩下的是「24 颗，其中 6 条前置机制」，不是「89 颗」。**
> 而 65 颗纯 run 层遗物说明**下一条主战线应该是接线**（TODOS 一.4~9），不是继续铺遗物。

---

### 第四十八批：最后六个编队（**编队线 63 / 63 收官**）

**本批是纯追加，没有补丁，实现侧一行行为代码都没改。** 开工前跑过 `--check`，已提交的
**208 个文件全部逐字节复现**；`--install` 之后 `git status` 只有 **6 个 `??`**，
`git diff --stat -- test/golden/traces` **为空**（零个 `M`），**不需要 `ALLOW_CHANGED`**。
参考侧两个 commit（骨架 / variant），gameplay 代码一行没动。
总例数 42585 → **43305**（+720）；体积 982MB → **994MB**（+12MB）；文件 208 → **214**。

装的六个：`TWO_FUNGI_BEASTS`（第一幕 strong 池）+ `LAGAVULIN_EVENT` /
`COLOSSEUM_EVENT_SLAVERS` / `COLOSSEUM_EVENT_NOBS` / `MUSHROOMS_EVENT` /
`MYSTERIOUS_SPHERE_EVENT`。**「编队」那一栏 63 / 63，参考的 `MonsterEncounter` 全部登记。**

#### 一、为什么这一批的「开发」几乎是空的

⚠⚠ **六个编队一只新怪都没有**，而且**一条 `ENCOUNTER_BUILDERS` 都不用加**——参考那六条
case 全是两三句裸 `createMonster`（`MonsterGroup.cpp:203-211 / :298-300 / :316-325 / :449-452`），
没有候选池、没有预留空位、不掷 `miscRng`，正好落在 `initCombat` 的**默认建怪路径**
（照 `ENCOUNTERS[id].enemies` 逐只建）。所以实现侧的全部改动是：

1. `enemies.ts` 加 3 条 / 改名 2 条（`colosseum` → `colosseum_event_nobs`、
   `mysterious_sphere` → `mysterious_sphere_event`，成员一个字没动，`events.ts` 的引用跟着改）；
2. `sts-combat.ts` 的 `SUPPORTED_ENCOUNTERS` 加 6 条；
3. `trace-replay.ts` 的 `ENCOUNTER` 映射加 6 条（`MONSTER` 表一个字没加）。

⚠ **唯一一处「看着要写代码、其实不用写」的是事件版拉加维林**：它与 `lagavulin` 的差别是
那句 `setHasStatus<MS::ASLEEP>(true)`，而那一句住在 `ENCOUNTER_SETUP.lagavulin`——按**编队
id** 索引，于是 `lagavulin_event` 天然取不到它。第十八批那条注释（「形状照抄，将来装它时
不必再改」）本批兑现，**真的一行没改**。这是「把特例写在编队上而不是怪物上」那个选择的回报。

#### 二、牌组是量出来的（第十一次「先量再定」）

标准那副 22 张（`BATCH_1 + SPOT_WEAKNESS`、未升级、40 种子、asc 0、tgt 0）对上第三十七批
那副 45 张全升级，各 120 条 trace：

| 编队                      | 平均回合数 A / B | 最薄招式执行 A / B | 觅敌之弱打出 A / B |
| ------------------------- | ---------------- | ------------------ | ------------------ |
...
**选 A**：它在六个里的五个更长（弱牌组买怪物回合，与第四十七批腐化之心那副「只防不攻」
同源），每只怪的每条招式在 A 下都执行 62~~498 次，B 一栏都没赢——它唯一领先的
`ORB_WALKER`（本批仅有的第三幕怪）领先幅度是 403 vs 415 与 505 vs 498，**等于没有**。
代价那一栏（B 的觅敌之弱多一倍）不值那两倍体积：A 的 42~~89 次已经离 0 很远。

⚠ **那一张觅敌之弱在这里比平时值钱**：本批的第一幕怪（真菌兽 / 拉加维林 / 地精头目）是
第十六~十八批登记的，它们的编队跑在第一幕乘积的 variant 0 上、那副牌组**没有**觅敌之弱。
所以这是**地精头目的攻击分类第一次有预言机**（下表 41 例，全部落在本批文件里）。

#### 三、变异测试（12 条探针，43305 例基线）

⚠ 跑批前跑了一次不带任何变异的对拍：**0 失败**（43596 个用例）。

| 改坏的地方                                             |    例数 | 落在哪里                                       |
| ------------------------------------------------------ | ------: | ---------------------------------------------- |
| `colosseum_event_nobs` 成员顺序对调（头目 0 / 工头 1） | **120** | 本批文件（整份）                               |
| `colosseum_event_slavers` 成员顺序对调                 | **120** | 本批文件（整份）                               |
| `mushrooms_event` 建 2 只而不是 3 只                   | **120** | 本批文件（整份）                               |
| `mysterious_sphere_event` 建 1 只而不是 2 只           | **120** | 本批文件（整份）                               |
| `two_fungi_beasts` 建 3 只而不是 2 只                  | **120** | 本批文件（整份）                               |
| **事件版拉加维林也置沉睡位**                           | **120** | 本批文件（整份）                               |
| **激怒改成「谁带激怒谁涨力量」**                       | **116** | 全部在 `colosseum_event_nobs`                  |
| 白名单漏掉地精头目两条攻击招                           |  **41** | 全部在 `colosseum_event_nobs`（**此前 0 例**） |
| 白名单漏掉真菌兽的巨口                                 |      89 | 本批 55（两个真菌兽编队）+ 旧文件 34           |
| 白名单漏掉拉加维林的攻击                               |     102 | 本批 50 + `lagavulin@relic6` 52                |
| 白名单**多收**拉加维林的吸魂（非攻击）                 |      38 | 本批 24 + `lagavulin@relic6` 14                |
| **孢子云 `addToTop` → `addToBot`**                     |   **0** | ⚠ **没关掉**，见下                             |

#### 四、关掉的三条老盲区，与**没**关掉的那一条

✅ **① 「金属化以睡着为前提」**（第十八批留的账）——`lagavulin_event` 是全参考唯一不置沉睡位
的编队，`preBattleAction` 那道 `if (hasStatus<ASLEEP>())` 第一次有了假分支。红 120 例（整份）。

✅ **② 「激怒的宿主不在 0 号位」**（第十八批留的账）——参考的激怒钩子是
`auto &m = monsters.arr[0]; if (m.hasStatus<MS::ENRAGE>())`（`BattleContext.cpp:1846-1849`），
**写死 0 号位**。`gremlin_nob` 是单怪编队、`slavers` 里没有头目，所以在此之前
「读 arr[0]」与「读带激怒的那只」永远同解。`colosseum_event_nobs` 是工头 0 / 头目 1，
于是参考的行为是**激怒整场一次都不触发**——红 116 例。
⚠ **要说清楚这一条的方向**：本批钉住的是「参考确实只看 0 号位」，不是「头目会涨力量」。
把它「修」成遍历全体就是发明，不是转写。

✅ **③ 「地精头目的攻击分类」**——上表 41 例，此前 0（它的编队跑在没有觅敌之弱的 variant 0 上）。

❌ **没关掉：孢子云的 `addToTop` ↔ `addToBot`**——本批是第三次量，**又是 0 例**。
⚠ **本批没有推翻第三十一批那条裁定，是第三次印证它**：那一批已经把根因证死了
（真菌兽「有同伴时死」从 83 / 375 涨到 120 / 120，插队顺序照旧不可观察 ⇒ 卡的是
**队列内容**而不是**成员结构**）。本批把「多只**带孢子云**的怪同场」这个此前根本不存在的
局面也造出来了（`two_fungi_beasts` 2 只 + `mushrooms_event` 3 只 = 240 条 trace），
仍然 0 例——**这条轴到此为止彻底走完了，下一个人不必再拿编队去撞它**。
关门条件仍是第三十一批写的那句：需要「真菌兽死掉的那一刻队列里还有别的动作」，
即一张会在结算中途入队别的动作的牌 / 遗物，与场上有几只真菌兽无关。

#### 五、`sts-combat-wiring.test.ts` 的「未迁移编队」用例换写法了

与第四十七批怪物那条遇到的是同一件事：**样本没了**。历来的做法是挑一个「真实存在但最晚
才会被登记」的编队（`gremlin_nob` → `giant_head` → `donu_deca` → `the_heart` →
`mysterious_sphere`），本批把最后六个一次装完之后，参考的 63 个编队**全部**在
`SUPPORTED_ENCOUNTERS` 里，这一族样本永久告罄。

本批照第四十七批的办法改成**断言不变量本身**：把参考 `MonsterEncounters.h` 的
`monsterEncounterEnumNames[]`（去掉 `INVALID` 哨兵）逐条抄成测试里的
`REFERENCE_ENCOUNTERS`，断言它与 `SUPPORTED_ENCOUNTERS` **双向相等且恰好 63**。
它是**独立来源**（不是从被测常量派生的），两个方向都守：谁删掉一条登记会红，
参考将来加了第 64 个编队也会红。

⚠ **那道 throw 留着，而且它现在守的东西变了**：不再是「还没铺到的编队」，而是
**旧近似表的残留**——`enemies.ts` 的 `ENCOUNTERS` 里还有 **13 条参考没有对应枚举**的条目
（`two_orb_walkers` / `exploder` / `spiker` / `repulsor` / `two_exploders` /
`small_slimes_a|b` / `large_slime_acid|spike` / `centurion` / `two_centurions` / …），
而且当前的 run 层**真的会掏到它们**（第一 / 三幕的权重表里有）。用例的样本因此换成
`two_orb_walkers`：真实可达、却永远不会有预言机（参考压根没有这一项）。
⚠ 这 13 条应当在接 `sts-encounters`（一.4）那一批整体删掉——**那时**这条用例才会再次
需要换写法。

---

### 第四十七批：第三幕的 `@tgt1` + 第四幕与事件编队（**怪物线 65 / 65 收官**）

**本批也是纯追加，也没有补丁。** 开工前跑过 `--check`，已提交的 **197 个文件全部逐字节
复现**；两次 `--install` 之后 `git status` 只有 **11 个 `??`**，`git diff --stat --
test/golden/traces` **两次都为空**（零个 `M`），**不需要 `ALLOW_CHANGED`**。
参考侧三个 commit（骨架 / 甲的 variant / 乙的 variant），gameplay 代码一行没动。
总例数 41265 → **42585**（+1320）；体积 938MB → **982MB**（+44MB）；文件 197 → **208**。

#### 一、两步验证的实测（这是这条套路第八次做，一次都没白做）

甲、乙**各要一个新乘积**（第八个 `act3TgtVariants`、第九个 `act4Variants`），所以先只加
两个乘积的骨架与硬检查、两个 variant 列表**都留空**，跑 `tools/regen-traces.sh --check`：

```
拆出 197 个编队: CULTIST=2775 JAW_WORM=1335 … DONU_AND_DECA@asc19=120
→ 校验：已提交数据是否被扰动
  ✓ cultist.jsonl 全部 2775 行 —— 一致
  …（197 行，全部「—— 一致」，没有一行是别的）…
  ✓ donu_and_deca@asc19.jsonl 全部 120 行（冻结） —— 一致

✓ 管道能逐字节复现已提交数据
```

（`grep -c "—— 一致"` = **197**，`grep -v` 之后除了四行进度提示与末尾那句结论一行都不剩，
退出码 0。）过了这一步才填 variant。

⚠ 两个新乘积都照抄第四十五 / 四十六批那条**硬检查**（每个 variant 必须同时钉死 `relics`
与 `potions` ⇒ 一次 `traceIdx` 都不读 ⇒ 排在哪里都一样）。于是现在**四条战线同时活着**：
遗物、药水、第三幕爬升度、第四幕，各往各的乘积追加，谁也不冻结谁。

---

#### 甲、第三幕 × `@tgt1`（实现侧 0 行）

##### 甲一、筛选：7 / 15，判据是「场上会不会同时有两只可选目标」

判据照抄第三十一批，**不是「开局有几只」**。逐个读 `MonsterGroup::createMonsters` +
`Monster::onHpLost`（全参考只有三种大史莱姆的分裂与守卫者的模式切换）+ 四个召唤宿主：

| 编队                                        | 开局                              | 收 / 不收 | 理由                                               |
| ------------------------------------------- | --------------------------------- | --------- | -------------------------------------------------- |
| `three_shapes` / `four_shapes`              | 3 / 4 只                          | ✅        | `createShapes`                                     |
| `sphere_and_two_shapes`                     | 3 只                              | ✅        | 两只形状 + 球状守卫者                              |
| `three_darklings`                           | 3 只                              | ✅        |                                                    |
| `reptomancer`                               | 1 / 2 / 4 号位三只（0 与 3 空格） | ✅        | 还会召唤                                           |
| `awakened_one`                              | **3 只**（邪教徒 ×2 + Boss）      | ✅        | ⚠ 编队与 Boss 同名，容易误判成单怪；Boss 在 2 号位 |
| `donu_and_deca`                             | 2 只                              | ✅        | 迪卡 0 号位、多努 1 号位                           |
| `orb_walker` / `spire_growth` / `transient` | 1 只                              | ✗         | 中途也不会变多                                     |
| `maw` / `writhing_mass` / `giant_head`      | 1 只                              | ✗         | 同上                                               |
| `nemesis` / `time_eater`                    | 1 只                              | ✗         | 同上                                               |

⚠⚠ **「中途会变多」这条在第三幕上要单独证一次，不能想当然**：第三幕**一个分裂都没有**
（`Monster::onHpLost` 那个 switch 里只有三种大史莱姆与守卫者），唯一的召唤宿主是蜥蜴法师
（已在名单里）。本幕其余的场上变化全是**减少**（复形怪消逝自杀、爆破怪自爆）或**就地**
（觉醒者假死、暗影客重生）。所以八个单怪编队真的一个都进不来。
✅ 这条判据现在是 harness 里的一道 `switch`（点名单怪编队就 `return 1`），不是注释。

⚠ variant 由 `act3Variants` **逐个派生**（只改 `targetPolicy` / `relics` / `potions`），
编队被筛空的三个 variant（批 33 / 35 / 38，全是单怪）整个不出现 —— 5 个 variant、7 个文件。
⚠ 钉死的是**金刚杵 + 格挡药水 + 力量药水**，与第四十六批逐字相同（金刚杵在 `src/combat`
里只有 `initRelics` 一个读点 ⇒ 这些文件里没有任何现象能记到遗物头上）。
⚠ 代价同第四十六批：这 7 个文件与它们的 asc0 对应文件在**遗物 / 药水上不可比**。
本批要买的东西全部是在同一份文件内部用变异量的，所以这笔账划算。

##### 甲二、⚠⚠ 第八个文件：全语料第一个**双后缀** `donu_and_deca@asc19@tgt1`

迪卡守护方阵的**镀甲那一半只在 asc>=19 存在**（第四十六批装的），所以它那条 `noAliveGate`
**两条轴同时**才观察得到。分组键的 `[@ascN][@tgtN]` 顺序从第三十一批起就定死了，
正是为了这一天——`split-traces.mjs` / 两个测试文件**一行都不用改**。

##### 甲三、逐条例数（3 条探针，42585 例基线）

| 改坏的地方                                             |                                           例数 |
| ------------------------------------------------------ | ---------------------------------------------: |
| 迪卡守护方阵·**格挡**那半补上 `monstersAlive > 1` 的门 | **53**（`@tgt1` 46 + `@asc19@tgt1` 7，此前 0） |
| 迪卡守护方阵·**镀甲**那半补上同一道门                  |            **7**（只有 `@asc19@tgt1`，此前 0） |
| `MINION_LEADER` 判胜路径整条去掉                       |                  **56**（全库，第三十一批 21） |

⚠⚠ **第三条要拆开看，而且拆开之后推翻了一条旧的盲区记录**。按文件拆：
`GREMLIN_LEADER@tgt1` 17 / `REPTOMANCER@relic4` 17 / `GREMLIN_LEADER@relic1` 7 /
`AUTOMATON@relic4` 6 / `GREMLIN_LEADER` 4 / `REPTOMANCER@relic1` 3 /
`COLLECTOR@relic4` 1 / `GREMLIN_LEADER@relic4` 1 —— **`REPTOMANCER@tgt1` 与
`AWAKENED_ONE@tgt1` 都是 0**。

结论有两条，都要写进盲区表：

1. **蜥蜴法师那一半其实早就有背书了**，是第四十批的 `reptomancer@relic4` 关掉的（17 例）。
   旧盲区表把「蜥蜴法师 / 觉醒者」写成一条、关门条件写成 `@tgt1`，那是**错的**。
2. **觉醒者那一半仍然 0**：`awakened_one@tgt1` 里玩家一次都没能在邪教徒还活着的时候
   打死二阶段（45 张牌组下二阶段 300 血 + 两只每回合 +3 力量的邪教徒）。
   新的关门条件是「一副能打穿觉醒者二阶段的牌组 × `@tgt1`」，**与目标策略本身无关**。

> **判据（补一条）：兑现一条盲区之后，把它的例数按文件拆开再下结论。**
> 合计非 0 完全可能来自另一个宿主，而被点名的那个还是 0 —— 这与第三十五 / 三十六批
> 「两个增量方向要逐条拆开量」是同一条教训的另一面。

---

#### 乙、第四幕 + 事件编队（**怪物线收官**）

##### 乙一、牌组：「先量再定」第十次，也是**第一次往弱里挑**

三个编队分两个 variant，因为**腐化之心要的东西与另外两个正好相反**。

腐化之心 750 血 + 无敌 300 ⇒ **强牌组三个回合就打完**，而「强化」那一招的档位由
`buffCount = getMonsterTurnNumber() / 3` 决定（第 4 / 7 / 10 / 13 个怪物回合各一档）。
**打得快与死得快都会压低回合数**，所以它要的是「活得久、但打不动」。实测（各 120 条）：

| 牌组                                     | 平均回合 | 最长 | 档 1（神器） | 档 2（死亡节拍） | 档 3（痛苦突刺） | 无敌最低值 |
| ---------------------------------------- | -------: | ---: | -----------: | ---------------: | ---------------: | ---------: |
| `BATCH_1 + SPOT_WEAKNESS`（22）          |     2.17 |    4 |            9 |                0 |                0 |        247 |
| 第三十七批那副（45，全升级）             |     3.24 |    7 |           58 |                6 |                0 |        137 |
| 第三十八批那副（59，全升级）             |     3.12 |    6 |           59 |                0 |                0 |     **14** |
| tank 32 张                               |     5.38 |   10 |          112 |               42 |                1 |        276 |
| tank 32 张（10 铁壁那版）                |     5.43 |    8 |          112 |               45 |                0 |        273 |
| ✅ **本批用的 40 张**（38 + 2 觅敌之弱） | **5.73** |   10 |      **112** |           **55** |            **3** |        266 |

⚠⚠ **代价要说清楚**：59 张那副是唯一能把无敌打到接近 0 的（14 / 300），**而且连它都
没打到 0** —— 所以 `min(damage, invincible)` 那道**钳制**在**任何**候选下都是盲区。
换成 tank 牌组因此**没有牺牲任何拿得到的东西**，反而买到了档 2 与档 3。
⚠ **没有一副牌组能打赢腐化之心**（0 / 120，全部候选）：750 血藏在 300 / 回合的上限后面，
对一副可重放的铁甲牌组来说就是够不着。这份语料里每一条 trace 都以玩家阵亡收场。
⚠ 那两张觅敌之弱是**为 `isMoveAttack` 加的**（它的唯一读者）。两张而不是四张：
每张在五张打击的牌组上是 +3 力量，是这副牌里唯一会缩短战斗的东西。

另外两个编队用**第三十七批那副 45 张全升级**（自带四张觅敌之弱）。三副候选都能让
六条尖塔招式与五条强盗招式全部「出现且执行」，选它是因为**它在任何一列上都不是最后一名**：

| 牌组               | `shield_and_spear` 平均 / 最长 | 重砸执行 | 贯穿执行 | `masked_bandits` 平均 / 最长 | 尖头怪执行 | 十字斩执行 |
| ------------------ | -----------------------------: | -------: | -------: | ---------------------------: | ---------: | ---------: |
| 22 张              |                       2.50 / 4 |      120 |      132 |                     4.47 / 6 |         80 |         72 |
| ✅ 45 张（全升级） |                       4.14 / 9 |      127 |      224 |                     3.88 / 7 |        130 |         68 |
| 59 张（全升级）    |                      4.65 / 10 |      112 |      222 |                     3.05 / 6 |         88 |         32 |

##### 乙二、三条共享路径上的新机制

| 机制                     | 位置                                                                                                        | 为什么是这一批                                                |
| ------------------------ | ----------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| **无敌（INVINCIBLE）**   | `attackedUnblockedHelper` 链的**第一格** + `damageUnblockedHelper` 的第一句 + `applyStartOfTurnPowers` 复位 | 全参考**只有腐化之心**带它                                    |
| **死亡节拍**             | `onAfterUseCard` 那道 `triggerOnUse` 门里的**第三条**                                                       | 第三十八批做时间扭曲时留的 `TODO(后续PR)`                     |
| **被围攻（SURROUNDED）** | `Monster::calculateDamageToPlayer` 的乘法链**第一格**，读 `Player::lastTargetedMonster`                     | 全参考**唯一**一处「怪物的 `preBattleAction` 给玩家上 Power」 |

⚠⚠ **`attackedUnblockedHelper` 那条 else-if 链现在八格全部有宿主、全部有背书**，这是第一次：
无敌（本批）→ 镀甲（二十五）→ 蜷缩（十三）→ 飞行（二十四）→ 易塑 / 反应（二十三 / 三十五）
→ 荆棘（三十二）→ 沉睡（十八）→ 变换（三十四）。第三十五批那句「链上现在只剩第一格
『无敌』没有宿主（腐化之心）」的账，本批结清。

##### 乙三、逐条例数（77 条探针，42585 例基线）

⚠ 跑批前后各跑了一次**不带任何变异**的对拍，两次都是 **0 失败**；跑批期间没有提交任何东西，
只开了一个 runner。收尾 `git status` 干净。
...
| 开局层数 300 → 200                              |                   **120** |
| 每个怪物回合的**复位**整条去掉                  |                   **116** |
| `attackedUnblockedHelper` 链首**整格**去掉      |                   **116** |
| `damageUnblockedHelper` 那一份去掉              | **0**（盲区，见「乙四」） |
| 链首的 `min()` **钳制**去掉（只扣层数不削伤害） |                     **0** |
| 挪到链的**最后一格**（位置）                    |                     **0** |
...
| 改坏的地方                    |    例数 |
| ----------------------------- | ------: |
| `onAfterUseCard` 那条整条去掉 | **120** |
| 开局层数 1 → 2                | **120** |
| 入队改同步                    |   **0** |
| 挪到时间扭曲之前（三者顺序）  |   **0** |

**（丙）被围攻 + `lastTargetedMonster`**

| 改坏的地方                                                       |    例数 |
| ---------------------------------------------------------------- | ------: |
| 整条去掉（×1.5 从不生效）                                        | **119** |
| 倍率 1.5 → 1.25                                                  | **119** |
| 不再写 `lastTargetedMonster`（整场停在初值）                     | **119** |
| `facingSelf` 去掉「上一次打的那只已经死了」那个析取项            |  **11** |
| `lastTargetedMonster` 初值 1 → 0                                 |   **0** |
| 写 `lastTargetedMonster` 的门去掉 `requiresTarget`（技能牌也写） |   **0** |

⚠ 第四行值得单记：护盾（0 号位）被玩家打死 **26 / 120**，此后长矛读
`arr[0].isDeadOrEscaped()` 为真、×1.5 当场停掉 —— 那 11 例就是这一段。

**（丁）腐化之心**（17 条）

| 改坏的地方                                      |                        例数 |
| ----------------------------------------------- | --------------------------: |
| 出招规则：`firstTurn` 的虚弱化去掉 / 两支对调   |           **120** / **120** |
| 血弹 / 回响收尾的 `noOpRollMove` 去掉           |                     **120** |
| 虚弱化：五张牌换序 / 改入队 / 去向改弃牌堆      | **120** / **120** / **120** |
| 回响伤害 40 → 35                                |                     **120** |
| 血弹段数 12 → 10                                |                     **113** |
| 强化：档 1 的神器 2 → 1                         |                     **112** |
| 强化：那 +2 力量整条去掉 / `buffCount` 除数 3→2 |           **112** / **112** |
| 强化的收尾：真 `rollMove` 改成 `noOpRollMove`   |                     **112** |
| 回响收尾的 else 分支抄成回响自己                |                      **96** |
| 血弹收尾的 `% 3` 改成 `% 4`                     |                      **94** |
| **强化：档 2 的死亡节拍 +1 整条去掉**           |                      **55** |
| **强化：档 3 的痛苦突刺整条去掉**               |                   **3** ⚠薄 |
| 虚弱化的三条减益改成入队                        |     **0**（等价改写，见下） |
| 强化：`setStatus(max(0,str)+2)` 抄成 `buff(2)`  |         **0**（盲区，见下） |

**（戊）尖塔护盾 / 尖塔长矛**（16 条）

| 改坏的地方                                          |                例数 |
| --------------------------------------------------- | ------------------: |
| 护盾开局：被围攻整条去掉 / 神器 1 → 2               |   **120** / **120** |
| 护盾收尾：去掉 `lastMoveBefore(INVALID)` 那个析取项 |             **120** |
| 护盾收尾：猛击那条两支对调 / `noOpRollMove` 去掉    |   **120** / **120** |
| 护盾出招规则：加固 / 猛击两支对调                   |             **120** |
| 猛击的 -1 力量**整条**去掉                          |             **120** |
| 灼烧打击塞的灼伤 2 → 1 张 / 改去抽牌堆              |   **120** / **120** |
| 长矛出招规则：`firstTurn` 去掉 / 收尾抄成护盾那条   |   **120** / **120** |
| 编队建怪顺序：护盾与长矛对调                        |             **120** |
| 加固：给 1 号位那半去掉 / 自己那半去掉              |   **119** / **119** |
| 贯穿段数 3 → 4                                      |             **119** |
| 重砸：格挡那半整条去掉                              |             **112** |
| 穿刺：改成只给自己（去掉 0 号位那半）               |             **106** |
| 灼烧打击伤害 5 → 6                                  |              **86** |
| 猛击伤害 12 → 11                                    |              **72** |
| **穿刺：补上 `monstersAlive > 1` 的门**             |              **11** |
| **重砸：格挡取基础 34 而不是算完的伤害输出**        |               **8** |
| **重砸：两条动作对调（先加格挡再打人）**            |               **7** |
| **猛击的 -1 力量改成入队**                          |               **3** |
| 加固：友军那半补上 `monstersAlive > 1` 的门         | **0**（盲区，见下） |

⚠⚠ **第「重砸格挡取基础值」那一行（8 例）是新原语 `deal_damage_block_equal` 存在的
唯一理由的直接背书**：格挡取的是**过完 `calculateDamageToPlayer` 之后**的那个整数
（力量、被围攻的 ×1.5、玩家易伤 / 虚弱都乘进去了），拆成 `deal_damage` + `gain_block`
会退化成写死的 34。例数薄是因为它只在「护盾还活着、玩家又留着格挡去挨下一击」时可见。
⚠ 「猛击的 -1 力量改成入队」3 例：**第二十七批工头那条判据的第三个实例**
（效果入队 + 猛击打死玩家 → 主循环跳出 → 入队那条永远轮不到）。
...
| 熊抱减敏捷 -2 → -1                                   |                   **120** |
| 猛扑格挡 9 → 5                                       |                   **115** |
| 熊的三拍循环回到熊抱（撕咬 → 熊抱）                  |                   **103** |
| 苦痛斩的虚弱 2 → 1                                   |                    **92** |
| 撕咬伤害 18 → 20                                     |                    **90** |
| 尖头怪的收尾从 `none` 改成 `roll` / `no_op_roll`     |           **88** / **88** |
| 罗密欧的三拍循环回到嘲讽                             |                    **65** |
| 十字斩伤害 15 → 17                                   |                    **56** |
| 尖头怪伤害 5 → 6 / 段数 2 → 1                        |           **21** / **20** |
| 嘲讽补一条 **99 点**伤害的效果                       |                   **119** |
| 熊抱改成入队 / 猛扑格挡改成同步 / 苦痛斩虚弱改成同步 |      **0** 各（等价改写） |
| 嘲讽补一条 **1 点**伤害的效果                        | **0** ⚠**探针选错**，见下 |

⚠⚠ **最后一行是一次「假的 0」的新形状，值得单记**：给「效果表本来是空的」那条 case
补 **1 点**伤害红 **0 例**，补 **99 点**红 **119 例**（= 整个编队）。1 点被玩家回合末
**剩余的格挡**吃掉了，而剩余格挡在下一个玩家回合开始本来就清零 —— 于是那一点伤害
**在两帧快照之间凭空消失**。

> **判据（补一条）：探针的量级必须大于「会被丢弃的缓冲」。**
> 玩家的格挡是每回合清零的，所以「往怪物回合里加一点点伤害」这种探针天然可能量到假 0；
> 要么把量级抬到穿透格挡，要么改成量一个不会被清零的量（血量 / 层数 / RNG 计数器）。

**（庚）`isMoveAttack` 白名单——两个增量方向逐条拆开**

| 方向 A：白名单**漏掉**这一条攻击招 | 例数    | 方向 B：白名单**多收**这一条非攻击招 | 例数              |
| ---------------------------------- | ------- | ------------------------------------ | ----------------- |
| `POINTY_ATTACK`                    | **61**  | `SPIRE_SHIELD_FORTIFY`               | **52**            |
| `SPIRE_SHIELD_BASH`                | **59**  | `CORRUPT_HEART_DEBILITATE`           | **25**            |
| `CORRUPT_HEART_ECHO`               | **33**  | `CORRUPT_HEART_BUFF`                 | **24**            |
| `SPIRE_SHIELD_SMASH`               | **32**  | `SPIRE_SPEAR_PIERCER`                | **4**             |
| `CORRUPT_HEART_BLOOD_SHOTS`        | **28**  | `ROMEO_MOCK`                         | **2**             |
| `ROMEO_AGONIZING_SLASH`            | **27**  | `BEAR_BEAR_HUG`                      | **0** ⚠见「乙四」 |
| `BEAR_MAUL`                        | **17**  |                                      |                   |
| `ROMEO_CROSS_SLASH`                | **16**  |                                      |                   |
| `BEAR_LUNGE`                       | **15**  |                                      |                   |
| `SPIRE_SPEAR_SKEWER`               | **5**   |                                      |                   |
| `SPIRE_SPEAR_BURN_STRIKE`          | **3**   |                                      |                   |
| **合计**                           | **237** | **合计**                             | **104**           |

「谓词恒真」那个方向是**全库数字 7056**（第三十三批是 2594），只能用来判「不是 0」；
本批三个新编队各自贡献 `SHIELD_AND_SPEAR` 55 / `THE_HEART` 47 / `MASKED_BANDITS_EVENT` 2。

##### 乙四、0 例的九条，逐条分类

⚠ 三类分开记：**结构性不可观测** / **盲区**（附关门条件）/ **等价改写**。

| 改法                                               |  例数 | 分类                                                                                                                                                                                                                                                                                |
| -------------------------------------------------- | ----: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 无敌：链首的 `min()` 钳制去掉                      | **0** | **盲区**：**没有任何候选牌组能把无敌打到 0**（最强的一副只到 14 / 300），所以钳制从没生效过。**关门条件：一副能在一个玩家回合里对腐化之心打出 300 点以上的牌组** —— 而那样的牌组会把仗压到 3 回合，与「强化」的档位互斥。两者要靠**两个 variant**（同一个编队两份文件）才能同时拿到 |
| 无敌：`damageUnblockedHelper` 那一份去掉           | **0** | **盲区**：腐化之心那副牌组里**一点非攻击伤害都没有**（没有荆棘 / 灼伤 / 主宰，钉的两瓶药也碰不到怪）。**关门条件：给那个 variant 加青铜鳞片或一张灼伤类的牌**                                                                                                                       |
| 无敌：挪到链的**最后一格**                         | **0** | **结构性不可达**：腐化之心身上**没有链上后七位中的任何一位**（镀甲 / 蜷缩 / 飞行 / 易塑 / 反应 / 荆棘 / 沉睡 / 变换），参考里也没有第二只带无敌的怪 ⇒ 与第三十二批荆棘、第三十四批变换的位置同族，**永久**                                                                          |
| 死亡节拍：入队改同步                               | **0** | **等价改写**：`onAfterUseCard` 这条路上它是那道门里最后一句、后面没有别的动作读玩家血量（按第二十六批那条判据：这条 case 自己排的队列动作只有它一条）                                                                                                                               |
| 死亡节拍：挪到时间扭曲之前                         | **0** | **结构性不可达**：三者共用一道门，但**没有一只怪同时带其中两位**（时间吞噬者带时间扭曲、巨头带缓慢、腐化之心带死亡节拍，三只都是单怪编队）。**永久**                                                                                                                                |
| `lastTargetedMonster` 初值 1 → 0                   | **0** | **盲区**：唯一能分辨的局面是「玩家第一个回合一张指向性的牌都不打，而带被围攻的怪就已经出手了」。**关门条件：一副第一回合只打技能牌的牌组 × `shield_and_spear`**                                                                                                                     |
| 写 `lastTargetedMonster` 的门去掉 `requiresTarget` | **0** | **等价改写**（在当前语料下可证）：非指向性的牌带的 `item.target` 恒是 0，而策略的指向性攻击也恒打 0 号位 ⇒ 两种写法写进去的值一样。⚠ 有了 `@tgt1` × 带被围攻的编队就会分岔，那是关门条件                                                                                            |
| 强化：`setStatus(max(0,str)+2)` 抄成 `buff(2)`     | **0** | **盲区**：`max(0, s) + 2` 与 `s += 2` **只在力量为负时才分岔**，而腐化之心那副牌组里没有任何降力量的手段。**关门条件：牌组里有缴械 / 黑暗镣铐**（同第三十七批觉醒者那句 `strength = max(0, strength)`，两条同族）                                                                   |
| 加固：友军那半补上 `monstersAlive > 1` 的门        | **0** | **盲区**：要「长矛先死而护盾还活着」，而策略恒打 0 号位（护盾）。**关门条件 `shield_and_spear@tgt1`** —— 与迪卡守护方阵那两条**完全同族**，而那两条本批正是这么关掉的                                                                                                               |
| 白名单多收 `BEAR_BEAR_HUG`                         | **0** | **盲区**：熊在 **2 号位**，而熊抱只出在第一个怪物回合 —— 那时尖头怪还活着，觅敌之弱指向 0 号位，熊的意图**根本没被问过**。**关门条件 `masked_bandits_event@tgt1`**（打下标最大的活怪 = 熊）                                                                                         |
| 熊抱 / 猛扑格挡 / 苦痛斩虚弱的「同步 ↔ 入队」      | **0** | **等价改写**：三条 case 里每一条自己排的队列动作都只有一条（或零条），按第二十六批那条判据不算盲区                                                                                                                                                                                  |
| 虚弱化的三条减益改成入队                           | **0** | **等价改写**：整条 case 里一个 `addToBot` 都没有（八句全同步），改成入队之后队列里就它们几条、立刻出队，而中间没有任何读者                                                                                                                                                          |
...
| 变异                             | 例数                              | 分类与理由                                                                                                                                                                              |
| -------------------------------- | --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 孢子云 `addToTop` → `addToBot`   | **0**（第四十八批第三次量，仍 0） | **结构性盲区，但根因换了**：亡语触发从 83 / 375 涨到 120 / 120 还是 0——卡的是「那一刻队列里没有别的动作」，**不是**「同伴先死」。第二十五批的裁定成立，这条轴救不了它                   |
| 狂怒连斩的 asc2 伤害档 7 → 8     | **0**                             | **结构性盲区，但关门条件从死结变活**：本批只做 asc0。以前的说法是「这一招的效果在任何档位下都没有预言机」，现在只差 `asc19 × tgt1` 那个组合                                             |
| ~~激怒（地精头目）改成遍历全体~~ | ✅ **116**                        | **第四十八批已关**：关门条件正是这里写的 `COLOSSEUM_EVENT_NOBS`（工头 0 / 头目 1），本批把它装了。参考写死 `monsters.arr[0]` ⇒ 头目在 1 号位时激怒**整场不触发**，改成遍历全体红 116 例 |
...
- 第十八批新增的盲区，全部 ⸙：
  ...
  - ~~**`preBattleAction` 的「睡着才上金属化」那道前提分不出来（0 例）。**~~
    ✅ **第四十八批已关（120 例 = 整份文件）**：关门条件正是这里点名的 `LAGAVULIN_EVENT`
    （睡魔事件版，不置沉睡位），本批把它装了。给事件版也置上沉睡位 ⇒ 红 120 例。
    ⚠ 同一段里**「睡满三回合自然醒」那条仍然开着**：事件版压根不睡，它救不了那一条。
...
- 第十六批新增的盲区，全部 ⁝：
  - **孢子云的 `addToTop` 改成 `addToBot` 分不出来（0 例）。** 参考写的是 `addToTop`
    ...
    ⚠⚠ **这条被量过三次，三次都是 0，而且每次都是不同的猜想被证伪**：第二十五批
    （真菌兽的仗更长）、第三十一批（`@tgt1` 让「有同伴时死」从 83 / 375 涨到 120 / 120）、
    **第四十八批**（`two_fungi_beasts` / `mushrooms_event` 把「多只带孢子云的怪同场」
    也造出来了，240 条 trace）。**「编队」这条路到此为止走尽了**，上面那句话里的
    「亡语 + 多条排队动作」才是唯一还没试过的方向——下一个人别再拿编队撞它。
...
⚠ 这一条不是数据表与参考的冲突，是第四十七批把它临时挂在下面「待裁定」里的；
第四十八批做完之后挪到这里，免得「待裁定」里躺着一条已经做完的事。

- ✅ **测试基础设施撞到规模上限（第四十七批记，第四十八批结清）：对拍把全部 trace
  一次性读进内存。** 当时 CI 连红两次，两次的表象完全不同：
  ...
  ⚠⚠ **当时给这两次写的因果有一半是错的，结清时一并纠正：**

  - 真凶是**池**，不是内存：`forks` 下 worker 要为 42586 个用例各发一次 `onTaskUpdate`，
    走进程间 IPC。换 `pool: "threads"`（MessageChannel、同进程）之后两个表象一起消失。
  - 「2.1GB 堆」这个数字是真的，但「4 个 worker 并发时峰值叠加，所以要 `--maxWorkers=1`」
    是假的。第四十八批实测：**拆分前 4 worker 峰值 RSS 2.70GB、1 worker 2.73GB**
    ——`maxWorkers` 在那时根本不是变量。那个旋钮从来没起过作用。

  **结清（第四十八批）：对拍拆成 16 个测试文件**，机器搬进 `test/helpers/trace-replay.ts`
  （非 `*.test.ts`，自身不产生用例），切片登记表在 `test/helpers/trace-slices.ts`。

  | 布局             | maxWorkers | 峰值 RSS    | 墙钟       |
  | ---------------- | ---------- | ----------- | ---------- |
  | 拆分前（单文件） | 4          | **2.70 GB** | 31.3 s     |
  | 拆分前（单文件） | 1          | 2.73 GB     | 33.3 s     |
  | 16 片            | 1          | **0.81 GB** | 36.3 s     |
  | 16 片            | 4          | **1.82 GB** | **13.8 s** |

  于是 `ci.yml` / `release.yml` 的 `--max-old-space-size=3072` 与 `test:ci` 的
  `--maxWorkers=1` **两个都删掉**（理由都已被上表证伪，墓志铭留在 `ci.yml`）。
  以后再遇到内存压力，旋钮是 `SLICE_COUNT`（加片），不是 `--max-old-space-size`。

  ⚠⚠ **粒度一个字没动，而且是量出来的。** 硬约束是「一条 trace 一个 `it()`」——
  下面这五条探针在**拆分前后**各跑一遍，逐例相同，且都能对上历史数字：

  | 探针（批次）                                                      | 历史 | 拆分前 | 拆分后 | 说明                                                                          |
  | ----------------------------------------------------------------- | ---: | -----: | -----: | ----------------------------------------------------------------------------- |
  | `Actions::AttackPlayer` 的 `clearOnCombatVictory` 改 true（十九） |    7 |    212 |    212 | 其中 `THE_GUARDIAN` 一组**恰好 7**；多出的 205 来自第二十批之后新增的 31 个组 |
  | 死亡节拍整条去掉（四十七乙）                                      |  120 |    120 |    120 | 一字不差（`the_heart` 独此一份）                                              |
  | 贤者之石 `initRelics` 加 `alive` 过滤（四十）                     |  480 |    480 |    480 | 一字不差（`@relic1` 冻结）                                                    |
  | 束缚回合末递减一层（三十三）                                      |  104 |    224 |    224 | `SPIRE_GROWTH` **104** + 第四十六批才有的 `@asc19` 120                        |
  | 打击假人去掉 +3（四十三）                                         |  343 |    343 |    343 | 一字不差（`@relic9` 冻结）                                                    |

  ⚠ **两条「对不上」的都是语料长大了，不是粒度变了**：按编队分组拆开之后，
  当时存在的那几个组仍然逐例相同。这条判据写进 WORKFLOW 了——
  以后拿旧例数当基线时先分组，别直接比总数。

  ⚠⚠ **拆分引入的唯一新风险已经被一条用例守住**：新增的 trace 文件若不落进任何切片，
  它会**静默地不被测试**。`test/sts-combat-trace-meta.test.ts` 里那条完备性断言用
  **目录的实际内容**验证「每个 `.jsonl` 恰好被一片认领」，两个方向都实测会红
  （把某片谓词改窄 → 报出 5 个孤儿文件名；改宽 → 报出被 16 片同时认领）。
  ⚠ 断言不许改成「核对写死的名单」，也不许加「兜底切片」——理由见那个文件的注释。

  用例数：对拍 **42585**（不变）+ 不变量 1 + **完备性断言 1（新增）** + 其余 288 = **42875**。
  ⚠ 比第四十七批的 42874 多的这 1 条就是完备性断言本身。
...
- ⚠⚠ **`Actions::MakeTempCardInDrawPile` 的 `shuffleInto == false` 那一支是空的（第四十七批）。**
  ...
  Action Actions::MakeTempCardInDrawPile(const CardInstance &c, int amount, bool shuffleInto) {
      return {[=](BattleContext &bc) {
          for (int i = 0; i < amount; ++i) {
              if (shuffleInto) { … createTempCardInDrawPile(idx, c); }
              // todo else
          }
      }};
  }
  ...
  全参考**只有一个调用点传 `false`**：尖塔长矛的灼烧打击在 **asc>=18** 时
  `MakeTempCardInDrawPile(CardId::BURN, 2, false)`（`MonsterSpecific.cpp:1802`）
  ——也就是说 **asc18 的长矛一张灼伤都不塞**，比 asc<18（塞两张进弃牌堆）还轻。
  真实游戏那一档是「把两张灼伤放到抽牌堆**顶**」。

  **三条判据只过第 ①**（形状里有明显的未完成痕迹：`// todo else`），②③ 都不过：
  没有第二个信源能说「放顶」的正确实现是什么（`createTempCardInDrawPile` 只有「洗进随机位」
  这一个入口，没有「放顶」的），而补一个等于**发明行为**。
  **照抄参考的实际行为（asc18 什么都不塞），记在这里。** 本批只做 asc0 ⇒ 那一支结构性不可达；
  第四幕的爬升度那一批必须先裁定这一条再动手。

- ⚠ **尖塔护盾的猛击有一支需要充能球，铁甲永远走不到（第四十七批）。**
  ...
  （`MonsterSpecific.cpp:1763-1767`；`Actions::SpireShieldDebuff` 是
  `aiRng.randomBoolean() ? debuff<FOCUS>(-1) : debuff<STRENGTH>(-1)`，`Actions.cpp:529-537`）

  `orbSlots` 只有机器人才非 0，所以这一支在**铁甲的任何牌组下都不可达**——而且它**多掷一次
  `aiRng`**，登记机器人时必须一起做，否则那一档的 RNG 计数器会整体错位。
  **这不是参考的 bug，是我们的角色覆盖面问题**，与「待裁定」那一族不同；记在这里只是
  为了让登记机器人的人一眼看到。**本批不实现**（没有预言机）。

- ⚠ **被围攻的判据参考自己标了 `// todo this is probably wrong`（第四十七批）。**

  `Monster::calculateDamageToPlayer`（`Monster.cpp:565-570`）用
  `p.lastTargetedMonster == idx || arr[p.lastTargetedMonster].isDeadOrEscaped()` 当
  「你正面对着它吗」。真实游戏的「背刺」是按**站位**（左右两侧）算的，与「你上一次打了谁」
  不是一回事。三条判据只过第 ①（参考自注），**照抄、不打补丁**——本批给它拿到了
  119 例的背书，那 119 例背书的是**参考的这条规则**，不是真实游戏的背刺。
...
## 第三幕完成度小结（第三十九批收官 16 / 16 编队；**第四十六批补上 asc19，两档全满**）
...
✅ **第四十六批把这一维补上了**：15 个第三幕编队 × asc19（`jaw_worm_horde@asc19`
第二十一批就在第一幕那份里），第三幕 17 只怪的 `ascCalibrated` 全部置起。
**至此三幕 54 / 54 个编队都有 `{asc0, asc19}` 两个档位**，`constructMonster` 那道
`ascension > 0` 的 throw 只剩腐化之心一只挡着（第四幕，它本来就没有招式）。
做法与第三十批逐字同构，两处不同：① 它走的是**第七个乘积**（不是往第三幕那个里追加），
② 每个 variant **钉死遗物与药水**，于是它不读 `traceIdx`、与遗物 / 药水两条战线互不冻结。
逐条例数见「验证方式 · 第四十六批」。

### 一、背书覆盖：16 / 16 个编队、17 只新怪，在 **asc0 与 asc19 两个档位**上有 trace 背书
...
| **四十六** | 第七个乘积的 8 个 | **上面 15 个全部 × asc19**                               | —（同 17 只怪的高档）             | ⚠ **由上面 8 行逐个派生**（只改 ascension / relics / potions） |
...
| 爬升度档位       | asc0 + 19 | asc0 + 19 | ✅ **asc0 + 19**（第四十六批） | **54 / 54 编队 × 两档**（全满） |
| 目标策略 `@tgt1` | 11 个多怪 | 12 个多怪 | **0 个**                       | 23 / 39（第三幕一个都没有）     |

⚠ **「16 个编队全装」不等于「第三幕的怪全装」，也不等于这份语料完备**。~~三条~~**两条**缺口：
~~① **爬升度**（本幕 17 只怪一只都没校准）；~~ ✅ **第四十六批做掉了**；
② **目标策略**（第三幕一个 `@tgt1` 文件都没有，而它恰恰是本幕好几条盲区的关门条件——
第四十六批之后**它是本幕唯一剩下的整块结构性缺口**）；
③ 剩下的 **6 只怪 / 9 个编队**在第四幕与事件里。
...
| 盲区                                                                | 批次                         | 关门条件                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ------------------------------------------------------------------- | ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ~~**第三幕全部 17 只怪的爬升度分档 + 第二组血量区间**~~             | 三十二~三十九                | ✅ **第四十六批全部关掉**（15 编队 × asc19 / +39MB，实测 43MB 那个估算偏高）。逐条例数见「验证方式 · 第四十六批」。⚠ 剩下的两条**不是**这一条的余数，单列在下面                                                                                                                                                                                                                                                                                                        |
| ~~**迪卡守护方阵的 `noAliveGate`**（给已死的多努加格挡 / 加镀甲）~~ | 三十九 / 四十六 / **四十七** | ✅ **第四十七批甲全部关掉**：格挡那半红 **53 例**（`donu_and_deca@tgt1` 46 + `@asc19@tgt1` 7），镀甲那半红 **7 例**（只有 `@asc19@tgt1`——那一半只在 asc>=19 存在，所以它是全语料第一个**需要两条轴同时**的盲区，也是第一个双后缀文件的由来）。⚠ asc0 / asc19 那两份原文件仍是 0，正好印证第四十六批「爬升度救不了它」的裁定                                                                                                                                            |
| **`MINION_LEADER` 在**觉醒者**身上的判胜路径**（蜥蜴法师那半已关）  | 三十六/三十七/**四十七**     | ⚠⚠ **第四十七批甲把这一条拆成两半，并推翻了它的一半**：**蜥蜴法师那一半其实早就有背书**（第四十批的 `reptomancer@relic4` 17 例 + `@relic1` 3 例；旧表把两只写成一条是错的），而 `reptomancer@tgt1` 本身是 **0**。**觉醒者那一半仍然 0**——`awakened_one@tgt1` 里玩家一次都没能在邪教徒还活着时打死二阶段。新关门条件是「一副能打穿觉醒者二阶段的牌组 × `@tgt1`」，**与目标策略本身无关**。⚠ 「去掉这个 buff 红 46 例」量的是**快照里少了一个 power 条目**，不是判胜路径 |
| ~~**`reptoSummonHelper` 的 `searchOrder` 第 4 项**~~                | 三十六                       | ✅ **第四十六批关掉**（asc18 一次召 2 只）：去掉第 4 项红 **58 例**、`noOpRollMove` 挪到循环外红 **120 例**（= 那份文件全部）。⚠ 同一行原先还挂着「`++monstersAlive` 在循环内还是循环外」——实测 **0 例**，那是**等价改写**不是盲区（循环体里没人读它）                                                                                                                                                                                                                 |
| **第三幕 15 处 `hpHigh` 的「阈值恰好是 7 / 8 / 9」**                | 四十六                       | 单一档位证不了阈值。实测匕首 8→7、巨头 8→7、时间吞噬者 9→8 **各 0 例**。关门条件是 **asc7 + asc8 + asc9** 三个档位（与第一 / 二幕那两条合并成同一条）                                                                                                                                                                                                                                                                                                                  |
| **尖刺客荆棘 `{3,4,7}` 的中间档**                                   | 四十六                       | `{0,19}` 只取得到两端。**关门条件 asc16**——与第三十批那四条（秘法师鼓舞 / 地精首领鼓舞 / 工头伤口 / 冠军暴怒）合起来这一族现在 **5 条**                                                                                                                                                                                                                                                                                                                                |
...
| **第三幕 17 只怪的第二组血量区间「阈值恰好是 7 / 8 / 9」**    | 全幕   | 单一档位证不了阈值（第二十二批已证，**第四十六批在第三幕上第三次量到 0 例**：匕首 8→7 / 巨头 8→7 / 时间吞噬者 9→8 各 0）。守着它的只有 `data-tables.test.ts` 那张**逐怪期望阈值**表，所以那张表**不许写死成一个常数** |

### 三、数据规格的最终数字

| 项                      | 值                                                                                                                        |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| 第三幕文件数            | **15 + 15**（`ENC_V0_ACT3` 与 `ENC_V0_ACT3_ASC19`，每份 120 行 = 40 种子 × 3 层，**整份冻结**）                           |
| 第三幕体积              | **56MB（asc0）+ 39MB（asc19）**                                                                                           |
| 第三幕例数              | **1800 + 1800**                                                                                                           |
| 最大 / 最小的第三幕文件 | `donu_and_deca` **8.0MB** / `orb_walker` **1.7MB**（差 4.7 倍，长度由 Boss 决定）                                         |
| **仓库总计**            | **197 个文件 / 938MB / 41265 条 trace**（第四十六批：+15 个 `@asc19` 文件 / +39MB / +1800 例）                            |
| **对拍例数**            | **41266**（41265 条 + 1 条「trace 数据自身的不变量」）                                                                    |
| 文件构成                | 第一幕 asc0 20 + asc19 20；第二幕 asc0 19 + asc19 19；`@tgt1` 23；第三幕 **asc0 15 + asc19 15**；`@relicN` 47；`@potN` 19 |
...
⚠⚠ **第三幕的 asc19 反而比 asc0 便宜（39MB vs 56MB），而这与第三十批那条「≈ 2.7MB / 编队 /
档位、体积几乎线性于种子数」的经验规律不符——原因值得记住**：种子数相同（都是 40），
差的是**仗长**。asc19 的玩家进场只有 68/75 血、还多一张登顶者之殇，三个 Boss 编队的平均
回合数从 8.55 / 7.18 / 6.81 掉到 4.65 / 4.72 / 4.97。
**所以「加一个档位」的体积估算要按「那个档位下的仗有多长」来推，而不是按 asc0 的文件大小乘系数。**
⚠ 反过来，这也正是本批最薄的两条背书（觉醒者复活 7 例、加速 asc19 的 32 格挡 10 例）的根：
**仗短 = 打不到血线**。要加厚只能换更强的牌组或更多种子。
...
| #     | 候选                                                           | 解锁量                                                                | 成本                                                                                            | 爆炸半径                                                                                                                                                                     |
| ----- | -------------------------------------------------------------- | --------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ~~①~~ | ~~**第三幕的爬升度**（`asc19 × 15`）~~                         | 本幕最大的一块盲区：17 只怪的全部数值分档 + 第二组血量区间            | 数据 ≈ 43MB（**实测 39MB**）                                                                    | ✅ **第四十六批已交付**。⚠ 实现侧比预估的**小得多**：招式与 `preBattleAction` 的分档第三十二~三十九批当时就照抄了，真正要填的只有 `hpHigh` + `ascCalibrated`，外加迪卡那笔账 |
| ②     | **`asc7 + asc16`**（跨三幕）                                   | 关掉「三档里的中间那一档」**13** 条 + 冠军那两族阈值 + 血量阈值 7/8/9 | 数据 ≈ **180~200MB**（两档 × 三幕）                                                             | **零**：实现侧一行不改，纯数据。⚠ 第四十六批给这一族添了第 5 条中间档（尖刺客荆棘 `{3,4,7}`）与 3 条阈值                                                                     |
| ~~⑥~~ | ~~**第三幕的 `@tgt1`**（15 编队 × 目标策略）~~                 | 迪卡守护方阵的两条 `noAliveGate`、`MINION_LEADER` 的判胜路径          | 实测 **7 个多怪编队 + 1 个双后缀 = 8 份 / 33MB**（判据筛掉 8 个单怪的，比 15 份的估算便宜一半） | ✅ **第四十七批甲已交付**，实现侧确实一行没改                                                                                                                                |
| ~~③~~ | ~~**第四幕**（`shield_and_spear` / `the_heart`）~~             | 2 个编队 / 3 只新怪（尖塔护盾 / 尖塔长矛 / 腐化之心）                 | 实测 **2 份 / ~8MB**（估的 8~16MB 偏高）                                                        | ✅ **第四十七批乙已交付**，顺带把 `MASKED_BANDITS_EVENT` 也装了 ⇒ **怪物线 65 / 65**                                                                                         |
| ~~④~~ | ~~**剩下的 6 个编队**（`TWO_FUNGI_BEASTS` + 五个 `*_EVENT`）~~ | **0 只新怪**——六个全部由已登记的怪组成                                | 实测 **6 份 / 12MB**（估得一分不差）                                                            | ✅ **第四十八批已交付**。实现侧比预估的还小：连 `ENCOUNTER_BUILDERS` 都没加。wiring 那条用例确实失去了样本，照办改成了不变量                                                 |
...
✅ **① / ③ / ④ / ⑤ / ⑥ 都已交付**（④ 是第四十八批）。**剩下的顺序建议：
（第四幕的爬升度与 `@tgt1`）→ ⑤（继续，遗物 93 颗 / 药水 6 瓶）→ ②。**
⚠⚠ **怪物与编队两条线都收官之后，「铺量」这个词从此只指遗物 / 药水**；剩下的两条轴
（爬升度、目标策略）是**给已装内容加档位**，不再有「新内容」。而真正的下一片空白是
[一、接线](#一接线) 的第 4~~9 项（run 层的六条 RNG 流），那不是铺量、是新写。
把 ⑥ 提到 ② 前面的理由是**单位成本**：⑥ 只要 ≈ 56MB 就能一次关掉第三幕（乙）表里
**剩下的两行**，而 ② 要 ≈ 180~~200MB 才能关掉「中间档」那一族 13 条 + 三条阈值。
⚠ ⑥ 现在做得起，正是因为第四十五 / 四十六批把「新乘积不冻结前面」这条变成了 harness 里的
硬检查——⑥ 只要照抄那条检查（钉死遗物与药水），挂在第八个乘积上即可，
遗物 / 药水 / 爬升度三条战线一条都不用停。
...
## 怪物线收官小结（第四十七批：`MOVE_RULES` 65 / 65）

参考的 `MonsterId` 枚举一共 **65** 项（`MonsterIds.h`），第四十七批装上最后六只之后
**每一只都在 `MOVE_RULES` 里、每一只都在 trace 的怪物快照里出现过**。
...
1. ✅ **`attackedUnblockedHelper` 那条 else-if 链八格全部有宿主、全部有背书**——这是第一次。
   无敌（四十七）→ 镀甲（二十五）→ 蜷缩（十三）→ 飞行（二十四）→ 易塑 / 反应（二十三 / 三十五）
   → 荆棘（三十二）→ 沉睡（十八）→ 变换（三十四）。第三十五批那句「链上现在只剩第一格
   『无敌』没有宿主」的账结清。⚠ **「每一格都有宿主」不等于「每一格的位置都有背书」**：
   链上有五格的**位置**仍然是结构性盲区（没有一只怪同时带其中两位），逐条见各幕的盲区表。
2. ⚠⚠ **`sts-combat-rules.test.ts` 那条「未登记怪物 rollMove」用例永久失去了样本**，
   本批把它改成断言不变量（见「覆盖现状」那一段）。**下一批装完剩下 6 个编队时，
   `sts-combat-wiring.test.ts` 那条「未迁移编队」会遇到同一件事**，办法照抄本批。
3. ⚠ **`enemies.ts` 里再也没有「只有数据、没有行为」的条目**。此前那种条目（`corrupt_heart`
   第三十九~四十六批）是测试样本的载体，以后不会再有——要新增这种东西必须有别的理由。

### 二、六只新怪的爬升度**一只都没校准**

`ascCalibrated` 现在挡着的正好是本批这六只（第三幕那 17 只第四十六批置起了）。
血量的第二组区间**只写在注释里、没有进 `hpHigh`**（半填会被 `data-tables.test.ts` 那条
用例挡下），招式的 `ascAmount` / `ascTimes` / `minAscension` 照抄了但**一条例数都没有**。
这与第三十二~三十九批对第三幕的办法逐字相同，账记在这里：**第四幕的爬升度是它自己的一批**。

⚠ 那一批要顺带裁定两处（见下方「待裁定」）：尖塔长矛 asc18 那条**参考没实现完**的塞牌，
以及尖塔护盾猛击里那条**需要充能球**才走得到的分支。

### 三、数据规格

| 项                    | 值                                                                            |
| --------------------- | ----------------------------------------------------------------------------- |
| 第四幕 / 事件文件数   | **3**（`shield_and_spear` / `the_heart` / `masked_bandits_event`，各 120 行） |
| 第三幕 `@tgt1` 文件数 | **8**（7 个多怪编队 + `donu_and_deca@asc19@tgt1`）                            |
| 本批体积              | **+44MB**（938MB → **982MB**）                                                |
| 本批例数              | **+1320**（41265 → **42585**）                                                |
| **对拍例数**          | **42586**（42585 条 + 1 条「trace 数据自身的不变量」）                        |

---

## 编队线收官小结（第四十八批：`SUPPORTED_ENCOUNTERS` 63 / 63）

参考的 `MonsterEncounter` 枚举一共 **63** 项（`MonsterEncounters.h`，不算 `INVALID` 哨兵），
第四十八批装上最后六个之后**每一个都在 `SUPPORTED_ENCOUNTERS` 里、每一个都有一份 trace 文件**。

| 来源                              | 个数   | 批次                                   |
| --------------------------------- | ------ | -------------------------------------- |
| 第一幕（含 harness 冻结的 20 个） | 21     | 第一~二十批 + 四十八                   |
| 第二幕                            | 19     | 第二十三~二十九批                      |
| 第三幕                            | 16     | 第三十二~三十九批                      |
| 第四幕                            | 2      | 第四十七批乙                           |
| 事件专属                          | 5      | 第四十七批乙（1）+ **第四十八批（4）** |
| **合计**                          | **63** |                                        |

### 一、两条铺量线都收官了，这件事的含义

1. **「铺量」这个词从此只指遗物 / 药水。** 怪物 65 / 65（第四十七批）、编队 63 / 63（本批）
   之后，`MOVE_RULES` / `ENCOUNTER_BUILDERS` / `enemies.ts` 三张表**不会再有新条目**，
   除非参考自己加。剩下的两条轴（爬升度、目标策略）是给**已装内容**加档位。
2. ⚠ **真正的下一片空白不是铺量，是接线**：[一、接线](#一接线) 的第 4~9 项——
   `sts-encounters` / `sts-map` / `sts-neow` 接进 run 层，以及六条 run 级 RNG 流
   （`cardRng` / `relicRng` / `eventRng` / `merchantRng` / `treasureRng` / run 级 `potionRng`）。
   战斗打得再准，「哪场仗在第几层」不对，逐位精确的战斗打的也不是原版那一场。
3. ⚠⚠ **`enemies.ts` 的 `ENCOUNTERS` 里还有 13 条旧近似表的残留**（参考没有对应枚举：
   `two_orb_walkers` / `exploder` / `spiker` / `repulsor` / `two_exploders` /
   `small_slimes_a|b` / `large_slime_acid|spike` / `centurion` / `two_centurions` …）。
   它们**当前真的可达**（第一 / 三幕的权重表里有），而且永远不会有预言机。
   接 `sts-encounters` 那一批应当整体删掉——在那之前它们是
   `sts-combat-wiring.test.ts`「未迁移编队」那条用例的样本。

### 二、六个新编队的爬升度**一个都没校准**

`ASC_SUPPORTED_ENCOUNTERS` 现在挡着的正是本批这六个（它们的**怪**早在第一 / 二 / 三幕那几批
就置了 `ascCalibrated`，但闸门是**编队级**的）。加上第四十七批那三个，**第四幕 + 事件编队
一共九个编队还是只有 asc0**。这与第三十二~三十九批对第三幕的办法逐字相同，账记在这里：
**它们的爬升度是它自己的一批。**
...
| 项           | 值                                                                                           |
| ------------ | -------------------------------------------------------------------------------------------- |
| 本批文件数   | **6**（各 120 行 = 40 种子 × 3 层，**整份冻结**）                                            |
| 本批体积     | **+12MB**（982MB → **994MB**）——与上一批那句「≈ 12MB」的估算一分不差                         |
| 最大 / 最小  | `colosseum_event_nobs` **2.5MB** / `two_fungi_beasts` **1.3MB**（差 1.9 倍，长度由仗长决定） |
| 本批例数     | **+720**（42585 → **43305**）                                                                |
| **对拍例数** | **43306**（43305 条 + 1 条「trace 数据自身的不变量」）                                       |
| **仓库总计** | **214 个文件 / 994MB / 43305 条 trace**                                                      |
...
| **杜乌娃娃 / 巨铃**                             | `initRelics` :268 / :280，**逐字相同的一句** `buff<STRENGTH>(r.data)`（第四十九批分两个 variant 才分得开） |
| **达摩鲁**                                      | ⚠ **两处**：`initRelics` :260（同步）+ `applyStartOfTurnRelics` Player.cpp:513（**入队**）                 |
| **情绪芯片**                                    | `Player.cpp:518` **一个空的 if**——照抄「什么都不做」，背书是反方向的                                       |
