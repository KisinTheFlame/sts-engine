...
| 怪物 | 59      | 65                | `MOVE_RULES`                                                                              |
| 遗物 | **58**  | 168               | `RELIC_IMMEDIATE` / `RELIC_AT_BATTLE_START` / `RELIC_AT_TURN_START` / `RELIC_OTHER_HOOKS` |
...
9. ⚠⚠ **`stsCombatCoverage` 不查遗物**（第四十一批发现）。它逐条查编队 / 牌 / 药水，
   ...
   ⚠ 补它**不是**纯接线：现在只转写了 20 / 168 颗，一加这道门，绝大多数遗物组合都会被
...
只能往别的方向去：怪物（4 / 65）、遗物（8 / 168，**第四十批起 13 / 168，第四十二批起 20 / 168，第四十三批起 58 / 168**）、药水（13 / 42）、编队（5 / 63），
...
- 测试：
  - `test/sts-combat-trace.test.ts` —— 逐帧对拍（**35626 例**，第四十三批），外加一条**数据自身的不变量**：
...
| 盲区                                             | 关门条件                                                                                                                                                                                              |
| ------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 姜 / 芜菁 / 冠军腰带**与神器的相对顺序**（三条） | 一个**同时带神器来源**的 variant：怪物侧用 `spheric_guardian` / `three_sentries` / `automaton`（带 `ARTIFACT` 的怪），玩家侧登记**发条纪念品**（`atBattleStart` 给 1 层神器）或不钉死药水（古代药水） |
...
- ⚠ **`bloody_idol` 登记了战斗行为，但 `relics.ts` 数据表里没有这个条目。**
  第四十三批复核时用 `isRelicSupported` 逐条比对数据表发现的（TODOS 表里写 58、
  实际能被数据表命中的是 **57**，差的就是它）。

  **两侧状态不一致**：
  - 对拍侧**有背书**（136 例）——harness 直接发遗物 id，`bc.relics` 不经过 `relics.ts`。
  - 真实引擎侧**不可达**——run 层的遗物来自 `relics.ts`，那里没有它，玩家永远拿不到。

  所以它既不是「未验证的实现」（有 136 例），也不是「已完成」（生产环境走不到）。
  这是本项目**第一次**出现这个形状：**预言机侧可达、产品侧不可达**。

  ⚠ 它暴露的是更大的一件事：**`relics.ts` 只有 168 条，而参考的 `RelicId` 有 181 个**，
  缺 13 条。此前从没查过两侧 id 的一一对应——遗物这条线一直在「参考有什么就转写什么」，
  没人回头问「我们的数据表里有没有这一条」。

  **需要裁定的是范围**，两个选项：
  1. 只补 `bloody_idol` 一条（最小改动，但下一批多半会再撞上）。
  2. 做一次**两侧 id 的全表比对**，把缺的 13 条补齐，并加一条数据表测试
     「凡 `isRelicSupported` 为真的 id 必须在 `relics.ts` 里」，把这个形状永久堵死。
     ⚠ 补数据表条目要填名称/稀有度/描述，那是**产品数据不是预言机数据**，
     与「未验证的实现」不冲突，但要确认稀有度之类不会污染奖励池。

  复核方倾向 ②，因为 ① 只是把同一个洞往后推一批。**未拍板，等你定。**
