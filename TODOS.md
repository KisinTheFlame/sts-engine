...
- ⚠ **血之药水 × 神圣树皮的两个常数看着是反的。裁定：不打补丁，但失败的是判据 ①不是③。**

  参考是 `hasBark ? 20 : 40`，而同一个 switch 里另外 **32 条全部是 A = 2B**（树皮翻倍）。
  真实游戏是 20% 基数、树皮翻成 40% —— 参考的**基数与方向都对不上**。

  第四十四批报的是「判据 ③ 不过」（「补上」有 `80:40` 与 `40:20` 两种写法）。
  ③ 确实不过，但**先失败的是 ①，而这个区别决定了将来怎么解锁**：

  - **① 在已登记内容里产生分歧？——不成立。** harness 的策略把喝药水排在出牌之前，
    三瓶在第 1 回合就喝光（第四十一批也量到过同一现象），**恒在满血时喝掉** ⇒
    回复量一次都观察不到。**当前数据里根本没有分歧。**
  - ③ 也不成立（两种写法行为不同）。

  ⚠ **记清楚是哪一条先失败很重要**：
  - **③ 失败**意味着「拿到真机 ground truth 之前永远补不了」（如地精头目 asc18 自锁）。
  - **① 失败**意味着「覆盖面扩大之后它会自己变成可补的」——这一条的关门条件是
    **改 harness 的策略让它在受伤时才喝药水**，那是我们自己能做的事，不需要第二个信源。

  所以正确的关门条件是：① 先改策略让回复量可观察 → ② 那时再看两个常数哪个与数据对得上
  → ③ 若数据仍分不开 `80:40` 与 `40:20`，才轮到真机 ground truth。

- ⚠⚠ **血之药水的神圣树皮分档看着是反的，而且它是全表唯一的例外。裁定：照抄 as-built、
  不打补丁；关门条件不存在（结构性不可观测）。**（第四十四批）
  ...
  **三条判据：③ 不成立 —— 所以不补。**
  ...
  - ① 有可观察的分歧（形状上有），但见下方那条：在本项目的语料里它**观察不到**。
  - ② 有预言机（重新生成即可自洽）。
  ...
  ⚠⚠ **顺带一条结构性的观察：这个分歧在本项目里根本观察不到。** harness 的 `pickAction`
  把喝药水排在出牌**之前**，所以三瓶药一律在第 1 回合、玩家**满血**时喝掉，
  而 `Player::heal` 末尾的 `min(maxHp, …)` 把 40% 与 20% 一起夹成同一个值。
  asc≥6 也不行（90% 上限仍在半血之上）。**要看见它得先改 harness 的策略**，
  那是另一件事。所以这条既不是「补了没背书」，也不是「不补有风险」——它是死的。
