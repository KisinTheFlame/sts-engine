...
| 编队 | 29      | 63                | `ENCOUNTER_BUILDERS` / `ENCOUNTER_SETUP`    |
...
**预言机能背书的范围现在是「第一幕 20 个编队 + 第二幕已追加的那些」**。第一幕那 20 个来自
harness 冻结的 `encounters` 列表（实测走到 25 只怪，含分裂产生的）；第二幕从第二十三批起
走**第二个乘积**，`act2Encounters` 一次列全了 19 个编队、每批只追加一个 filter 到本批的
新 variant（见 WORKFLOW）。第三幕照此再追加第三个乘积。
...
- ⚠⚠ **带壳寄生虫出招里的 `roll2` 是「掷了但取值一定被短路吃掉」的死变量**
  （`MonsterSpecific.cpp:2712-2724`）。第二十五批登记它时确认。参考写的是
  ...
  if (roll < 60 || roll2 < 60) {      // ← roll < 20 蕴含 roll < 60，右边永远不求值
      ...双重打击 / 吸取...
  } else if (!lastTwoMoves(SUCK)) { return SUCK; } else { return DOUBLE_STRIKE; }
  ```
  ...
  **证据链有三条**（与 `usedEntangle` / `escapeNext` / 酸液 L 同族的「死代码」判据）：
  ...
  这与第十八批那个「同一个枚举有两张字符串表、只有一张错」的判据同形；
  ③ 作者之所以要引入第二个变量，是因为 `getMoveForRoll` 的形参是 `const int roll`
  **改不了**（真实游戏那边是直接 `num = AbstractDungeon.aiRng.random(20, 99);` 覆盖同一个数）。

  **裁定（第二十六批执行）：打补丁。** 判据 ③ 是**行为唯一**，不是**文本唯一**——
  这条报告自己已经写清了「行为是唯一确定的，只是文本形状有两种」，那就够了。
  两种写法（`roll2` 初值取 `roll` 后只判 `roll2`、或者三元）**行为逐位相同**：
  `roll < 20 && lastMove(FELL)` 时用那次 [20,99] 重掷去判 60 那道线，其余情况用原 `roll`。
  与地精头目 asc18 那条的差别正在这里——那条的两个候选**产生不同的出招序列**，
  是真的行为二义；这条只是同一行为的两种拼法，选哪种都不改变一个字节的 trace。

  **判据 ③ 的措辞因此收紧为：「行为唯一确定」。** 文本上有几种等价拼法不构成障碍，
  否则任何补丁都能被「还能换个写法」拖住。

  执行要求：① 采用「`roll2` 初值取 `roll`、后面只判 `roll2`」这个形状（最接近真实游戏的
  `num` 覆盖语义，且不引入三元表达式）；② 那次 `aiRng.random(20,99)` 的**掷骰位置与次数
  一个字都不许动**（它已有 80 例背书）；③ 补丁只影响 `shell_parasite` 与
  `shelled_parasite_and_fungi` 两个已冻结文件，走
  `ALLOW_CHANGED="shell_parasite shelled_parasite_and_fungi"`，**必须确认没有别的文件变**；
  ④ 第二十五批在这两个文件上量到的例数（含那 80 与 41）**必须重量**并更新。

  以下是打补丁之前的记录，保留以便追溯：三条判据里 ①② 都过
  （分歧真实存在、`shell_parasite` 就在已装编队里，补丁一定有 trace 走到；本批实测把它
  「修好」会红 **41** 例，而那次掷骰的**次数**本身有 **80** 例背书），
  卡住的是第 ③ 条「修法唯一」的**文本**层面：
  行为是唯一确定的（`roll < 20 && lastMove(FELL)` 时改用那个 [20,99] 的重掷来判 60 那道线），
  但要写成「覆盖 roll」还是「`roll2 === 100 ? roll : roll2`」是两种写法。
  与地精头目 asc18 那条的区别在于：那条**连行为都读不出来**，这条只是文本形状有两种。
  **关门条件**：拿到真实游戏 `ShelledParasite.java` 的 ground truth（一行就够），
  或由项目主人直接裁定采用「覆盖 roll」这个形状。
  ⚠ 在那之前我们与参考一致、与真实游戏很可能不一致，而且影响面不是边角：
  `SHELLED_PARASITE_FELL` 在本批数据里出现 1966 次，每一次之后的下一跳都可能落在这条路上。
...
| 指标                      | 第二十一批               | 第二十二批               | 第二十三批           | 第二十四批 | 第二十五批             |
| ------------------------- | ------------------------ | ------------------------ | -------------------- | ---------- | ---------------------- |
| `test/golden/traces` 总量 | 268MB                    | 285MB                    | 302MB                | 302MB†     | **348MB**              |
| 对拍例数                  | 16861                    | 17581                    | 18706                | 19831      | **20956**              |
| 编队文件数                | 34（20 asc0 + 14 asc19） | 40（20 asc0 + 20 asc19） | 43（+3 第二幕 asc0） | 46         | **49**                 |
| `variant0` 冻结文件       | 15×375 + 14×120 行       | 15×375 + 20×120 行       | 18×375 + 20×120 行   | 21×375+…   | **24×375 + 20×120 行** |
...
每回合末补格挡，仗长得多；多怪编队更贵。按 6~~9MB / 编队算，剩下 10 个第二幕编队
大致 **70~90MB**，装满第二幕之后总量约 420~~440MB。
...
- ⚠ **第十三~二十三批只有两次不是纯追加**（第二十三批是纯追加：3 个新文件、零个 `M`）：第十六批的 `usedEntangle` 补丁改写了
...
### 未装（10 / 19），按「新机制 / 爆炸半径」排的建议顺序

| 建议批次   | 编队                                                              | 新怪                                         | 为什么这么排                                                                                                                                                                     |
| ---------- | ----------------------------------------------------------------- | -------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **二十六** | `sentry_and_sphere` / `cultist_and_chosen` / `three_cultist`      | **0**                                        | 三个编队全是已登记怪的组合，一只新怪都不用写。价值在**多怪局面**：球状守卫者带同伴（尖锐外壳的 `arr[0]` 第一次能分辨）、三只同种怪的 `rollMove` 交错                             |
| **二十七** | `centurion_and_healer`                                            | 百夫长 / 秘法师                              | `heal_ally` / `buff_ally` 两条效果原语第一次有预言机（秘法师给友军加格挡并治疗，无受伤友军则治自己）                                                                             |
| **二十八** | `gremlin_leader` / `slavers` / `book_of_stabbing`（第二幕三精英） | 地精首领 / 工头 / 刺穿之书                   | **召唤**（`summon`）+ `MINION_LEADER` + **`escapeNext` 的写入点**（第十七批记着「参考里没人写它」，真实游戏由地精首领置位——到这一批才判得了要不要打补丁）+ `miscInfo` 的连刺计数 |
| **二十九** | `champ` / `collector` / `automaton`（第二幕三 Boss）              | 冠军 / 收藏家 / 火炬头 / 青铜机器人 / 青铜球 | 召唤 + `STASIS`（把玩家一张牌扣住）+ 怪物侧 `ARTIFACT 3` 的第二个宿主 + **镀甲的第二个宿主**（DECA/DONU 是第三幕，青铜机器人不带；见下）。**体积最大的一批**，排最后             |
| **三十**   | 第二幕的**爬升度**（另开一个 variant，`ascension = 19`）          | 0                                            | 与第二十一/二十二批同构。⚠ 必须等第二幕的怪装完，否则要来回补 `ascCalibrated`                                                                                                    |
