...
| 类别 | 已登记 | 数据表里有               | 登记表                                      |
| ---- | ------ | ------------------------ | ------------------------------------------- |
| 卡牌 | 99     | 359（铁甲 74 / 无色 41） | `CARD_RULES`                                |
| 怪物 | 4      | 227                      | `MOVE_RULES`                                |
...
按登记表逐条转写参考项目的精确行为，范围先限定铁甲 + 无色（16 张待实现）。
...
- 测试：
  - `test/sts-combat-trace.test.ts` —— 逐帧对拍（7155 例）
...
当前 115MB / 7155 例。**最大单个文件 `jaw_worm_horde.jsonl` 48.5MB**
（GitHub 单文件硬上限 100MB）。
...
⚠ **例数是随数据变的**，换 variant 就得重量。下面标 ∮ 的是在**当前布局（7155 例，
variant 9/10 为第九批的 22 张聚焦牌组、只跑 CULTIST/TWO_LOUSE）**上量的；
标 ¶ 的是第八批那版布局（6675 例，
...
- 第九批（出牌队列嵌套）新增的盲区，全部 ∮：
  ...
  - **扣能量读 `card.costForTurn` 还是 `item.energyOnUse`（0 例）。** 普通打牌两者恒等
    （`playCard` 就是拿 `costForTurn` 填的 `energyOnUse`），而自动打出的牌两条都被
    `!autoplay` 挡掉。等 X 费牌（第十批，`energyOnUse` 才真正分岔）登记后会变得可观察。
  - **`playTopCardInDrawPile` 把 `energyOnUse` 设成「当前全部能量」（0 例）。**
    读它的只有 X 费牌（未登记）与复制项（复制项的这个值同样无人读）。
