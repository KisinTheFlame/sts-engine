...
| 编队 | 14      | 63                | `ENCOUNTER_BUILDERS` / `ENCOUNTER_SETUP`    |
...
- 测试：
  - `test/sts-combat-trace.test.ts` —— 逐帧对拍（12556 例），外加一条**数据自身的不变量**：
...
当前 187MB / 12931 例（第十七批 +375 例 = `gremlin_gang` 一个编队 × 375）。
...
⚠ **例数是随数据变的**，换 variant 就得重量。下面标 ⸸ 的是在**当前布局（12931 例 =
12556 + 第十七批 `gremlin_gang` 的 375）**上量的；标 ⁝ 的是第十六批那版布局（12556 例 =
