...
| 编队 | 17      | 63                | `ENCOUNTER_BUILDERS` / `ENCOUNTER_SETUP`    |

⚠ **第十八批之后，第一幕 20 个编队里只剩三个 Boss（守卫者 / 六火幽魂 / 史莱姆王）没登记。**
换句话说 `ENC_V0` 这条路已经走到头了——第十九/二十批做完，再铺量就必须给 harness
**追加一遍第二幕循环**（不能动原列表，见 WORKFLOW）。
...
- ~~**玩家出牌 → 怪物获益的钩子**~~ **第十八批已完成**（`onUseSkillCard`，激怒）。
  ...
  - ⚠ **参考只看 `monsters.arr[0]`**：写死下标 0、也不判死活。地精头目是单怪编队，
    ...
    **关门条件是具体的**：`COLOSSEUM_EVENT_NOBS` = 监工（下标 0）+ 地精头目（下标 1）,
    ...
    ⚠ 同一个 `arr[0]` 写法还有第二个用户：`onUseSkillCard` 里的**尖锐外壳**
    （`BattleContext.cpp:1757`），守卫者带它且是单怪 Boss，同样当前等价、同样记在这里。
    仍缺同族的：只有攻击牌 / 能力牌那两条钩子里的东西（爆发、复制、回响形态、残影、
...
- 测试：
  - `test/sts-combat-trace.test.ts` —— 逐帧对拍（14056 例），外加一条**数据自身的不变量**：
...
当前 207MB / 14056 例（第十八批 +1125 例 = 三个精英编队 × 375）。
...
⚠ **例数是随数据变的**，换 variant 就得重量。下面标 ⸙ 的是在**当前布局（14056 例 =
12931 + 第十八批三个精英编队各 375）**上量的；标 ⸸ 的是第十七批那版布局（12931 例 =
...
⚠ **⁑ / ⁜ / ⁝ / ⸸ 这四批的分母与别的不同**：`ENC_V0` 编队各只有 375 例（只留 variant 0），
