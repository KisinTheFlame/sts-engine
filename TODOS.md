...
| 编队 | 7       | 63                | `ENCOUNTER_BUILDERS` / `ENCOUNTER_SETUP`    |

「卡牌 108 + 1」里的 +1 是**状态牌黏液**（`slimed`，第十三批）。它不算在「铁甲 + 116」那条
铺量线里——状态牌不属于任何角色，也不进任何奖励池。⚠ 它是目前**唯一一条没有 trace 背书**的
登记（打出 0 次，原因见下方「验证方式 · 已知盲区」）。
...
| **分裂**（`largeSlimeSplit`）                             | L 号史莱姆才有，第十四批                                                                                                                                                                                          |
| `Monster::applyEndOfTurnTriggers` 的**易塑**（MALLEABLE） | 史莱姆一族的 Power，但 M/S 号都没有（只有 L 与史莱姆王有）。第十二批已在那个循环里留了 TODO                                                                                                                       |
...
- ~~**怪物的回合末触发**（`Monster::applyEndOfTurnTriggers`）~~ **第十二批已完成**（本批需要的
  ...
  - **仍缺（都留了 TODO）**：金属化 / 镀甲（都要 `Monster::addBlock`）、易塑
    ...
    写了也没有预言机走到——等登记带它们的怪（拉加维林的金属化、史莱姆一族的易塑）时按
    上面的顺序补。⚠ 第十三批登记的是 M / S 号史莱姆，**它们没有易塑**（只有 L 号与
    史莱姆王有），所以这条仍然空着。
- ~~**招式的收尾：下一个意图怎么定**~~ **第十三批已完成**（`MOVE_TURN_END`）。
  在此之前 `doMonsterTurn` 一律 `addToBot(rollMove)`，那只是「绝大多数怪」的形态。
  参考把收尾写在 `Monster::takeTurn` 每条 case 的最后一句，**三种并存**，对 `aiRng` 的消耗
  各不相同（1 / 1 / 0），选错就是 counter 当场对不上：
...
- ~~**怪物往玩家牌堆塞状态牌**~~ **第十三批已完成**（`takeTurn` 的 `add_card` 效果）。
  ...
  `cardRandomRng`），以及 L 号史莱姆的 2 张 / 史莱姆王的 3 张（张数不同、机制相同）。
...
⚠ 代价是**这两个文件不含任何聚焦牌组**，所以「要靠长战斗才走到的东西」在它们上面可能结构性
不可达——第十三批的黏液就是这么变成盲区的（见下方盲区一节）。

当前 156MB / 10305 例（第十三批 +750 例 = 2 个编队 × 375）。
...
⚠ **第十三批是纯追加：现有五个文件逐字节未变。** 本批**没有改 harness**（`trace_dump.cpp`
...
- 第十三批新增的盲区，全部 ⁑：
  - **黏液（`slimed`）整条登记零背书。** 实测：`small_slimes` 里它在**弃牌堆**出现 426 帧、
...
- **`slimed` 黏液的 `cost` 从 `null`（=打不出）改为 `1`**，随登记一起改。
  ...
  ⚠ **这一改没有 trace 背书**（黏液在本批数据里一次都没进过手牌），见「验证方式 · 已知盲区」。
