...
| 怪物 | 59      | 65                | `MOVE_RULES`                                                      |
| 遗物 | 13      | 168               | `RELIC_IMMEDIATE` / `RELIC_AT_BATTLE_START` / `RELIC_OTHER_HOOKS` |
...
只能往别的方向去：怪物（4 / 65）、遗物（8 / 168，**第四十批起 13 / 168**）、药水（13 / 42）、编队（5 / 63），
...
- 数据：`test/golden/traces/<encounter>.jsonl`，每行一条 trace。文件名带后缀时是同一个编队
  ...
  `@relicN`（遗物 / 药水组，第四十批起）。**后缀顺序固定为 `[@ascN][@tgtN][@relicN]`**
- 测试：
  - `test/sts-combat-trace.test.ts` —— 逐帧对拍（**32746 例**，第四十批），外加一条**数据自身的不变量**：
...
| 扰动      | `git status -- test/golden/traces` 恰好 **10 个 `??`、零个 `M`**                                                              |
...
| `energyPerTurn += 1` 改成 `energy += 1`（只加一回合） | **955** | ⚠ 与上一条差 5 例：有 5 条 trace 一回合就打完，两种写法同解                 |
| `largeSlimeSplit` 那两句去掉                          | **221** | `large_slime` 120 + `slime_boss` 的子代再分裂 101                           |
...
| 盲区                                            | 例数 | 分类                                                                                                                                                                                                                                                                                             |
| ----------------------------------------------- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **手钻在 `Monster::damage` 那条路上的那一份**   | 0    | **盲区（分母为 0）**：`@relic1` 的三颗遗物里没有任何**非攻击伤害**来源，22 张牌组也全是攻击牌，而药水（火焰 / 爆炸）被 `pickAction` 在**第 1 回合**就喝光了、那时怪还没加过格挡。⚠ **关门条件很便宜**：把**青铜鳞片**（荆棘走 `Monster::damage`）加进遗物组，配球状守卫者 / 蠕动血块             |
...
| **仓库总计**            | **126 个文件 / 660MB / 32745 条 trace**（第四十批：+10 个 `@relicN` 文件 / +38MB）            |
| **对拍例数**            | **32746**（32745 条 + 1 条「trace 数据自身的不变量」）                                        |
| 文件构成                | 第一幕 asc0 20 + asc19 20；第二幕 asc0 19 + asc19 19；`@tgt1` 23；第三幕 15；**`@relicN` 10** |
...
| ~~⑤~~ | ~~**遗物 / 药水**~~                            | 遗物 8 → 168、药水 13 → 42，是**两条最大的空白战线**              | ⚠⚠ **先解决下面那个设计问题**                                         | ✅ **第四十批已开张**（管线 + 5 个遗物 + 10 份文件 / 38MB），见文末「遗物 / 药水」一节 |
...
| 候选遗物                                                                | 钩子                                                      | 需要什么                                                                                  |
| ----------------------------------------------------------------------- | --------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| **赤备**（`AKABEKO`）                                                   | `initRelics`：`p.buff<PS::VIGOR>(8)`                      | 任何编队；⚠ 要先登记玩家侧 **VIGOR** 这个 Power                                           |
| **硫磺**（`BRIMSTONE`）                                                 | `initRelics`：玩家 +2 力量、**每只可指向的怪 +1**         | 任何编队；⚠ 它的循环**有** `isTargetable()`，与贤者之石那格正相反，两条并排量正好互为背书 |
| **靴子 / 拳套 / 苦无 / 手里剑**                                         | `attackedUnblockedHelper` 顶部 / `onUseAttackCard` 计数   | 已登记的攻击牌足够                                                                        |
| **拆信刀**（`LETTER_OPENER`）                                           | 读 `p.skillsPlayedThisTurn`（引擎里已有字段、当前无读者） | 已登记的技能牌足够                                                                        |
...
⚠ **药水那半（13 / 42）现在也解锁了**：`DeckVariant.potions` 已经在跑（第四十批用它把
`@relic2` / `@relic3` 钉成一对只差一颗御守的 A/B）。铺药水时唯一要记住的实测教训是
**`pickAction` 把喝药水排在出牌之前，三瓶在第 1 回合就全喝光**——想让某瓶药水在特定局面
生效（比如「怪加过格挡之后再打非攻击伤害」）是做不到的，得换别的路子。
