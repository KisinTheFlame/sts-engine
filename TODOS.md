...
| 编队 | 45      | 63                | `ENCOUNTER_BUILDERS` / `ENCOUNTER_SETUP`    |
...
- 测试：
  - `test/sts-combat-trace.test.ts` —— 逐帧对拍（**30466 例**，第三十三批），外加一条**数据自身的不变量**：
...
（40 种子下 ≈2.5~~2.8MB 一个编队，仓库现在是 **581MB / 107 个文件**）。
剩下 10 个编队约 **26MB**，Boss 与精英更耐打、可能到 35~~45MB。总量到 **~620MB**。
