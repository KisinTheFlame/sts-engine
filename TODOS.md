...
| 编队 | 51      | 63                | `ENCOUNTER_BUILDERS` / `ENCOUNTER_SETUP`    |
...
- 测试：
  - `test/sts-combat-trace.test.ts` —— 逐帧对拍（**31186 例**，第三十六批），外加一条**数据自身的不变量**：
...
| ---------- | --------------- | ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **三十七** | `awakened_one`  | 1（觉醒者）      | ⚠ ~~爆炸半径最大的一批~~ **第三十四批已经把 `halfDead` 那条共享路径铺好了**（`CombatMonster.halfDead` + `doMonsterTurn` 的门 + `PowerInstance.cleared` + harness 的 `halfDead` 快照字段），这一批只剩觉醒者自己那条分支要写。它走的是 `Monster::die` 的**第一个分支**（`halfDead = true; removeDebuffs(); removeStatus<CURIOSITY>(); setMove(REBIRTH); cardQueue.clear();`，`Monster.cpp:285-291`）——它排在「判胜 return」**之前**，是那条链上唯一能跳过判胜的路径。另有 `CURIOSITY`（玩家打能力牌时 +力量）与二阶段才上的 `MINION_LEADER`。编队里还有两只邪教徒（已登记） |
| **三十八** | `time_eater`    | 1（时间吞噬者）  | ⚠⚠ **第二大爆炸半径，单独做。** **`TIME_WARP`** 是第一个**改回合结构**的 Power：玩家每打出 12 张牌就 +2 力量并**立即结束玩家回合**，读点在 `useCard` 那条共享路径上。另有 `HASTE`（`curHp = maxHp / 2`，一次性）。⚠ 动 `executeActions` 的回合边界，按 WORKFLOW 的规矩这一批**不叠别的机制**                                                                                                                                                                                                                                                                               |
...
（复仇魔 3.0 + 蜥蜴法师 3.1MB；仓库现在是 **600MB / 113 个文件**）。
剩下 3 个编队按 3~5MB 估约 **12MB**，Boss 更耐打、可能到 18MB。总量到 **~620MB**。
