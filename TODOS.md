...
这道闸门**留着**——第三 / 四幕那 23 只怪一只都没校准，在 `ascension > 0` 下仍然显式抛错。
...
| **`CENTURION_FURY` 的效果与收尾**                    | 二十六          | ⚠⚠ 根是 `pickAction` 恒打 `firstAliveMonster`，百夫长恒在 0 号位。**关门条件是「目标策略」轴**（见「四、下一步 ②」）。意图**选择**另有 8 例背书，两件事分开记。⚠ 第三十批**连它的 asc2 伤害档也一起 0 例**——这一招的效果在任何档位下都没有预言机，爬升度救不了它                                                                                   |
...
| 指标                      | 第二十三批 | 第二十四批 | 第二十五批 | 第二十六批 | 第二十七批 | 第二十八批 | 第二十九批 | **第三十批** |
| ------------------------- | ---------- | ---------- | ---------- | ---------- | ---------- | ---------- | ---------- | ------------ |
| `test/golden/traces` 总量 | 302MB      | 302MB†     | 348MB      | 388MB      | 410MB      | 432MB      | 456MB      | **507MB**    |
| 对拍例数                  | 18706      | 19831      | 20956      | 22456      | 23206      | 23956      | 24706      | **26986**    |
| 编队文件数                | 43         | 46         | 49         | 53         | 55         | 57         | 59         | **78**       |
...
**结论：① → ② → ③ → ④。** ① 与 ② 都只在 `DeckVariant` 上加字段（爬升度那个第二十一批就装好了、
目标策略那个还没做），互不相干，**但不要在同一批里做**——两条轴同时加会让「哪一条把数据改坏了」
无法定位。
⚠ ① 现在的形态是「补两个**中间**档位」（`asc7 + asc16`，跨两幕），实现侧零改动、纯数据；
② 要写 harness 的 30 行。**如果只想要「关掉最多盲区 / 每 MB」，② 反而更划算**
（10~~30MB 关掉一整族「同伴先死」的盲区），而 ① 是 180~~200MB 关掉 12 条中间档 + 两条阈值。
按「先把已有的轴用尽」的惯例仍然排 ① 在前，但这个权衡要写明。
