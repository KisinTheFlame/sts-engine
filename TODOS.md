...
| 类别 | 已登记 | 数据表里有               | 登记表                                      |
| ---- | ------ | ------------------------ | ------------------------------------------- |
| 卡牌 | 95     | 359（铁甲 74 / 无色 41） | `CARD_RULES`                                |
| 怪物 | 4      | 227                      | `MOVE_RULES`                                |
...
按登记表逐条转写参考项目的精确行为，范围先限定铁甲 + 无色（20 张待实现）。

选牌屏、牌生命周期、玩家事件钩子、卡牌实例级状态、随机卡池取牌那五批已做（见下方第三项）。
剩下卡在这些子系统上：
...
| 卡                     | 为什么跳过                                                                                                                                                                                            |
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `transmutation` 嬗变   | 它要卡池（已有了），但同时是 **X 费**牌（`getEnergyCost` 返回 -1，要读 `energyOnUse`）。X 费整套排在第十批，本批不顺手做——顺手做等于把第十批的机制提前塞进来，而 X 费还牵扯化学 X、旋风斩、劈砍那一族 |
...
- ~~**选牌屏**（card select screen）~~ **已完成**：`inputState` 加 `card_select`、
  ...
  尚缺的 task：`codex` / `dual_wield` / `forethought` / `gamble` / `hologram` /
...
- ~~**战斗内卡池**：从卡池随机取一张牌**定义**凭空造牌~~ **已完成**（第八批）。
  ...
  - `CombatColorlessCardPool`（34 张无色牌）—— 多面手取它
    ...
    仍缺：X 费的嬗变（第十批）、法典 / 混沌 / 各类「发现」药水（都走同一套取样）
...
- 测试：
  - `test/sts-combat-trace.test.ts` —— 逐帧对拍（6675 例）
...
| 6       | 同上                                        | 前 40  | 是   |
| 7       | 起始 + 第八批 + 3 张使能牌（18 张，聚焦）   | 前 40  | 否   |
| 8       | 同上                                        | 前 40  | 是   |

当前 111MB / 6675 例。**最大单个文件 `jaw_worm_horde.jsonl` 48.4MB**
（GitHub 单文件硬上限 100MB）。第八批的这一对 variant 单独占 9.5MB——比之前每批的
~8MB 还大一点，因为蜕变/变形每打一次就往抽牌堆塞 3(5) 张、后续每一帧快照都跟着变长。
按这个速度**再两批就会撞上 100MB**，届时必须换布局（拆文件、或砍掉部分 variant），
不能再靠「每批新开一对」。
...
⚠ **例数是随数据变的**，换 variant 就得重量。下面标 ¶ 的是在**当前布局（6675 例，
variant 7/8 为第八批的 18 张聚焦牌组）**上量的；标 § 的是第七批那版布局（5475 例，
...
- 第七批（卡牌实例级状态）新增的盲区，全部 §：
  ...
  - **`setCostForTurn` 的 `costForTurn >= 0` 门（§0，¶仍 0）。** 只有 X 费牌（-1）与打不出的
    状态/诅咒牌（-2）会命中它。第八批之后 X 费的**嬗变**真的会被多面手/发现造出来躺在手里
    （腐化的钩子也会作用到它），但它未登记、harness 不会打它，于是那个 -1 变没变没人读。
