...
| 编队 | 52      | 63                | `ENCOUNTER_BUILDERS` / `ENCOUNTER_SETUP`    |
...
（复仇魔 3.0 + 蜥蜴法师 3.1MB）、第三十七批 **7.4MB / 1 编队**（⚠ **一个编队顶别人两个**
——强牌组把仗从 3.6 回合拉到 8.7 回合，仓库现在是 **608MB / 114 个文件**）。
剩下 2 个编队按同样的道理都不便宜（时间吞噬者 456 血、迪卡+多努两只带神器）：
按 **6~8MB / 编队**估约 **12~16MB**，总量到 **~625MB**。
⚠ 第三幕的爬升度是**另一批**（`ascCalibrated` 一只都没置），成本按第三十批实测的
2.7MB / 编队 / 档位算，16 编队一档约 43MB。

### ⚠⚠ 牌组不是常量：第三十七批第一次为「让新代码被走到」换牌组
