...
**只有 asc0 这一个档位**、**第三幕一个都没有**。做法与两步验证见 WORKFLOW「目标策略这条轴」。
⚠ 三维目前是**部分笛卡尔积**：`{asc0, asc19} × 54 编队` 全满（第四十六批），`tgt1` 只与
asc0 配过、而且只覆盖前两幕。缺的两格各自守着一批盲区：
`asc19 × tgt1`（23 个组合）是「百夫长狂怒连斩 asc2 伤害档」现在唯一的关门条件；
`第三幕 × tgt1`（15 个组合）是「迪卡守护方阵的两条 `noAliveGate`」与
「`MINION_LEADER` 在蜥蜴法师 / 觉醒者身上的判胜路径」的关门条件（见文末盲区表）。
...
| **迪卡守护方阵的 `noAliveGate`**（给已死的多努加格挡 / 加镀甲） | 三十九 / 四十六 | **`donu_and_deca@tgt1`**（打下标最大的活怪 = 多努）。⚠ 第四十六批**在 asc19 语料上重量过，仍然 0 例**——爬升度这条轴救不了它（卡的是「场上局面」不是「回合数」）。⚠ 本批新加的**镀甲那一半**同源，两条合并成这一条                                      |
...
| ⑥     | **第三幕的 `@tgt1`**（15 编队 × 目标策略）     | 本幕**现在唯一剩下的整块结构性缺口**：迪卡守护方阵的两条 `noAliveGate`、`MINION_LEADER` 在蜥蜴法师 / 觉醒者身上的判胜路径 | 数据按 asc0 那 15 份估 ≈ **56MB**（多怪编队才有意义，实际会少一些） | **零**：实现侧一行不改（目标逐字记在 `steps[].action.target` 里）                                                                                                            |
