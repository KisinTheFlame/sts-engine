...
✅ **第一幕铺量到此为止。** 20 个编队全部装完，`ENC_V0` 里再没有可加的名字；
下一条战线（爬升度 / 第二幕 / 遗物 / 药水）的输入见文末「第一幕完成度小结」。
...
- 测试：
  - `test/sts-combat-trace.test.ts` —— 逐帧对拍（14806 例），外加一条**数据自身的不变量**：
...
#### （丙）**最大的一块结构性盲区不在上表里：爬升度**

每只怪都有 asc2 / asc3 / asc4 的伤害档、asc7（精英 8 / Boss 9）的血量档、asc17~~19 的出招与
数值档，**全部是死代码**——harness 固定 `ascension = 0`。第一幕铺完 25 只怪之后，
这已经是项目里最大的单块盲区，而且**随每批线性增长**。
做法与代价见「待裁定 · 要不要给 harness 开爬升度这条轴」。
⚠ 顺带：`enemies.ts` 的 `EnemyDef` 只装得下**一组**血量区间，所以当前引擎
**只在 asc0~~6 下血量正确**——那是类型改动，同样在「待裁定」里。

### 三、数据体积与例数（收官数字）

| 指标                      | 第十九批之后 | 第二十批之后    |
| ------------------------- | ------------ | --------------- |
| `test/golden/traces` 总量 | 236MB        | **247MB**       |
| 对拍例数                  | 14806        | **15181**       |
| 编队文件数                | 19           | **20**          |
| `variant0` 冻结文件       | 14 × 375 行  | **15 × 375 行** |
...
- ⚠ **第十三~二十批全部是纯追加**（唯一的例外是第十六批的 `usedEntangle` 补丁改写了
  `red_slaver` / `exordium_thugs` 两个文件）。所以这 15 个文件在 git 里各只有一份 blob。

### 四、下一步的三个候选（按「解锁量 / 爆炸半径」排）

1. **爬升度这条轴**——不需要动 `encounters` 列表（新 variant 追加在末尾即可，零扰动），
   一次点亮全部 25 只怪的 asc 分支。代价约 +100MB（全编队 asc17）或更少（只挑有分支的）。
   ⚠ 但它要连玩家侧一起对拍（起始血量、asc10 起的 `ascenders_bane`）。
