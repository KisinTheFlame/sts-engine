...
| 编队 | 53      | 63                | `ENCOUNTER_BUILDERS` / `ENCOUNTER_SETUP`    |
...
⚠ **「未迁移编队」的测试样本本批被自己顶掉**（`giant_head`），换成 `donu_deca`。
...
| 跳过的                                   | 为什么跳过                                                                                                                                                                                                                                                                                                                                                                                                              |
| ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Monster::die` 里**其余五条死亡触发**    | ~~停滞（`STASIS`，还牌）~~ **第二十八批已补**；~~暗影客的重生（`REGROW`）~~ **第三十四批已补**（那条 else-if 链的三格现在全齐：孢子云 → 重生 → 停滞）。~~觉醒者的假死~~ **第三十七批已补**（`Monster::die` 的**第一个**分支，⚠ 它排在判胜 `return` **之前**，不在那条 else-if 链上）。仍缺：尸爆（`CORPSE_EXPLOSION`，静默的牌）、地精之角 / 活体样本（两个遗物，harness 的遗物轮换里都没有）。`monsterDie` 里留了 TODO |
...
- 测试：
  - `test/sts-combat-trace.test.ts` —— 逐帧对拍（**31426 例**，第三十八批），外加一条**数据自身的不变量**：
...
| 匕首自爆去掉 `suicide`           | **113** |                                        |
| 匕首自爆 `triggerRelics` → false |   **0** | **探针无效（结构性）**，见下方 0 例表  |
...
| 匕首自爆 `triggerRelics` → false                    | **0** | **探针无效（结构性）**：两支的差别只在死亡链（亡语 / 地精角 / 活体样本）与扣格挡，而匕首**没有亡语**、**从不获得格挡**，harness 的八个遗物轮换里也没有那两个。与贤者之石那几条同族                                                                                                                                                   |
...
| 匕首自爆的 `triggerRelics` 这一位                                                                                   | 0    | **探针无效（结构性）**：匕首没有亡语、从不获得格挡，harness 的八个遗物轮换里也没有地精角 / 活体样本                                                                         |
| `MINION_LEADER` 在蜥蜴法师身上的判胜路径                                                                            | 0    | **盲区（分母为 0）**：法师 180~190 血，这副 22 张牌组 120 / 120 都打不死它。它的背书全部来自地精首领（第二十七批 4 例、第三十一批 `@tgt1` 21 例）                           |
| 贤者之石给召唤出来的匕首 +1 力量                                                                                    | 0    | **结构性盲区（未实现，留 TODO）**：那个遗物不在 harness 的八个轮换里，与前三条召唤路径里同一支同族                                                                          |
...
| 植入那条 `if (!hasRelic<OMAMORI>()) { if (hasRelic<DARKSTONE_PERIAPT>()) increaseMaxHp(6); }`              | 0    | **结构性盲区（未实现，留 TODO）**：两个遗物都不在 harness 的八个轮换里，与贤者之石那几条同族                                           |
...
| 贤者之石给复活的暗影客 +1 力量（`MonsterSpecific.cpp:1494-1496`）          | 0    | **结构性盲区（未实现，留 TODO）**：harness 的遗物轮换里没有那颗（八个里没有），与第十四批分裂那两条同族，一起等「遗物轮换扩容」那一批 |
...
⚠⚠ **第三幕做完之前，不许在这个乘积后面再挂任何新乘积。** 每一批第三幕都要往
`act3Variants` 里追加一个 variant，而往「不是最后一个」的乘积里追加会平移其后所有
`traceIdx`。这与第二幕当年的处境相同，但第二幕那时它本来就是最后一个——现在这条是
**主动的约束**，排批次时先认下来（详见 WORKFLOW「新轴必须挂成新的乘积」）。
⚠ 推论：「⑤ `asc19 × tgt1` 那一格」与「① `asc7 + asc16` 中间档」这两个候选，
**在第三幕装完之前都做不了**（它们都要新乘积）。要么先把第三幕装满，要么接受
第三幕语料重排——后者代价是本批开始的全部第三幕文件作废，不划算。
...
⚠ 下表按「解锁量 / 爆炸半径」排，**爆炸半径大的（改共享路径的）一个机制一批**。
每一行的「新怪」都要做四份工作（`enemies.ts` 数值 / `MOVE_RULES` / `takeTurn` 的 effects /
收尾），外加回 `MonsterMoves.h` 逐条抄 `isMoveAttack` 白名单。

| 建议批次   | 编队            | 新怪             | 为什么这么排 / 主要机制                                                                                                                                                                                                                                                                                   |
| ---------- | --------------- | ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **三十九** | `donu_and_deca` | 2（迪卡 / 多努） | 第三幕最后一个 Boss，两只**都带 `ARTIFACT`**（asc19 是 3 层）。机制上全是已有原语的新宿主（力量 / 神器 / 群体伤害），是收官批。装完第三幕 16 / 16。⚠⚠ **它同时是当下「未迁移编队」那两条测试用例的样本**（第三十五批从 `giant_head` 换过来的），做这一批之前先换掉——判据与换法见 WORKFLOW「附：踩过的坑」 |
...
（⚠ 两个 Boss 都是「一个编队顶别人两个」——强牌组把仗从 3.6 拉到 7~9 回合，
仓库现在是 **614MB / 115 个文件**）。
剩下 1 个编队（迪卡 + 多努，两只带神器）按同样的道理不便宜，按 **6~8MB** 估，总量到 **~622MB**。
...
| 贤者之石给复活的暗影客 +1 力量                                             | 0    | **结构性盲区（未实现，留 TODO）**：harness 的遗物轮换八个里没有它，与第十四批分裂那两条同族                                                           |
...
| 贤者之石给召唤出来的匕首 +1 力量                                                                                 | 0    | **结构性盲区（未实现，留 TODO）**：遗物不在 harness 的八个轮换里，与前三条召唤路径里同一支同族                                                                      |
...
| 植入那条遗物分支（御守 / 暗石护符 → `increaseMaxHp(6)`）                                               | 0    | **结构性盲区（未实现，留 TODO）**：两个遗物都不在 harness 的八个轮换里，与贤者之石那几条同族                                                                                                                    |
