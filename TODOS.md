...
| 编队 | 47      | 63                | `ENCOUNTER_BUILDERS` / `ENCOUNTER_SETUP`    |
...
- ~~**怪物的回合末触发**（`Monster::applyEndOfTurnTriggers`）~~ **第十二批已完成**（本批需要的
  ...
  它全参考项目唯一的宿主）——在那之前「后半早就转写好了」这句话是不准确的，缓慢与锁定
  至今仍是 TODO（当前登记的怪一只都不带）。
...
- 测试：
  - `test/sts-combat-trace.test.ts` —— 逐帧对拍（**30706 例**，第三十四批），外加一条**数据自身的不变量**：
...
不能再按「2.5MB 一个编队」估；仓库现在是 **589MB / 109 个文件**）。
剩下 8 个编队按 3~5MB 估约 **30MB**，Boss 与精英更耐打、可能到 40MB。总量到 **~630MB**。
