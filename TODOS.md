...
| 编队 | 19      | 63                | `ENCOUNTER_BUILDERS` / `ENCOUNTER_SETUP`    |

⚠ **第十九批之后，第一幕 20 个编队里只剩六火幽魂（`hexaghost`）一个没登记。**
换句话说 `ENC_V0` 这条路只剩最后一批——第二十批做完，再铺量就必须给 harness
...
当前 236MB / 14806 例（第十九批 +750 例 = 两个 Boss 编队 × 375；
两个新文件 12MB / 18MB 是 `ENC_V0` 里最大的两个——Boss 血厚仗长，单条 trace 的步数是精英的两三倍）。
...
- **怪物攻击的 `clearOnCombatVictory` 抄成了默认的 `true`，参考是 `false`**（第十九批修）。
  ...
  **恒是它自己那组动作里的第一个**，永远轮不到「胜利之后被清扫」。
  ...
  ⚠ 同族还有一处**尚未修**：灼伤的 `addToTop(DamagePlayer(2, true))` 也该是 `false`
  （`Actions.cpp:91-95` 同一行形状）。当前同样看不见（回合末结算时它也是第一条），
  按「补丁跟着登记一起打」留到有数据能分辨它的那一批。
