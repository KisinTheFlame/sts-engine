...
| 编队 | 12      | 63                | `ENCOUNTER_BUILDERS` / `ENCOUNTER_SETUP`    |
...
- ~~**招式的收尾：下一个意图怎么定**~~ **第十三批已完成**（`MOVE_TURN_END`），
  **第十四批补了第四形态 `none`**（case 尾部什么都没有——分裂就是这样，收尾在
  ...
  - **任意函数** —— 第十五批新增的**第五形态**。抢劫者的抢劫收尾是
    ...
    （冠军的阶段位；⚠ 红奴隶主的「用过纠缠没」在参考里是死的，见第十五批那节）。
...
- ~~**编队成员由 miscRng 掷定**~~ **第十三批补齐了两种新形态**（`ENCOUNTER_BUILDERS`），
  ...
  - **「构造全部再选一个」**（`exordium_thugs`，`MonsterGroup.cpp:477` / `:497`）——
    **第十五批已完成**（`createFromConstructedPool`）。三条逐位对齐点：
    ...
    `createStrongWildlife`（同一套「构造全部再选一」，但只有 2 个候选、
    且 `construct` 传的 idx 恒为 0）。
- ~~**逃跑**（`isEscapingB`）~~ **第十五批已完成**（`monsterEscape`）。
  ...
  `escapeNext`（意图渲染用）、以及战斗后的 `g.info.stolenGold` 还钱（run 层第 7 项）。
- ~~**纠缠**（ENTANGLED）~~ **第十五批已完成**。两处：`cardCanUse` 的 ATTACK 分支加一道门
  ...
  ⚠ 是**整条清除**不是递减——红奴隶主连放两次缠绕也只封住一个玩家回合。
  ⚠ 那道**门本身没有预言机**（重放只照着已记录的动作走，放宽一条限制永远不会让重放分岔），
  见下方盲区。有背书的是「层数落在玩家身上」与「回合末清除」。
...
- 测试：
  - `test/sts-combat-trace.test.ts` —— 逐帧对拍（12180 例），外加一条**数据自身的不变量**：
...
当前 177MB / 12180 例（第十五批 +1500 例 = 4 个编队 × 375）。
...
⚠ **例数是随数据变的**，换 variant 就得重量。下面标 ⁜ 的是在**当前布局（12180 例 =
10680 + 第十五批四个 `ENC_V0` 编队各 375）**上量的；标 ⁑ 的是第十三批那版布局（10305 例 =
...
全部例数照旧成立。
⚠ **⁑ / ⁜ 这两批的分母与别的不同**：`ENC_V0` 编队各只有 375 例（只留 variant 0），
所以**一只怪的可观测面最多就是它出现的那几个编队 × 375**（史莱姆那几只 750、
蓝/红奴隶主 750、抢劫者 750）。这两批的个位数（如 roll 阈值那几条）**不代表薄得像盲区**，
...
- ⚠ **红色奴隶主的「缠绕一场只能用一次」在参考里根本没生效，第十五批照抄了参考。**
  `getMoveForRoll` 开头是 `const bool usedEntangle = miscInfo;`（`MonsterSpecific.cpp:2777`），
  可是**全项目没有任何地方给红色奴隶主写过 `miscInfo`**——`RED_SLAVER_ENTANGLE` 那条 case
  只有 `addToBot(DebuffPlayer<ENTANGLED>(1))` + `addToBot(RollMove(idx))` 两句（`:1017`）。
  于是 `usedEntangle` 恒为 false，后果两条：
  ① `roll >= 75 && !usedEntangle` 退化成 `roll >= 75`，缠绕**可以反复放**（实测 375 条
  `red_slaver` 的 trace 里有一条放了 8 次，`ENTANGLE` 总执行 72 次）；
  ② `roll >= 50 && usedEntangle && !lastTwoMoves(STAB)` 那一整段是**死代码**。
  形状与已打过补丁的 `IMPATIENCE`（`hasAttack = false` 应为 `true`）同族：参考自己声明了
  这个语义（变量就叫 `usedEntangle`）却漏写了赋值，真实游戏的 `SlaverRed` 在 takeTurn 的
  缠绕分支里置 `usedEntangle = true`。
  **没有自行拍板打补丁的理由**：修它不只是补一句 `miscInfo = true`——那会让第二段
  `roll >= 50` 从死代码变成活代码，而那个阈值本身也需要对着真实游戏裁定（疑似 55）。
  「补一句让死分支活过来、但活过来之后的阈值没人验证」比照抄更危险。
  **影响面**：补丁只会改 `red_slaver` / `exordium_thugs` 两个文件（第一幕的 20 个编队里
  只有这两个有红奴隶主），**不碰任何已冻结的老数据**，所以裁定之后随时可以打。

  **裁定（第十六批执行）：打补丁。** 理由是「哪边离真实游戏更近」，不是「哪边更保险」：

  - 不打 = **确定错**。缠绕反复放，实测一条 trace 放了 8 次，分歧覆盖大量对局。
  - 打 = 标志位确定对，只是让 `roll >= 50` 这个**没人验证过**的阈值从死代码变成活代码。
    真实游戏若是 55，分歧只落在 roll ∈ [50,55) 那 5% 上。**打补丁严格减少期望分歧。**
  - 「活过来之后的阈值没人验证」这条反对意见，对**参考里的每一个阈值**都成立——
    我们从头到尾都在信任参考的数值。这里唯一的区别是我们**已经确知**另一半是错的。
    只修确知错的那半、保留仅仅无法独立验证的那半，正是前 11 个补丁一贯的打法。
  - 证据强度：参考声明了变量、读了它、从不写它。这是**移植时漏抄赋值**的典型签名，
    与 `IMPATIENCE` / `DISARM` / `TRIP` 同族，那一族至今 11 次判断全部成立。

  执行要求：① 阈值 50 在补丁注释里**显式标注为未验证**；② 走
  `ALLOW_CHANGED="red_slaver exordium_thugs"`；③ 第十五批在这两个文件上量到的
  缠绕相关例数（41 / 41 / 35 / 72）**必须重量**并更新。
