...
| 编队 | 23      | 63                | `ENCOUNTER_BUILDERS` / `ENCOUNTER_SETUP`    |
...
⚠⚠ **这次改动没有预言机**：`isMonsterAttacking` 唯一的读者是**觅敌之弱**与**瞄准眼睛**，
两张牌都在卡牌批次的聚焦 variant 里，从没与任何怪物编队同场。实测三个方向
（去掉硬化 / 多加汲取 / 整表退回读 `intent`）**各 0 例**。这不是「改坏了没人管」，
而是这条谓词**整族**都还没有背书（盲区表里那 17 条），关门条件仍是「让觅敌之弱与怪物同场」。
...
| 诅咒 / 困惑的 **bool 语义**（置 1 还是累加）                   | 二十三    | 参考里**没有任何内容会施加第二次**：选民的诅咒只在第二个怪物回合出（asc17 是首回合），史尼克的惑目只在首回合。第一次施加时两种写法结果相同 |
...
| 指标                      | 第二十批  | 第二十一批               | 第二十二批               | 第二十三批               |
| ------------------------- | --------- | ------------------------ | ------------------------ | ------------------------ |
| `test/golden/traces` 总量 | 247MB     | 268MB                    | 285MB                    | **302MB**                |
| 对拍例数                  | 15181     | 16861                    | 17581                    | **18706**                |
| 编队文件数                | 20        | 34（20 asc0 + 14 asc19） | 40（20 asc0 + 20 asc19） | **43**（+3 第二幕 asc0） |
| `variant0` 冻结文件       | 15×375 行 | 15×375 + 14×120 行       | 15×375 + 20×120 行       | **18×375 + 20×120 行**   |
...
### 未装（16 / 19），按「新机制 / 爆炸半径」排的建议顺序

| 建议批次   | 编队                                                                                                             | 新怪                                         | 为什么这么排                                                                                                                                                                         |
| ---------- | ---------------------------------------------------------------------------------------------------------------- | -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **二十四** | `shell_parasite` / `three_byrds`                                                                                 | 带壳寄生虫 / 鸟                              | 补齐 `attackedUnblockedHelper` 那条 else-if 链剩下的两格：**镀甲**（破甲即改出 `SHELLED_PARASITE_STUNNED`）与**飞行**（层数归零改出 `BYRD_STUNNED`）。两只都带「被打断就换招」的形状 |
| **二十五** | `two_thieves` / `snecko`                                                                                         | 劫匪 / 史尼克                                | 劫匪 = **抢劫者逃跑收尾 `"none"`** 那条盲区的关门条件（同伴还活着时逃跑）+ 偷金钳制；史尼克带**困惑**（与 HEX 同族的纯 bool，`randomize_hand_costs` 那条路第一次有预言机）           |
| **二十六** | `chosen_and_byrds` / `sentry_and_sphere` / `cultist_and_chosen` / `three_cultist` / `shelled_parasite_and_fungi` | **0**                                        | 五个编队全是已登记怪的组合，一只新怪都不用写。价值在**多怪局面**：孢子云的亡语、球状守卫者带同伴、`GainBlockRandomEnemy` 的非空支                                                    |
...
| **二十九** | `champ` / `collector` / `automaton`（第二幕三 Boss）                                                             | 冠军 / 收藏家 / 火炬头 / 青铜机器人 / 青铜球 | 召唤 + `STASIS`（把玩家一张牌扣住）+ 怪物侧 `ARTIFACT 3` 的第二个宿主。**体积最大的一批**，排最后                                                                                    |
...
| 盲区                              | 关门的那一批 | 怎么关                                                                                                                                                |
| --------------------------------- | ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| 抢劫者逃跑的收尾 `"none"`         | 二十五       | `two_thieves` 里劫匪有同伴，逃跑不会当场判胜                                                                                                          |
| 偷金的 `min(玩家金币, 额度)` 钳制 | 二十五       | 两只劫匪各 15~20，够把玩家偷穷                                                                                                                        |
| 孢子云的 `addToTop` ↔ `addToBot`  | 二十六       | `shelled_parasite_and_fungi` 里真菌兽有同伴，亡语真的会跑                                                                                             |
