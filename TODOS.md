...
| 编队 | 35      | 63                | `ENCOUNTER_BUILDERS` / `ENCOUNTER_SETUP`    |
...
| **工头那次「白掷」的区间**（`hpDiscardRoll` 的上下界）                                                      | 二十七        | ⚠ **结构性不可观测，连「盲区」都算不上**：取值被丢弃，而 `Random::nextLong(n)` 的实际前进步数**与 n 无关**（rejection 循环几乎必然只转一次），所以上下界写什么都逐位等价。**次数**有背书（去掉整条红 375 例），区间没有、也不可能有。与第四 / 十五批那两处「白吃 RNG 的 bound」同族                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| **二十九** | `champ` / `collector` / `automaton`（第二幕三 Boss）     | 冠军 / 收藏家 / 火炬头 / 青铜机器人 / 青铜球 | **召唤的第二 / 第三族**（`SpawnTorchHeads` 按 `3 - monstersAlive` 决定召几只、**要 `initHp`**；`SummonOrbs`）+ `STASIS`（把玩家一张牌扣住）+ `MINION_LEADER` 的第二 / 三个宿主 + 怪物侧 `ARTIFACT 3` 的第二个宿主。**体积最大的一批**，排最后 |
