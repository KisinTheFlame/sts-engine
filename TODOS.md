...
| 类别 | 已登记 | 数据表里有               | 登记表                                      |
| ---- | ------ | ------------------------ | ------------------------------------------- |
| 卡牌 | 106    | 359（铁甲 74 / 无色 41） | `CARD_RULES`                                |
| 怪物 | 4      | 227                      | `MOVE_RULES`                                |
...
按登记表逐条转写参考项目的精确行为，范围先限定铁甲 + 无色（**9 张待实现**）。

第十一批之后，红 + 无色剩下的 9 张是：`dark_shackles` / `violence`（第十二批，两张都要先在
参考侧修 bug）、`forethought`（参考的升级分支整段被注释掉，补它是发明而非转写），以及参考
**压根没实现**的 6 张 `shiv` / `smite` / `through_violence` / `insight` / `miracle` / `safety`
（三个 switch 里都没 case，永远不会有预言机 —— `seek` 是第 7 张，见文末）。
...
第十一批（四个小机制）点名跳过的：**没有**。四张全部登记，红 + 无色因此只剩上面那 9 张。
...
- ~~**战斗内卡池**：从卡池随机取一张牌**定义**凭空造牌~~ **已完成**（第八批）。
  ...
  - `CombatColorlessCardPool`（34 张无色牌）—— 多面手取它
    ...
    各登记掉 4 / 3 / 4 张，**现在只剩 7 张**（`dark_shackles` / `violence` / `forethought` /
...
- 测试：
  - `test/sts-combat-trace.test.ts` —— 逐帧对拍（9075 例）
...
当前 137MB / 9075 例。**最大单个文件 `jaw_worm_horde.jsonl` 48,469,656 字节（46MB）**
——第十一批新加的四个 variant 都带编队过滤、不跑三颚虫，所以它**没有变大**（反而小了
86KB：variants 7/8 与 13/14 的内容变了，见下）。GitHub 单文件硬上限 100MB。
长的两个是 `cultist.jsonl` 32,784,722 与 `two_louse.jsonl` 27,385,025。
...
⚠ **例数是随数据变的**，换 variant 就得重量。下面标 ※ 的是在**当前布局（9075 例，
variants 15-18 为第十一批的两对聚焦牌组）**上量的；标 ∬ 的是第十批那版布局（8115 例，
