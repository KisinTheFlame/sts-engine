...
| 编队 | 42      | 63                | `ENCOUNTER_BUILDERS` / `ENCOUNTER_SETUP`    |
...
（第三十二批登记的三只形状怪也在其中：尖刺客的荆棘 `{3,4,7}` 三档一条都没有预言机）。
...
3 层 = 360 例 / 8.0MB，**零扰动**（`git status` 只有 3 个 `??`）。
...
| ---------- | ------------------------------------- | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **三十三** | `orb_walker` / `spire_growth` / `maw` | 3（暗球游荡者 / 尖塔增生 / 大嘴） | **三只单怪、机制互不重叠**，是最便宜的一批。带进来的都是**已有原语的新宿主**：`hpDiscardRoll`（暗球游荡者，参考唯一注了 `// first call is discarded by game` 的那只）、`hpNoRoll` 的第二个宿主（大嘴）、`GENERIC_STRENGTH_UP`（暗球游荡者的开局 Power）、往牌堆塞**灼伤**（第二十批六火幽魂已打通）。⚠ 尖塔增生的**束缚 CHOKED** 是新 Power，但它只是个数值                                                                                                                     |
| **三十四** | `transient` / `three_darklings`       | 2（无常 / 暗黑之种）              | 两条**新的死亡/回合边界**机制，而且互不干扰：**`FADING`**（无常，回合末层数递减、归零自杀）+ **`SHIFTING`**（受击时把伤害转成 −力量 与 SHACKLED，`attackedUnblockedHelper` 链的**最后一格**）+ **`REGROW`**（暗黑之种，`Monster::die` else-if 链的**第二格**——第二十八批装停滞时特意留的那个「中间格」，装它才能把链的形状钉死）。⚠ 三只暗黑之种互为同伴，重生天然可观察                                                                                                        |
| **三十五** | `writhing_mass`                       | 1（蠕动血块）                     | **`REACTIVE`**：`attackedUnblockedHelper` 里与易塑**共用同一格**（`else if (MALLEABLE \|\| REACTIVE)`，进去之后两个 if 各判各的），而且它排一条 **`Actions::ReactiveRollMove`**——本项目第一条「挨打就重滚意图」。⚠ 蠕动血块同时带易塑与反应，正是把那一格的形状钉死的唯一宿主。单怪编队、体积小                                                                                                                                                                                 |
| **三十六** | `giant_head` / `nemesis`              | 2（巨头 / 复仇魔）                | 两个第三幕精英。**`SLOW`**（巨头：玩家每打一张牌加一层，伤害按层数放大）+ **怪物侧 `INTANGIBLE`**（复仇魔隔回合无敌，`Monster::attacked` / `damage` 的入口各有一处钳制）。⚠⚠ **装 `giant_head` 之前先把 `sts-combat-wiring.test.ts` / `sts-combat-rules.test.ts` 那个「未迁移编队」样本换掉**（现在用的就是它），否则那两条用例会红                                                                                                                                             |
...
| **三十八** | `awakened_one`                        | 1（觉醒者）                       | ⚠⚠ **爆炸半径最大的一批，单独做。** `isDeadOrEscaped` 的第三位 **`isHalfDead()`** 至今零背书，而觉醒者的假死走的正是 `Monster::die` 的**第一个分支**（`halfDead = true; removeDebuffs(); removeStatus<CURIOSITY>(); setMove(REBIRTH); cardQueue.clear();`，`Monster.cpp:285-291`）——它排在「判胜 return」**之前**，是那条链上唯一能跳过判胜的路径。另有 `CURIOSITY`（玩家打能力牌时 +力量）与二阶段才上的 `MINION_LEADER`。编队里还有两只邪教徒（已登记）                       |
...
| **四十**   | `donu_and_deca`                       | 2（迪卡 / 多努）                  | 第三幕最后一个 Boss，两只**都带 `ARTIFACT`**（asc19 是 3 层）。机制上全是已有原语的新宿主（力量 / 神器 / 群体伤害），是收官批。装完第三幕 16 / 16                                                                                                                                                                                                                                                                                                                               |

⚠ **体积估算**：按第三十二批实测 **8.0MB / 3 编队 / 40 种子**（≈2.7MB 一个编队）算，
剩下 13 个编队约 **35MB**，Boss 与精英更耐打、可能到 45~55MB。总量从 574MB 到 **~620MB**。
