...
| 编队 | **57**  | 63                | `ENCOUNTER_BUILDERS` / `ENCOUNTER_SETUP`                                                                                         |
...
⚠ 「编队」那一栏还剩 **6 个**：`TWO_FUNGI_BEASTS`（第一幕 strong 池）与五个事件编队
（`LAGAVULIN_EVENT` / `COLOSSEUM_EVENT_SLAVERS` / `COLOSSEUM_EVENT_NOBS` /
`MUSHROOMS_EVENT` / `MYSTERIOUS_SPHERE_EVENT`）。**六个全部由已登记的怪组成**，
所以它们大概率是一批装完的事。
...
| ④     | **剩下的 6 个编队**（`TWO_FUNGI_BEASTS` + 五个 `*_EVENT`） | **0 只新怪**——六个全部由已登记的怪组成                                | 体积小（都是短仗），≈ **12MB**                                                                  | 小：`MonsterGroup::createMonsters` 里已有 case。⚠ 装完这一批「编队」那一栏就是 63 / 63，而 `sts-combat-wiring.test.ts` 的「未迁移编队」样本会**永久失去样本**，届时要照 `sts-combat-rules.test.ts` 本批那条的办法改成断言不变量 |
...
✅ **① / ③ / ⑤ / ⑥ 都已交付，剩下的顺序建议改成：④ →（第四幕的爬升度与 `@tgt1`）→ ⑤（继续）→ ②。**
