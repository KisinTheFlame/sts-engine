...
| 编队 | 49      | 63                | `ENCOUNTER_BUILDERS` / `ENCOUNTER_SETUP`    |
...
⚠ **本批发现一处疑似参考笔误，裁定不打补丁**：蠕动血块的**萎缩**走
`attackPlayerHelper(bc, asc2 ? 12 : 10)` 却**不在** `isMoveAttack` 白名单里
（`MonsterMoves.h:527-529` 只收挥击 / 乱抽 / 重抽）。按第三十二批立下的判据
（「白名单收的就是走 `attackPlayerHelper` / `Actions::AttackPlayer` 的那些招」）它应该在，
真实游戏里萎缩显示的也是攻击 + 减益双意图。**三条判据只过了 ①③**（补丁没有预言机），
与爆破怪自爆那条同族——**照抄参考、记进「待裁定」**，并且这条差异已被数据钉住
（单独把萎缩加进白名单红 24 例）。
...
| ~~**`Monster::halfDead`**（僧侣 / 觉醒者的「假死」）~~ | ✅ **第三十四批结清**（暗影客的重生）。`isDeadOrEscaped` 的第三位现在真的被走到：`CombatMonster.halfDead` 独立建模，`doMonsterTurn` 的门改回 `(!isDeadOrEscaped() \|\| isHalfDead())`（去掉这条放行红 **113 例**）。另外两个循环（`applyPreTurnLogic` / `applyEndOfRoundPowers`）的门是 `isDying() \|\| isEscaping()`，它们照样跳过半死的怪（半死必然伴随 `curHp == 0`），故那两处仍写 `!alive`。仍缺的是**觉醒者**那条假死（`Monster::die` 的**第一个**分支，第三十八批） |

第十六批（孢子云 / 死亡触发）点名跳过的：

| 跳过的                                   | 为什么跳过                                                                                                                                                                                                                                                                                                                                            |
| ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Monster::die` 里**其余五条死亡触发**    | ~~停滞（`STASIS`，还牌）~~ **第二十八批已补**；~~暗影客的重生（`REGROW`）~~ **第三十四批已补**（那条 else-if 链的三格现在全齐：孢子云 → 重生 → 停滞）。仍缺：觉醒者的假死（`Monster::die` 的**第一个**分支，第三十八批）、尸爆（`CORPSE_EXPLOSION`，静默的牌）、地精之角 / 活体样本（两个遗物，harness 的遗物轮换里都没有）。`monsterDie` 里留了 TODO |
...
- ~~**死亡触发**（`Monster::die`）~~ **第十六批已完成**（本批需要的那一支：孢子云）。
  ...
  - 孢子云本身是个 **Power**（`preBattleAction` 里 `buff<MS::SPORE_CLOUD>(2)`），
    ...
    仍缺：觉醒者的假死（`halfDead` 的**另一个**写入点，`Monster::die` 的第一个分支）、
...
- ~~**逃跑**（`isEscapingB`）~~ **第十五批已完成**（`monsterEscape`）。
  ...
  仍缺：`escapeNext`、觉醒者那条假死、以及战斗后的 `g.info.stolenGold` 还钱（run 层第 7 项）。
...
- 测试：
  - `test/sts-combat-trace.test.ts` —— 逐帧对拍（**30946 例**，第三十五批），外加一条**数据自身的不变量**：
...
⚠ `three_darklings` 是目前第三幕最大的一份：**多怪 + 会复活 = 战斗均长 9.3 回合**
（那六个单怪编队是 3~7）。体积估算不能再按「2.5MB 一个编队」推。
...
| ⚠⚠ 其中「萎缩」那 **24** 例   |          | **它是疑似参考笔误的那一格**（走了 `attackPlayerHelper` 却不在表里），见「待裁定」 |
...
- **⚠⚠ 蠕动血块的「萎缩」走 `attackPlayerHelper` 却不在 `isMoveAttack` 白名单里。
  裁定：照抄参考，不打补丁，等真机。** 第三十五批按上一条那个判据反查时撞上的，
  **它是那条判据的反方向、也是至今最像笔误的一格**。

  ```cpp
  case MMID::WRITHING_MASS_WITHER: // 3                    // MonsterSpecific.cpp:1560-1565
      attackPlayerHelper(bc, asc2 ? 12 : 10);              // ← 走的就是那个函数
  ...
  而 `MonsterMoves.h:527-529` 只列了三条：
  `WRITHING_MASS_FLAIL` / `WRITHING_MASS_MULTI_STRIKE` / `WRITHING_MASS_STRONG_STRIKE`。
  植入不在是对的（它一点伤害都没有），**萎缩不在则与判据直接冲突**。

  ⚠ **它与爆破怪那条不是一回事，方向正相反**：爆破怪是「打了 30 点但走
  `Actions::DamagePlayer`」——参考在**它自己的**判据下是自洽的，只是与真实游戏的意图分歧；
  萎缩是「走了 `attackPlayerHelper` 却没进表」——**参考在它自己的判据下就不自洽**。
  ⚠ 旁证：同族的「攻击 + 减益」招式**全都在表里**（`CHOSEN_DEBILITATE` /
  `SPHERIC_GUARDIAN_ATTACK_DEBUFF` / `MYSTIC_ATTACK_DEBUFF` / `SNECKO_TAIL_WHIP` …），
  萎缩是唯一的例外；真实游戏里 Writhing Mass 的 Wither 显示的也是**攻击 + 减益**双意图。

  **三条判据：①③ 成立，② 不成立 —— 所以不补。**
  - **① 在已登记内容里真的产生分歧？——成立，实测过。** 觅敌之弱在本批的牌组里，
    单独把 `writhing_mass/wm_wither` 加进白名单红 **24 例**（「多收三条非攻击」那次
    合计 66 例，拆开是 凝视 30 / 植入 12 / 萎缩 24）。这不是理论风险。
  - **② 补丁有预言机？——不成立。** 预言机就是参考本身，改了它等于把断言写进被测物。
    要判它得有**第二个信源**（真实游戏 `WrithingMass.java` 那条 `setMove(..., Intent.ATTACK_DEBUFF, ...)`）。
  - **③ 行为唯一确定？——成立。** 只有一种改法（把 `WRITHING_MASS_WITHER` 加进
    `isMoveAttack`），不像地精头目那条有两个候选。

  **关门条件**：真实游戏 `WrithingMass.java` 的 ground truth，不是更多 trace。
  ⚠ 在那之前我们这边**照抄参考**（`MONSTER_ATTACK_MOVES` 不收 `writhing_mass/wm_wither`），
  而且**这条差异已经被 24 例数据钉住**——谁要是「顺手修好」会当场红，不会静默飘走。
  ⚠ 建议：等真机 ground truth 那一批到来时，把爆破怪自爆与这一条**一起裁**，
  它们是同一个信源的两个问题。
...
| ---------- | --------------- | -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **三十六** | `nemesis`       | 1（复仇魔）          | 第三幕精英。**怪物侧 `INTANGIBLE`**（隔回合无敌，`Monster::attacked` / `damage` 的入口各有一处钳制），它同时是 `attackedUnblockedHelper` else-if 链**第一格**的宿主之一——链上现在只剩那一格没有宿主。另有一条往弃牌堆塞灼伤的招式（`Actions::MakeTempCardInDiscard({BURN}, asc3 ? 5 : 3)`，**同步 `.actFunc(bc)`**）。⚠ 它的 `NEMESIS_ATTACK` / `NEMESIS_SCYTHE` 都在 `isMoveAttack` 白名单里，`NEMESIS_DEBUFF` 不在                                                                                                                                                       |
| **三十七** | `reptomancer`   | 2（蜥蜴法师 / 匕首） | **召唤的第四族**（`reptomancerSummon`）+ **预留空位的第四种写法**：`++monsterCount; createMonster(DAGGER); createMonster(REPTOMANCER); ++monsterCount; createMonster(DAGGER);`（`MonsterGroup.cpp:339-345`）——**0 号位与 3 号位空、两把匕首在 1 / 4 号位、法师在中间的 2 号位**，`monsterCount = 5`。前三族的并列表见 WORKFLOW，第四族要照着再加一列。⚠ 匕首的自爆 `DAGGER_EXPLODE` **在** `isMoveAttack` 白名单里（它走 `attackPlayerHelper`），与第三十二批爆破怪那条正好相反                                                                                            |
| **三十八** | `awakened_one`  | 1（觉醒者）          | ⚠ ~~爆炸半径最大的一批~~ **第三十四批已经把 `halfDead` 那条共享路径铺好了**（`CombatMonster.halfDead` + `doMonsterTurn` 的门 + `PowerInstance.cleared` + harness 的 `halfDead` 快照字段），这一批只剩觉醒者自己那条分支要写。它走的是 `Monster::die` 的**第一个分支**（`halfDead = true; removeDebuffs(); removeStatus<CURIOSITY>(); setMove(REBIRTH); cardQueue.clear();`，`Monster.cpp:285-291`）——它排在「判胜 return」**之前**，是那条链上唯一能跳过判胜的路径。另有 `CURIOSITY`（玩家打能力牌时 +力量）与二阶段才上的 `MINION_LEADER`。编队里还有两只邪教徒（已登记） |
| **三十九** | `time_eater`    | 1（时间吞噬者）      | ⚠⚠ **第二大爆炸半径，单独做。** **`TIME_WARP`** 是第一个**改回合结构**的 Power：玩家每打出 12 张牌就 +2 力量并**立即结束玩家回合**，读点在 `useCard` 那条共享路径上。另有 `HASTE`（`curHp = maxHp / 2`，一次性）。⚠ 动 `executeActions` 的回合边界，按 WORKFLOW 的规矩这一批**不叠别的机制**                                                                                                                                                                                                                                                                               |
| **四十**   | `donu_and_deca` | 2（迪卡 / 多努）     | 第三幕最后一个 Boss，两只**都带 `ARTIFACT`**（asc19 是 3 层）。机制上全是已有原语的新宿主（力量 / 神器 / 群体伤害），是收官批。装完第三幕 16 / 16。⚠⚠ **它同时是当下「未迁移编队」那两条测试用例的样本**（第三十五批从 `giant_head` 换过来的），做这一批之前先换掉——判据与换法见 WORKFLOW「附：踩过的坑」                                                                                                                                                                                                                                                                  |

⚠ **体积估算**：第三十二批实测 **8.4MB / 3 编队**、第三十三批 **7.4MB / 3 编队**、
第三十四批 **7.4MB / 2 编队**（⚠ 三暗影客一份就 **5.2MB**——**多怪 + 会复活 = 仗长得多**）、
第三十五批 **6.0MB / 2 编队**（两个单怪编队各 3.0MB；仓库现在是 **595MB / 111 个文件**）。
剩下 6 个编队按 3~5MB 估约 **25MB**，Boss 与精英更耐打、可能到 35MB。总量到 **~630MB**。
...
| **萎缩不在 `isMoveAttack` 白名单里**                                                                   | 24   | ⚠ **不是盲区，是「待裁定」的分歧**：单独把它加进白名单红 24 例。疑似参考笔误（它走 `attackPlayerHelper`），三条判据只过了 ①③，照抄参考等真机                                                                    |
