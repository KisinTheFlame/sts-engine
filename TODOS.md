...
| 顺位 | 做什么                               | 为什么排这里                                                                                                                                                                                                                                                                                  |
| ---- | ------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | **中毒（POISON）+ 扭曲漏斗**         | 战斗内**唯一一颗「只差一个前置机制」的遗物**（一.9 那张分类表里的最后一格）。读点只有三处：`Monster::applyPreTurnLogic:36`（挂 `PoisonLoseHpAction`）、`Monster.cpp:548` 的 `removeStatus`、`BattleContext.cpp:424` 的 `DebuffAllEnemy<POISON>(4)`。一个 `@relicN` variant + 一场长仗就能背书 |
| 2    | **`sts-neow` 的效果**（一.6 第二片） | 待办里最大的一块，但**第一片已经就位**（真实选项已进 `GameState.neowOptions`），换代那一步是「逐个转写 19 × 6 的效果 + 把事件屏切过去」。⚠ 老档已按近似效果结算过，回填只能给出「本该是哪四个」                                                                                               |
| 3    | **六条 run 级 RNG 流**（一.7）       | 奖励 / 遗物池 / 事件 / 商店 / 宝箱，**全新写而非转写**。做完才能删掉玩具 `rng.ts`——那是「近似实现全部删除」这条总纲最后一块没兑现的                                                                                                                                                           |
| 4    | **`asc7 + asc16`**（跨三幕）         | 纯数据、实现侧一行不改，但 ≈ 200MB。它关掉「三档里的中间那一档」13 条 + 冠军两族阈值 + 血量阈值 7/8/9                                                                                                                                                                                         |
| 5    | 剩下 23 颗遗物                       | 逐条的阻塞原因见一.9 的分类表：充能球 6 / 姿态 2 / 造参考没实现的牌 2 / 选牌屏 5 / 状态牌可打性 2 / 参考自己注释掉的 1（梅兰奇，可像情绪芯片那样登记成空操作）                                                                                                                                |
...
- **参考自己注释掉了唯一读点**（1）：梅兰奇（`// TODO SCRY Action`）——「好奇心」那一族，
  可以像情绪芯片一样登记成空操作
- **可以直接做**（1）：**扭曲漏斗**（`atBattleStart` → `DebuffAllEnemy<POISON>(4)`），
  唯一的前置是**中毒**这个 Power 我们还没建模
