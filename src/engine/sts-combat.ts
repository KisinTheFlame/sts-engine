...
const MOVE_RULES: Record<string, MoveForRoll> = {
  ...
  // ⚠⚠ **`!mysticAlive` 那两支（狂怒连斩）的「选择」有 8 例背书，「效果」一例都没有
  //   ——两件事必须分开看。**
  //   ① **选择**：狂怒连斩在 375 条 trace 里作为意图出现 **88 次，全部落在一具已死的百夫长
  //      身上**。机制是**荆棘**（遗物轮换里的青铜鳞片）：百夫长斩击玩家触发的荆棘走
  //      `addToTop`，插在它自己那条**入队的 RollMove 之前**——低血的百夫长先被荆棘打死，
  //      紧接着那次 RollMove 在尸体上执行，此时 `monstersAlive == 1` → 返回狂怒连斩。
  //      死怪的意图**在快照里**（harness 连死怪一起 dump），所以这一支被钉住了：
  //      把 `mysticAlive` 改成恒真红 **8 例**。
  //   ② **效果**（`takeTurn` 的 6×3 与收尾）：**0 例，是盲区**——死怪永远不会 `takeTurn`。
  //      要让活着的百夫长出这一招，得让秘法师**先死**，而 `CENTURION_AND_HEALER` 是全参考
  //      项目**唯一**带百夫长的编队（`MonsterGroup.cpp:193-196` 是 `MonsterId::CENTURION`
  //      的唯一出处）、百夫长恒在 0 号位、harness 的策略恒打 `firstAliveMonster` = 0 号位。
  //      ⚠ 换牌组救不回来：单体伤害只落在百夫长身上、群伤两只平摊，而秘法师每次治疗给两只
  //      各回 16 且自己血上限更低（回血更容易被上限吃掉）——「秘法师先死」在任何牌组下都
  //      不成立。这与第十五批抢劫者逃跑那条同族（「编队的成员结构本身」那一维），
  //      **关门条件是给 harness 加一条「打最右侧」的目标策略轴**（与爬升度轴同构：
  //      `DeckVariant` 加个默认 0 的字段 + 文件名后缀）。本批不做那个轴（那是第二个机制，
  //      见 WORKFLOW），如实记进 TODOS 盲区表。
  //      ⚠ 真实游戏里「先秒奶妈」是标准打法，所以这一支不是边角——只是我们量不到。
  // ⚠ 顺带：`check-coverage.mjs` 把这一招报成「出现 0 / 执行 0」，因为它的 `countMoves`
  //   跳过 `!alive` 的怪。覆盖表报 0 时要再去数据里 grep 一遍死怪的意图，见 TODOS。
  centurion: (bc, m, roll) => {
