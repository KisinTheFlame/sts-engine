...
// ⚠⚠ **近似遭遇池（三幕的 weak / strong / 精英 / Boss 权重表 + `weightedPick`）第五十一批
//   整块删掉了**，接位的是 `sts-encounters` 的 `generateEncounters`：一条持久的 `monsterRng`
//   在开局一次性生成三幕全部序列，run 层按序索引（见 `GameState.encounterPlan`）。
//   删除的理由与第四十七批删近似战斗、第五十一批之前删近似遗物钩子同源——
//   **有了游戏级对应物的近似实现一律删掉，不留两套**。
//   ⚠ 那三张权重表当年的注释自己写着「此处只含已实现怪物的子集」「切片」，
//     即它们从来就不是原版的数值；留着只会让「同种子复现原版」这句话失真。
