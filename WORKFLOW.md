...
- ⚠⚠ **「未迁移**编队**」的样本没有永久解，每隔几批就要换一次——把这件事当成常规维护，
  ...
  **判据只有一句：挑最晚才会被登记的那个**（读 TODOS 的批次计划表，取最后一行）。
  ...
  → **`giant_head`**（第三十五批顶掉）→ **`donu_deca`**（第三幕收官批第四十批才会顶掉）。
  ⚠ 别换成「下一批就要装」的那个（第三十五批时的 `nemesis` / `reptomancer`）。
  ⚠ 别换成 `the_guardian` / `hexaghost` / `slime_boss`：它们**就在**冻结的那 20 个里。
  ⚠ **想换到第四幕（`the_heart` / `shield_and_spear`）或事件编队（`MASKED_BANDITS_EVENT`
  那一族）能撑更久，但要先掂量代价**：`enemies.ts` 里根本没有那些怪的定义
  （腐化之心 / 尖塔护盾 / 尖塔长矛 / 罗密欧 / 熊 / 尖头怪一只都没有），
  为了一条测试样本去加**没有预言机的新数据**与「未验证的实现比没有实现更糟」相抵触。
  第三十五批因此选了「现有条目里最晚被登记的那个」。
