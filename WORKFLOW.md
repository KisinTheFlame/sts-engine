...
- ⚠⚠ **`preBattleAction` 上 Power 有第三种写法：`setHasStatus<X>(true); setStatus<X>(0);`
  ...
  - **所有读点必须用 `hasStatus`（条目在不在）而不是「层数 > 0」**，否则这条 Power
    **一次都触发不了**（0 > 0 恒假）。这与飞行那一族（层数会减成负数）是同一个坑的两个方向。
    ...
    （= 整个编队）。
- ✅✅ **`attackedUnblockedHelper` 那条 else-if 链第四十七批起八格全部有宿主、全部有背书**
  （无敌 47 → 镀甲 25 → 蜷缩 13 → 飞行 24 → 易塑/反应 23/35 → 荆棘 32 → 沉睡 18 → 变换 34）。
  第一格「无敌」的宿主是**腐化之心**，全参考只有它一只带。
  ⚠ **「每一格都有宿主」不等于「每一格的位置都有背书」**：链上有五格的**位置**仍是结构性
  盲区（没有一只怪同时带其中两位），逐条见 TODOS 的盲区表。
...
- ⚠⚠ **怪物批次的牌组也不是常量：第三十七批第一次为「让新怪的代码被走到」换了牌组。**
  ...
  **判据（补一条）：先问「这一批新写的每一行，各自要什么局面才可观察」，再挑牌组；
  ...
  ⚠⚠ **第四十七批第四次量牌组，而这次的答案是「更弱的牌组」——这是第一次。**
  腐化之心 750 血 + 无敌 300（每回合最多掉 300）⇒ **强牌组三个怪物回合就打完**，
  而它「强化」那一招的档位由 `getMonsterTurnNumber() / 3` 决定（第 4 / 7 / 10 / 13 个
  怪物回合各一档）。**打得快与死得快都会压低回合数**，于是最优解是一副**只防不攻**的
  40 张（10 铁壁 + 8 幽灵护甲 + 4 耸肩 + 4 包扎 + 2 金属化 + 2 觅敌之弱）：
  平均 5.73 个怪物回合、最长 10，档 2 从 6 / 120 涨到 55 / 120、档 3 从 0 涨到 3。
  **判据（补一条）：先问「这一批要背书的东西是随回合数增长的，还是随伤害增长的」。**
  前者要的是**长仗**，而「打得动」与「长仗」在有伤害上限的 Boss 身上是**互相矛盾**的。
...
#### 第二次做（第四十七批，第三幕）：7 / 15，而且第一次叠了两条轴

第三幕 15 个编队里只有 **7 个**过「场上会不会同时有两只可选目标」这道判据。
⚠⚠ **第三幕的特殊之处：它一个分裂都没有**（`Monster::onHpLost` 那个 switch 里只有三种
大史莱姆与守卫者），唯一的召唤宿主是蜥蜴法师，其余的场上变化全是**减少**（消逝自杀、
自爆）或**就地**（假死、重生）。所以八个单怪编队真的一个都进不来——这条要**证一遍**，
不能照第三十一批「有四个开局单怪却要算进来」的印象推。
✅ 现在这份名单是 harness 里的一道 `switch`（点名单怪编队就 `return 1`），不是注释。

⚠⚠ **同一批出现了全语料第一个双后缀文件 `donu_and_deca@asc19@tgt1`**：迪卡守护方阵的
**镀甲**那一半只在 asc>=19 存在，它那条 `noAliveGate` 因此**两条轴同时**才观察得到
（实测红 7 例）。分组键的 `[@ascN][@tgtN]` 顺序从第三十一批起就定死了，
`split-traces.mjs` 与两个测试文件**一行都没改**——这是「形状先定死」那句话的兑现。
...
#### ⚠⚠ 「单读点」这一族可以整批吃，但 8 颗是硬上限、分组才是难点（第四十三批）

第四十三批一次登记 **38 颗**，是前三批（5 / 5 / 5）的七倍多。做得到的唯一原因是**先按
「战斗内有几个读点」筛过一遍**：

```bash
grep -rn "\b<RELIC_NAME>\b" src/combat include/combat | wc -l
```

⚠ **用裸名字、而且要带 `include/combat`**：`Monster.cpp` 写 `RelicId::X`、`BattleContext.cpp`
写 `R::X`，只搜其中一个别名会得到「零命中」这种危险的假象；而**姜 / 芜菁 / 冠军腰带三颗的
唯一读点在头文件里**（`Player.h` / `BattleContext.h` 的模板函数），只搜 `src/` 会整族漏掉。

全部 181 个 `RelicId` 里，战斗目录下有引用的是 **119 个**，其中**只有一个读点**的 **70 个**
——这一族基本都是 `initRelics` 里一两行的开局效果，或者某个时点上的一个 `if`。剩下 49 个是
多钩子的（2~9 个读点），每颗都要单独读参考、对多个时点，**不适合混在同一批里**。

**三条硬约束**（前两条是 harness 当场拒绝，第三条是量出来的）：

1. **一个 variant 最多 8 颗**——`initRelics` 的 `atBattleStart` 是 `fixed_list<RelicId, 8>`，
   harness 有一道粗检查兜底。所以 38 颗**至少**要 5 个 variant。
2. `relicSet` 必须互不相同且非 0。
3. ⚠⚠ **覆盖密度**：38 颗堆一起会把彼此的分母压垮。第四十三批的分组原则是三条：
   - **效果不互相遮蔽**：如尼金字塔（回合末不弃手牌）与不休陀螺（手牌空了补一张）放一起，
     后者**结构性不可达**；山铜的门是「回合末格挡为 0」，与四颗加格挡的遗物放一起会被饿死。
   - **仗长要保住**：五颗 `energyPerTurn++` 让平均回合数从 ~6 掉到 **2.5**（比第四十一批
     硫磺那次 9.54 → 7.17 猛得多）。做法不是「摊开」，而是**把它们单独关进一个 variant**
     ——那一组的效果全部在开局第一帧就可见，短仗反而无所谓。
   - **编队要能看见那一组的效果**，而这个必须量，见下一条。

#### ⚠⚠ 「先量再定」第六次：这次量的是**四十个门**，而且量法变了

前五次都是「拿 2~3 个候选跑一遍、比一张表」。四十个门没法这么干，第四十三批的做法是：

**给参考源码打一层临时的计数器补丁**（`++g_m[N]`，一个 `extern long long g_m[64]` 声明 +
每个门一行），harness 里加一段 per-(variant × encounter) 的 stderr 汇总，跑完
`git checkout -- .` 还原。⚠ 关键是**计数器加在「门的条件」上而不是「遗物的分支」里**——
这样**一次不带遗物的 run 就能量出全部四十个门的分母**，用来挑编队；之后再拿真正的分组跑
第二次，确认叠起来之后每个门仍然非 0。

这一轮里**真的改变了决策**的几格（第一次 run：22 个候选编队 × 120 条、22 张牌组、
只带一颗光滑石；血腥雕像那一行来自第二次 run，因为它要贪婪之手在牌组里才可能触发）：

| 门                            | 最肥的编队                | 看着该有、实际是 0 的                               |
| ----------------------------- | ------------------------- | --------------------------------------------------- |
| 奇特蘑菇（**玩家**易伤）      | champ 503 / collector 316 | hexaghost **0**、three_byrds **0**、lagavulin **0** |
| 冰淇淋（回合末剩能量）        | time_eater 69             | 其余 20 个编队全在 1~23                             |
| 血腥雕像（贪婪之手打死怪）    | three_sentries 85         | donu_and_deca **0**                                 |
| 卡戎的骨灰 / 奇异汤匙（消耗） | slime_boss 506            | 22 张牌组下**其余 19 个编队全 0**                   |

最后一行值得单记：**它不是「换编队」能解决的，要改牌组**。22 张那副里一张会消耗的牌都没有，
所以「有牌进消耗堆」只在怪物往牌堆里塞状态牌的三个编队里发生。加 2 张献祭（0 费、消耗、
还掉 6 点血）一次解决三件事：给两颗遗物喂门、把玩家打到满血以下（不然回血类遗物全看不见）。

> **判据：一颗遗物的门是 0，先问「这是编队的问题还是牌组的问题」。**
> 前者换编队，后者往牌组里加一张**专门为这道门服务**的 0 费牌，并在 harness 注释里
> 写清「这张牌是为谁加的」。

#### ⚠ 排除比登记更需要写清楚：七个「不能登记」的族

第四十三批筛完 70 颗单读点遗物：11 颗此前已登记、38 颗本批登记、**21 颗排除**。
排除理由归成七族，以后遇到同族的直接照这个判：

| 族                        | 例                                                   | 判据                                                               |
| ------------------------- | ---------------------------------------------------- | ------------------------------------------------------------------ |
| 需要充能球                | 破裂核心 / 核电池 / 共生病毒 / 如尼电容 / 冰核       | 我们没有充能球模型，harness 快照里也没有                           |
| 需要姿态                  | 泪滴挂坠                                             | 同上，且 `changeStance` 会连锁排动作                               |
| 读 `r.data`，而 data 是 0 | 杜乌娃娃 / 基里亚（`buff<STRENGTH>(r.data)`）        | 整条 case 是**空操作**，没有任何可观察面 ⇒ 拿不到变异非 0          |
| 读 `gc.curRoom/lastRoom`  | 缩放仪 / 昆虫标本 / 奴隶主颈圈 / 勇气投索 / 古董茶具 | harness 从没设过这两个字段（恒 `Room::INVALID`）⇒ **结构性盲区**   |
| 从整个牌池随机造牌        | 手册 / 枯枝 / 尼尔的法典                             | 随机牌可能是**未登记**的牌 ⇒ trace 不可重放                        |
| 参考的实现被注释掉了      | 什锦（`// addToBot(SetState(SCRY, 3))`）             | 没有预言机，与 `SEEK` 那族卡同理——**永远不能登记**                 |
| run 层 / 会卡死           | 黑血 / 燃血 / 骨头上的肉；活体样本                   | 前三个在战斗后的回血 switch 里；活体样本排的 InputState 没有消费者 |

⚠ **第四族是唯一带明确关门条件的**：给 `DeckVariant` 加一个 `Room room`（沿用
`ascension` 那个「默认值保持逐字节不变」的老套路）+ trace 头部加一个 `room` 字段，
**一次开五颗**。本项目至今没有把 `room` 送进战斗，所以这是一条真正的接线缺口，不是盲区。

#### ⚠⚠ `RelicInstance.data` 有**两种**语义，而它们该走**两条**不同的通道（第四十四批）

`initRelics` 对八颗遗物读 `r.data`，但读法不是一回事：

- **数值**（快乐花 / 熏香炉 / 墨水瓶 / 插入器 / 双节棍 / 笔尖 / 日晷 / 尼奥的挽歌）——
  搬进 `Player` 的某个计数器，`updateRelicsOnExit` 再写回去，**跨战斗延续**。
- **真假**（御守 / 蜥蜴尾）—— `p.setHasRelic<X>(r.data)`，**覆盖**从 run 层拷进来的位：
  **`data == 0` 等于「这颗遗物在战斗内不存在」**。

⚠⚠ **六颗「数值」的写法两两都不同**（`= data + 1` 再判 / `= data` 再 `++` 判 / 光搬不判 /
`data == 9` 走另一支），而且**同一颗遗物在 `initRelics` 与回合开始两处的写法也不同**
（快乐花：一处同步加 `energy`、一处 `addToBot(GainEnergy)`）。**逐处照抄，别提取公共函数。**

⚠ **trace 格式只需要带「数值」那一半**：给「真假」那两颗加一道 harness 检查
（不许以 data 0 发出，那等于白发一颗），「出现在清单里 ⟺ data 非 0」就按构造成立。
**这不是洁癖，是为了让新字段对既有语料是空操作**——`writhing_mass@relic3` 从第四十批起
就带着 `{OMAMORI, data 2}`，把它算进新数组，那 120 行的头部当场就变了。

⚠ **引擎侧要多一份 `player.relicBits`**（对齐 `Player::relicBits0/1`）：`initRelics` 的第一句
把容器的位拷一份，此后**四处只改这一份**（御守 / 蜥蜴尾的 `setHasRelic<X>(r.data)`、
蜥蜴尾复活用掉、百年拼图触发过一次）。**用容器判「有没有这颗遗物」会让这四处静默失效。**

⚠ **写回那一半永远没有 trace 预言机**：一条 trace 就是一场战斗。

#### ⚠⚠ 「照抄邻居必错」的又一次：**被省略掉的那个默认实参**（第四十四批）

`Actions::DebuffAllEnemy<s>(int amount, bool isSourceMonster = true)`（`Actions.h:42`）。
全参考 10 个调用点里，**大理石袋等 8 处显式写了 `false`，只有红面具走默认值 `true`**
——于是红面具那层虚弱**跳过第一个回合末的递减**。两条 case 在同一个 switch 里上下相邻，
差别只有那个被省略的实参。实测抄成 `false` 红 240 例（= 那一组两个文件的全部）。

> **判据：抄一条 `Actions::X(...)` 之前，先去 `Actions.h` 看一眼它有没有默认实参，
> 以及邻居有没有显式覆盖它。** 「函数名与参数个数都一样」正是最危险的形状。

#### ⚠⚠ `CardQueueItem::card` 是**副本**，出牌中途改手牌属性时两边不同解（第四十四批）

参考的 `CardQueueItem::card` 是 `CardInstance card;`（按值），而 `cards.hand[...]` 是另一个
对象；`useCard()` 末尾那句 `player.useEnergy(c.costForTurn)` 读的是**副本**。
我们这边 `item.card` **故意**指着手牌里那个对象（好让卡效果对它的改写跟着进弃牌堆）。
在**木乃伊之手**之前两者同解——它是第一个「在出牌中途改**别的手牌**的费用」的东西，
而且它的候选表**包含正在被打出的这张能力牌**（参考此刻还没把它移出手牌，移出那一句排在
`onUsePowerCard` 之后）。抽中自己时参考照样把手牌那份压成 0，但那份马上被移出、
扣能量读的是副本 ⇒ **一点都不少扣**。不快照的话那张牌会变免费，实测红 **73 例**。

> **判据：登记一个「在出牌中途写手牌」的东西时，先数一遍参考在那之后还会从
> `curCardQueueItem.card` 读哪些字段——凡是它从副本读的，都要用「进 `useCard` 那一刻」的值。**

#### ⚠ 遗物族里最容易抄串的三种形状

1. **「同一道门里的二选一」不是两次乘法。** 纸蛙 / 纸鹤 / 奇特蘑菇都写成
   `if (有这个状态) { if (有这颗遗物) damage *= A; else damage *= B; }`——写成「先 ×B 再补一份」
   在 `float` 域上会分岔。⚠ 而且**纸蛙与奇特蘑菇是两个不同的函数**
   （`BattleContext::calculateCardDamage` 管「牌打怪」、`Monster::calculateDamageToPlayer`
   管「怪打玩家」），两者的默认倍率恰好都是 `1.5f`——照着邻居抄必错。
2. **免疫类遗物排在神器之前。** `Player::debuff` 的顺序是
   `amount == 0` → 姜（虚弱）→ 芜菁（脆弱）→ 神器。所以被姜挡掉的那次虚弱**一层神器都不消耗**。
   挪到神器下面是可观察的差别，不是风格问题。
3. **上下界的开闭不对称。** 靴子是 `damage < 5`（不含 5），鸟居是 `damage <= 5`（含 5）。
   两条并排读的时候极容易抄串，各自的变异都要量。

#### ⚠⚠ 「计数器什么时候清零」是承重的，登记读它的遗物之前先把位置摆对

`BattleContext::afterMonsterTurns` 里三个计数器的清零排在
`applyStartOfTurnPostDrawRelics` / `...Powers` **之后**（BattleContext.cpp:2240-2243），
参考在那里自注 `// this has to be here because some relics check this info.`。
两族遗物因此读到的都是**上一个回合**的值：战争艺术读 `attacksPlayedThisTurn`（更靠前的
`applyStartOfTurnRelics`），**怀表**读 `cardsPlayedThisTurn`（就在这三句上面）。

我们此前把这三句摆在「清格挡之后、入队抽牌之前」——在怀表登记之前两种摆法同解，
登记之后就不是了（那样怀表**每回合都触发**，`0 <= 3` 恒真）。

> **判据：登记一颗「读回合内计数器」的遗物时，先回参考数一遍它与清零点的相对位置，
> 别信我们这边现有的摆法——那个摆法很可能只是「当时没人读，怎么放都对」。**

#### ⚠ 参考函数的签名少一个实参，也是一种转写债

`BattleContext::calculateCardDamage` 的签名一直是
`(const CardInstance &card, int targetIdx, int baseDamage)`，我们这边一直只有后两个——
因为在打击假人之前**没有任何伤害修正读那张牌**。补它的时候把参数写成**必填**，
`tsc` 会把 37 个调用点一次全找出来；写成可选参数则会留下「哪天忘了传就静默少 3 点」的坑。
...
前三条轴都是「一个 int 既当旋钮又当文件名」（`ascension` / `targetPolicy` / `relicSet`）。
药水这条做不到，因为它一次要表达两件独立的事：

| 字段           | 是什么                                                                   | 进分组键 |
| -------------- | ------------------------------------------------------------------------ | -------- |
| `potionSet`    | **身份**：`@potN` 后缀 + variant 指纹 + **放宽可重放药水白名单的开关**   | ✅       |
| `potionPolicy` | **喝药时机**：0 = 开局立刻喝（历来），1 = 只在 `curHp * 2 <= maxHp` 时喝 | ❌       |

「一个 potionSet = 一个 variant」，时机是它的**属性**而不是身份——与 `relicSet` 之于
`relics` 清单完全同构。想让 6 组药水共用同一条时机规则、又各自有文件，就必须拆成两个字段。
⚠ `potionPolicy` 仍然进 **variant 指纹**（`split-traces.mjs` / `variant0-rows.mjs`），
只是不进文件名——指纹多一维永远是免费的，少一维才会出哑弹。

#### ⚠⚠ 「可重放药水」白名单**不能全局放宽**，这是本批最大的一处地雷

`isReplayablePotion` 不只决定「显式清单里的药水能不能喝」：**熵酿会从整个药水池补槽**，
而轮换里 13 项中有熵酿 ⇒ 大约 3/13 的 trace 手里有一瓶熵酿。把新登记的药水加进那张表，
策略就会开始喝**以前跳过的**那些 —— 163 个已提交文件里相当一部分当场被改写。

做法：`isReplayablePotion(p, bool extended)`，`extended = (variant.potionSet != 0)`。
于是「新药水只在 `@potN` 文件里出现」按构造成立，旧数据是空操作。
⚠ **推论：以后要发一瓶新登记的药水，就得开一个 `@potN` variant**；往 `@relicN` variant
里塞是做不到的。

#### ⚠⚠ 新乘积挂在最后，但这一次**没有**冻结它前面那个

第三十一/四十批的规矩是「新轴另开乘积挂到最后，且此后不许往前面的乘积追加 variant」。
遗物那条战线还在每批往 `relicVariants` 追加，而 `relicVariants` 排在这个乘积**之前**——
按老规矩两者只能活一个。

解法是把第四十二批那条观察**升级成硬约束**：`traceIdx` 只驱动遗物轮换与药水轮换，
**两者都显式钉死的 variant 一次 traceIdx 都不读**，它排在乘积里的什么位置完全无关。
harness 因此对 `potionVariants` 加了一道检查：**每个 variant 都必须同时给出非空的
`relics` 与 `potions`**，否则当场 `return 1`。有了它，遗物那条战线继续追加对
`@potN` 的 19 个文件是空操作。

> **判据：两条战线要并行往不同乘积追加时，靠的不是「谁排最后」，
> 而是「后面那个乘积读不读 `traceIdx`」。把它变成 harness 的硬检查，别写在注释里。**

#### 两步验证照旧（第六次做，一次都没白做）

先只加字段与后缀、**不加任何 variant** → `tools/regen-traces.sh --check`，
第四十五批实测 **163 / 163 个文件逐字节复现**。不过就停下来查。

#### ⚠⚠ 这条轴的价值：三族行为在 `potionPolicy 0` 下是**结构性死的**

历来的 `pickAction` 把喝药排在出牌**之前**且从不拒绝，所以三瓶药一律在**第 0 回合、满血**
喝光。第四十五批把「这到底遮掉了什么」量了出来（840 条 × 7 个编队）：

| 现象                       | policy 0                                    | policy 1（`curHp * 2 <= maxHp`）      |
| -------------------------- | ------------------------------------------- | ------------------------------------- |
| 喝在第几回合               | **0.00**                                    | **7.27**                              |
| 喝的时候玩家血量           | **80 / 80**（区间 80~80）                   | **32.5**（区间 1~40）                 |
| 血之药水**实际回复量**     | **0，360 / 360 次**（`min(maxHp, …)` 夹平） | **32，240 / 240 次**（带树皮恒 16）   |
| 液态记忆看到的弃牌堆张数   | **0.0，360 / 360 次是空的**（当场 return）  | **8.4**，选牌屏点亮 **1361** 次       |
| 喝的那一刻场上有怪带着格挡 | 360（**全部**来自拉加维林开局自带的 8 点）  | 296（champ 223 / three_darklings 73） |

三条可复用的结论：

1. **「回复类药水在满血时喝」不是「少一点背书」，是 0 例**——`Player::heal` 末尾的
   `min(maxHp, …)` 把所有回复量夹成同一个数。凡是靠回血量分辨的常数，policy 0 下一律不可观察。
2. **「开局第一步」这个时点本身会让一整族动作退化成空操作**：液态记忆的
   `BetterDiscardPileToHandAction` 对空弃牌堆直接 `return`，而第 0 回合弃牌堆**恒空**。
   登记一瓶「读牌堆」的药水之前先问一句「第 0 回合那个牌堆里有东西吗」。
3. ⚠ **编队要量，policy 1 下「玩家会不会真的被打到半血」逐编队差别极大**：
   7 个候选里 `three_sentries` 有 **112 / 120 条一口药都没喝**、`gremlin_nob` 95、
   `lagavulin` 66；`champ` 与 `three_darklings` 是 **0**。**「仗长」不等于「玩家会掉血」。**

⚠ 还有一个**涌现**的行为要知道：policy 1 下喝完血之药水会把血量抬回半血以上，于是
**同一批药不再一次喝光**——`@pot1` 里 240 条 trace 喝了 658 次而不是 720 次，第二、三瓶
要等血量再掉下来。这不是 bug，但它会让「一个 variant 里三瓶药各喝几次」不再相等。

#### ⚠ 神圣树皮的 A/B：一颗遗物换来一组三瓶的分档背书

`hasBark ? A : B` 在 `drinkPotion` 里有 **33 条**，每条是一对独立的字面量。要给它们拿背书，
最省的形状是**成对的 variant**：B 面 = A 面 **只多一颗神圣树皮**，其余（牌组 / 种子 / 编队 /
爬升度 / 目标策略 / 药水清单 / 喝药时机）逐字相同。于是

- 一次 `diff` 就是那三瓶的分档证据（不必靠变异去推）；
- 变异「只改树皮那一档」红的例数**只落在 B 面文件上**，与 A 面完全分开，
  两个方向各拿各的例数。

第四十五批六组全部这么做，代价是每组多一个 variant、一个编队（B 面只挂一个编队就够）。

#### ⚠⚠ 判「一瓶药能不能登记」的第一道门不是覆盖，是**参考会不会当场崩**

第四十三批那张「七个不能登记的族」表在药水这边多了**三条新的**，而且第三条最阴：

| 新增的族                      | 例                                    | 判据                                                                                                                                                         |
| ----------------------------- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **动作是默认构造的 `Action`** | 暗影精华 / 容量药水 / **毒药**        | `Actions::X` 直接 `return sts::Action();` ⇒ `actFunc` 是**空的 `std::function`**，`executeActions` 那句 `a(*this)` 抛 `bad_function_call`、整个 harness 终止 |
| 造一张参考没实现的牌          | 瓶中奇迹（MIRACLE）/ 诡计药水（SHIV） | 与 `seek` 同族：那张牌三个 switch 里都没有 case                                                                                                              |
| 参数是**上一次开屏的残值**    | 赌博酿                                | `Actions::GambleAction` 从不设 `pickCount`，`isValidMultiCardSelectAction` 的 GAMBLE 分支也不读它 ⇒ 弃几张由上一块屏遗留的值决定                             |

⚠⚠ **毒药那一条要单独记住：它的崩溃是延迟的。** `DebuffEnemy<MS::POISON>` 本身跑得好好的，
`Monster::applyStartOfTurnPowers`（`Monster.cpp:36-38`）会在**那只怪的下一个回合**才入队
`Actions::PoisonLoseHpAction()`——同样是空 Action。所以「试一下没崩就行」这种验证方式
会漏掉它。**判据：登记之前先 `grep -n 'Action Actions::<名字>' src/combat/Actions.cpp`
看函数体是不是 `return sts::Action();`，别只看 `drinkPotion` 那条 case。**

⚠ 反过来，**「改已有牌的费用 / 升级位」不算「造牌」**（第四十四批的判据）：蛇形油的
`RandomizeHandCost` 与熔炉祝福的 `UpgradeAllCardsInHand` 都可以登记，混沌精华把抽牌堆顶
那张打出去也可以——只要那个 variant 的**牌组**里没有未登记的牌 / 造牌牌。
harness 对 `potionVariants` 加了一道显式的牌组检查兜底（混沌精华与浩劫 / 混乱同族，
它打牌**一道门都不过**）。

#### ⚠ 「一个乘积 = 一族药水」时，分组的约束与遗物那边不同

遗物那边的三条分组原则（效果不互相遮蔽 / 仗长要保住 / 编队要看得见）照旧成立，
但药水多了一条**槽位**约束：`bc.potionCapacity` 是 3，`variant.potions` 按
`i % size` 循环填槽，**所以一个 variant 最多同时表达 3 瓶**（列 1 瓶 = 三槽都是它）。
15 瓶因此至少要 5 个 variant，和「8 颗遗物一个 variant」不是一个量级。

⚠ 仗长那条在药水这边一样咬人：**混沌精华三瓶在第 0 回合白打 9 张牌**，平均回合数从
~7.2 掉到 **4.38**（`gremlin_nob` 只剩 **0.42** 回合）。做法与第四十三批的
「五颗 `energyPerTurn++`」相同——**单独关进一个 variant**。
...
⚠⚠ **跑批期间，工作区属于跑批脚本，别的东西一律不许碰它。第四十五批连踩两次：**

1. **`git add -A` 把探针提交进了历史。** 当时跑批在后台，前台在写文档、顺手
   `pnpm format && git add -A && git commit` —— 工作区里正躺着「集中药水 2 → 3」那条探针，
   于是它进了 docs 那个 commit。**下一次对拍当场红 240 例**才发现（这一步没被跳过，
   所以它没变成静默的错），修回来另开了一个 `fix` commit。
   ⚠ 危害不止于「多一个错的常数」：`git checkout -- <file>` 从此还原到的是**带着探针的
   HEAD**，后续每一条探针的「基线」都不再是干净的。
2. **两个跑批脚本并发**。第二个脚本是「等第一个写满 N 行就启动」的 waiter 触发的，
   而那个条件在第一个脚本**还剩两条**时就成立了 —— 两个进程同时 `writeFileSync` 同一个
   文件、同时 `git checkout --`，最后工作区里同时躺着**两条**不同探针的改动，
   日志里还留下一行被截断的输出。受影响的六条只能全部重跑。

**两条硬规矩**：跑批期间 ① **不提交任何东西**；② **同一时刻只允许一个跑批进程**
（要分批就串行接在同一个脚本里，别用「等日志写到第 N 行」这种条件去并发触发）。
**收尾自检**要看两样：`git status` 干净，以及**不带任何变异的一次对拍是 0 失败**
——第四十五批的重跑脚本因此在第一条探针**之前**先跑一次基线，把「还原到底干不干净」
变成一个打印出来的数字而不是假设。
...
⚠⚠ **「假的 0」的第六种形状（第四十七批）：探针的量级小于「会被丢弃的缓冲」。**
给蒙面强盗的嘲讽（效果表本来是空的）补一条 **1 点**伤害红 **0 例**，补 **99 点**红
**119 例**（= 整个编队）。那 1 点被玩家回合末**剩余的格挡**吃掉，而剩余格挡在下一个玩家
回合开始本来就清零 —— 于是它**在两帧快照之间凭空消失**。

> **判据：探针要改的那个量，必须大于沿途任何「会被丢弃的缓冲」。**
> 玩家格挡每回合清零、怪物格挡在怪物回合开始清零，所以「往怪物回合里加一点点伤害」
> 这类探针天然可能量到假 0。要么把量级抬到穿透缓冲，要么改成量一个不会被清零的东西
> （血量 / 层数 / RNG 计数器 / 意图历史）。

⚠⚠ **兑现一条盲区之后，把例数按文件拆开再下结论（第四十七批）。**
「`MINION_LEADER` 在蜥蜴法师 / 觉醒者身上的判胜路径」这条盲区，本批的探针合计红 56 例，
看着像关掉了；按文件一拆才发现 **`reptomancer@tgt1` 与 `awakened_one@tgt1` 都是 0**
——那 56 例里真正的增量来自 `gremlin_leader@tgt1`，而蜥蜴法师那一半其实**早在第四十批
就被 `reptomancer@relic4` 关掉了**（旧盲区表把两只怪写成一条，是错的）。

> **判据：合计非 0 完全可能来自另一个宿主，而被点名的那个还是 0。**
> 这与第三十五 / 三十六批「两个增量方向要逐条拆开量」是同一条教训的另一面。
...
- ⚠⚠ **「未迁移**编队**」的样本没有永久解，每隔几批就要换一次——把这件事当成常规维护，
  ...
  ⚠⚠ **第四十七批起「样本用光了」这件事第一次真的发生，办法值得照抄**：`MOVE_RULES`
  铺满 65 / 65 之后，「在 `enemies.ts` 里却不在 `MOVE_RULES` 里」的怪一只都不存在了，
  而造哨兵怪是被明令禁止的（见下面那条 ⚠）。做法是**把那条用例改成断言「让那道 throw
  不可达的不变量本身」**：导出 `SUPPORTED_MONSTER_IDS`，断言它与 `ALL_ENEMIES` 双向相等
  且恰好 65；那道 throw **留着**当第二道闸（守「参考将来加了第 66 只怪」）。
  这比原来那条样本用例**更强**——原来只证明「有一只没登记的会抛错」，现在证明
  「一只没登记的都没有」——而且**不会再被下一批顶掉**。
  ✅✅ **第四十八批：编队那条用例也遇到了同一件事，照抄了这个办法。** 63 / 63 装满之后
  「真实存在却没登记的编队」一个都不剩，于是那条用例改成断言
  **`REFERENCE_ENCOUNTERS`（从参考 `MonsterEncounters.h` 的 `monsterEncounterEnumNames[]`
  逐条抄下来，去掉 `INVALID`）与 `SUPPORTED_ENCOUNTERS` 双向相等且恰好 63**。
  ⚠ **两条用例的不变量来源不同，这个差别要记住**：怪物那条能拿仓库里的 `ALL_ENEMIES`
  当另一侧（数据表本身就是参考的转写），编队这条**不能**——`ENCOUNTERS` 里还混着 13 条
  旧近似表的残留（参考没有对应枚举），双向相等会当场红。所以编队这条的另一侧是
  **手抄的参考枚举名单**，它仍然是独立来源，只是来源在参考里而不在仓库里。
  ⚠ **那道 throw 仍然留着，样本也还在**，只是它守的东西换了：从「还没铺到的编队」变成
  「旧近似表的残留」（样本 `two_orb_walkers`）。接 `sts-encounters` 那一批把那 13 条删掉时，
  这条用例会**第三次**需要换写法——那时它就与卡牌那条一样，只能靠「参考压根没有的字符串」。

  ⚠ 下面这一整段（「挑最晚才会被登记的那个」那条判据与它的历史）**从第四十八批起是历史
  记录，不再是操作指南**：编队与怪物两条线都收官了，没有「下一批会被登记的编队」可挑。
